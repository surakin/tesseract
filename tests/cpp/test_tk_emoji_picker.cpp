...
TEST_CASE("GridView wraps cells across the viewport's width",
          "[tk][gridview]") {
    Stage st;
    GridView grid;
...
TEST_CASE("GridView::index_at recognises cell coordinates",
          "[tk][gridview]") {
    Stage st;
    GridView grid;
    grid.set_cell_size(20, 20);
...
TEST_CASE("GridView click fires on_cell_clicked", "[tk][gridview]") {
    ...
    GridView grid;
    grid.set_cell_size(20, 20);
    CountingGridAdapter ad; ad.n = 30;
    grid.set_adapter(&ad);
    int clicked = -1;
...
TEST_CASE("EmojiPicker tab click switches category",
          "[tk][view][emoji]") {
    Stage st;
    EmojiPicker picker;
    st.run(picker, { 0, 0, 320, 360 });
    ...
    std::string picked;
    picker.on_selected = [&](const std::string& g) { picked = g; };
...
TEST_CASE("EmojiPicker grid click emits the glyph",
          "[tk][view][emoji]") {
    Stage st;
    EmojiPicker picker;
    st.run(picker, { 0, 0, 320, 360 });
