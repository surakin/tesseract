...
public:
    D2DTestSurface(int w, int h) : w_(w), h_(h)
    {
        ...
        f.wic->CreateBitmap(static_cast<UINT>(w), static_cast<UINT>(h),
        ...
        f.d2d->CreateWicBitmapRenderTarget(wic_bitmap_.Get(), props,
                                           rt_.GetAddressOf());
    ...
    tk::Color read_pixel(int x, int y) override
    {
        ...
        return tk::Color::rgba(unmul(pixel[2]), unmul(pixel[1]),
                               unmul(pixel[0]), a);
    }
