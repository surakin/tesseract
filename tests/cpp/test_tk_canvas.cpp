...
TEST_CASE("Canvas::push_clip_rect restricts subsequent paint",
          "[tk][canvas]") {
    auto s = TestSurface::create(64, 64);
    auto& c = s->canvas();
    c.clear(Color::rgb(0xffffff));
...
TEST_CASE("Canvas::push_clip_rounded_rect masks corners",
          "[tk][canvas]") {
    auto s = TestSurface::create(64, 64);
    auto& c = s->canvas();
    c.clear(Color::rgb(0xffffff));
...
TEST_CASE("CanvasFactory::build_text returns a measurable layout",
    ...
    auto layout = s->factory().build_text("Hello, world",
                                           tk::TextStyle{});
    REQUIRE(layout);
...
TEST_CASE("Canvas::draw_text writes glyphs into the surface",
          "[tk][canvas]") {
    auto s = TestSurface::create(200, 50);
    auto& c = s->canvas();
    c.clear(Color::rgb(0xffffff));
...
TEST_CASE("Canvas::draw_initials_circle fills bg and draws letter",
    ...
    auto& c = s->canvas();
    c.clear(Color::rgb(0xffffff));
    c.draw_initials_circle("Alice",
                            Point{32, 32}, /*diameter=*/40.0f,
