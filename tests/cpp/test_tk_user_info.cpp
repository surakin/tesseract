...
TEST_CASE("UserInfo image_provider receives the avatar URL on paint",
    ...
    info.set_image_provider([&](const std::string& mxc) -> const tk::Image* {
        requested = mxc;
...
TEST_CASE("UserInfo primary callback fires on a click that lands inside the row",
          "[tk][view][user_info]") {
    Stage st;
    UserInfo info;
    ...
    bool fired = false;
    tk::Point seen{};
    info.on_primary = [&](tk::Point world) { fired = true; seen = world; };
...
TEST_CASE("UserInfo primary callback does NOT fire on a release outside the row",
          "[tk][view][user_info]") {
    Stage st;
    UserInfo info;
    ...
    bool fired = false;
    info.on_primary = [&](tk::Point) { fired = true; };
...
TEST_CASE("UserInfo::on_secondary is settable and survives paint cycles",
    ...
    bool fired = false;
    info.on_secondary = [&](tk::Point) { fired = true; };
