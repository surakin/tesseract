...
TEST_CASE("find_complete — no match when closing colon not immediately before cursor/space") {
    ShortcodeEngine eng;
...
TEST_CASE("lookup — returns Unicode emoji matches") {
    ...
    for (const auto& s : results)
        if (!s.glyph.empty()) { found_unicode = true; break; }
    CHECK(found_unicode);
}
...
TEST_CASE("lookup — returns custom emoticon matches") {
    ...
    for (const auto& s : results)
        if (s.glyph.empty() && s.shortcode == "happy_cat") { found_custom = true; break; }
    CHECK(found_custom);
}
