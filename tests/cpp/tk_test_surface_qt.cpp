...
public:
    QtTestSurface(int w, int h)
        : img_(w, h, QImage::Format_ARGB32_Premultiplied),
          painter_(&img_),
    ...
    ~QtTestSurface() override {
        if (painter_.isActive()) painter_.end();
    }
    ...
    tk::Color read_pixel(int x, int y) override {
        ...
        return tk::Color::rgba(
            static_cast<std::uint8_t>(qRed(px)),
            static_cast<std::uint8_t>(qGreen(px)),
            static_cast<std::uint8_t>(qBlue(px)),
            static_cast<std::uint8_t>(qAlpha(px)));
    }

    int width()  const override { return img_.width();  }
    int height() const override { return img_.height(); }
