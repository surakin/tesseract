...
namespace
{
bool contains_markdown(const std::string& hay, const char* needle)
{
...
TEST_CASE("markdown_inline: renders inline emphasis without block wrapper")
{
    ...
    CHECK(html == "<em>x</em>");
    CHECK_FALSE(contains_markdown(html, "<p>"));
}
...
{
    CHECK(contains_markdown(markdown_to_html("**b**").formatted_body,
                   "<strong>b</strong>"));
    CHECK(contains_markdown(markdown_to_html("*i*").formatted_body, "<em>i</em>"));
    // pulldown-cmark (CommonMark) nests *** as <em><strong> (outermost first).
    CHECK(contains_markdown(markdown_to_html("***bi***").formatted_body,
                   "<em><strong>bi</strong></em>"));
    CHECK(contains_markdown(markdown_to_html("~~s~~").formatted_body, "<del>s</del>"));
    auto code = markdown_to_html("`x<y`").formatted_body;
    CHECK(contains_markdown(code, "<code>x&lt;y</code>")); // escaped inside code
}

TEST_CASE("markdown: underscore emphasis only at word boundaries")
{
    CHECK(contains_markdown(markdown_to_html("_em_").formatted_body, "<em>em</em>"));
    CHECK(contains_markdown(markdown_to_html("__bold__").formatted_body,
                   "<strong>bold</strong>"));
...
TEST_CASE("markdown: links are http(s) only")
{
    ...
    CHECK(contains_markdown(ok, "<a href=\"https://example.org\">site</a>"));

    // javascript: / other schemes must never become an <a>.
    auto bad = markdown_to_html("[x](javascript:alert(1))");
    CHECK_FALSE(contains_markdown(bad.formatted_body, "<a "));
    CHECK_FALSE(
        contains_markdown(bad.formatted_body, "javascript:alert")); // escaped/plain
}

TEST_CASE("markdown: HTML is escaped (XSS guard)")
{
    auto r = markdown_to_html("**<script>alert('x')</script>**");
    CHECK(contains_markdown(r.formatted_body, "<strong>"));
    CHECK(contains_markdown(r.formatted_body, "&lt;script&gt;"));
    CHECK_FALSE(contains_markdown(r.formatted_body, "<script>"));
}
...
TEST_CASE("markdown: blockquote / lists")
{
    ...
    CHECK(contains_markdown(bq, "<blockquote>"));
    CHECK(contains_markdown(bq, "<p>quoted</p>"));
    CHECK(contains_markdown(bq, "</blockquote>"));
    // pulldown-cmark emits newlines around list items.
    auto ul = markdown_to_html("- one\n- two").formatted_body;
    CHECK(contains_markdown(ul, "<ul>"));
    CHECK(contains_markdown(ul, "<li>one</li>"));
    CHECK(contains_markdown(ul, "<li>two</li>"));
    CHECK(contains_markdown(ul, "</ul>"));
    auto ol = markdown_to_html("1. a\n2. b").formatted_body;
    CHECK(contains_markdown(ol, "<ol>"));
    CHECK(contains_markdown(ol, "<li>a</li>"));
    CHECK(contains_markdown(ol, "<li>b</li>"));
    CHECK(contains_markdown(ol, "</ol>"));
}
...
TEST_CASE("markdown: fenced code block escapes content")
{
    ...
    CHECK(contains_markdown(r.formatted_body, "<pre><code>"));
    CHECK(contains_markdown(r.formatted_body, "let x = a &lt; b;"));
    CHECK(contains_markdown(r.formatted_body, "</code></pre>"));
}

TEST_CASE("markdown: fenced code block records the language as a class")
{
    auto r = markdown_to_html("```rust\nfn main() {}\n```");
    CHECK(contains_markdown(r.formatted_body, "<pre><code class=\"language-rust\">"));
}

TEST_CASE("markdown: fence language is lower-cased and first-token only")
{
    auto r = markdown_to_html("```Python title=foo\nx = 1\n```");
    CHECK(contains_markdown(r.formatted_body, "<pre><code class=\"language-python\">"));
    CHECK_FALSE(contains_markdown(r.formatted_body, "title"));
}
...
TEST_CASE("markdown: fence language cannot inject markup (sanitized class)")
{
    ...
    CHECK(contains_markdown(r.formatted_body, "<pre><code class=\"language-js\">"));
    CHECK_FALSE(contains_markdown(r.formatted_body, "<img"));
}
...
TEST_CASE("markdown: a language-less fence still emits a bare code block")
{
    auto r = markdown_to_html("```\nplain\n```");
    CHECK(contains_markdown(r.formatted_body, "<pre><code>"));
    CHECK_FALSE(contains_markdown(r.formatted_body, "class=\"language-"));
}
...
TEST_CASE("markdown: U+2028 line separator is treated as newline (QTextEdit Shift+Enter)")
{
    ...
    CHECK(contains_markdown(r.formatted_body, "<pre><code class=\"language-python\">"));
    CHECK(contains_markdown(r.formatted_body, "x = 1"));
    CHECK(contains_markdown(r.formatted_body, "</code></pre>"));
}
