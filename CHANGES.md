...
- fix(threads): the per-thread unread dot now survives an app restart. Two changes. (1) `send_thread_read_receipt` was discarding the acked event id at the FFI boundary and re-deriving the receipt target from the `ThreadListService` (unsubscribed when a single thread is opened without the list panel), falling back to the thread *root* — an event the homeserver may not store as a thread receipt; the acked reply id (`MessageListView::newest_visible_real_event_id()`) is now threaded through `send_thread_read_receipt(room_id, thread_root, event_id)` and used verbatim. New pure `pick_thread_receipt_target` helper. (2) Synapse's MSC3960 sliding-sync receipts extension does not echo the user's own threaded read receipts back into the SDK state store on the live path (confirmed by instrumentation on Synapse 1.160; cf. element-hq/synapse#17247, element-hq/element-web#24392), so `load_user_receipt` returned nothing after restart and every read thread reverted to unread. The optimistic `thread_read_markers` map is now persisted to a dedicated per-account `thread_read_state.db` (write-through on send, seeded on `start_sync`, pruned when a real receipt does catch up) — the same local-persistence approach the mobile Element clients use. The file survives "Clear all caches" (it is read-state, not cache); `logout` clears it with the data dir. Linux (Qt6 + GTK4) build + full ctest; macOS/Windows share the code, unbuilt. +7 Rust tests
- fix(tk): a disabled widget is now opaque to input. Every tree-walking dispatcher (`dispatch_pointer_down` / `_move` / `_right_click` / `_file_drop` / `_drag_hover`, `hit_test`, `dispatch_key_down`) stops at a disabled widget instead of relying on its handler to reject the event, so a disabled `Button` — or any disabled widget, leaf or container — swallows clicks / hover / right-clicks / drops rather than passing them to the widget painted behind it or to its own children. Disabled subtrees are also skipped entirely by Tab traversal, and clicking a disabled widget no longer clears keyboard focus. `dispatch_wheel` is unchanged (the page still scrolls when the pointer is over a disabled area). Linux (Qt6 + GTK4) build + full ctest; macOS/Windows share the code, unbuilt. +6 C++ tests (3 existing pass-through tests updated to the absorb behaviour)
...
- fix(windows): the DirectComposition login surface no longer paints over the custom title bar (`LoginView::layout()` was resetting its HWND to `(0,0)` every call). Unbuilt (written from Linux)
