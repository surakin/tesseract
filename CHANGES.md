...
- feat(rooms): the room info panel's member list now has a real scrollbar — it previously had none at all (mouse-wheel only, no visual indicator, no drag), making a long expanded list unusable. Split `RoomInfoPanel` into a thin outer chrome widget (backdrop + fixed header) and a new `RoomInfoPanelBody` that genuinely inherits `tk::ScrollableBase` (mirrors `ImagePackEditorView`/`ImagePackSectionList`'s existing fixed-header-plus-scrollable-body composition), so it gets the same painted/draggable thumb every other scrollable view has. Public API unchanged — zero call-site changes in `RoomView.cpp` or any platform shell. Linux (Qt6 + GTK4) build + full ctest, 1850/1850; unverified live this session. Windows/macOS share the code, unbuilt
- fix(compose): on Windows, `/selfie` froze the app instead of showing the camera overlay. `MainAppWidget::any_modal_open_()` (the shared gate `arrange()` uses to force-hide the compose bar's native text area within the same layout pass) didn't know about the camera/screen-picker overlays, so Windows' own post-arrange `set_on_layout` hide fought `ComposeBar::arrange()`'s unconditional re-show every pass, each flip rescheduling another full relayout — a self-sustaining relayout storm that starved `WM_PAINT`. `any_modal_open_()` now includes both overlays, matching every other modal. Windows build + full ctest, 1824/1824; user-verified live. Qt6/GTK4/macOS share the fix, unbuilt this session (macOS had the identical latent gap in its own post-arrange hide, not yet reported there)
...
- fix(theme): on Windows, picking any accent theme other than Blue in Settings → Appearance had no visible effect — `MainWindow::apply_theme_ui_` unconditionally overwrote the resolved palette's accent/unread/selection colors with one derived from the Windows system accent color, a pre-existing convention that predates the accent-theme picker and wasn't gated on which accent the user had actually chosen. Added a `System` accent option (now the default, preserving today's live "always match the OS accent color" Windows behavior for existing settings) so that convention becomes an explicit, opt-in choice; any other accent (Blue/Forest/Sunset/Violet) is now left alone. Linux (Qt6 + GTK4) build + full ctest, 1850/1850 (+3); Windows fix unverified live this session (no Windows toolchain available), reported and root-caused from a live user test on Windows. macOS unaffected (no equivalent override existed there)
- feat(theme): three new accent-color themes (Forest, Sunset, Violet) alongside the existing Blue, picked from a new "Accent color" combo in Settings → Appearance. Each accent is generated from a single seed hue; only the accent-dependent Palette fields (accent/hover/pressed, reaction-chip "me" colors, unread badge, avatar-initials disc, selection) are derived, while every neutral/surface field stays the existing hand-tuned Light/Dark palette. Blue itself is unchanged (not regenerated) so the default theme carries zero regression risk. Backed by a new WCAG contrast-ratio utility (`tk::color_contrast`); every accent/mode combination is checked against the same AA floor already implicit in `theme.cpp`'s hand-written contrast comments. Linux (Qt6 + GTK4) build + full ctest, 1847/1847 (+13); unverified live this session. Windows/macOS share the code, unbuilt
...
- feat(mentions): typing `@room` in the composer and accepting it from the mention popup now shows the room's own avatar in the inserted pill too (previously only received/timeline `@room` pills got one). Root cause: GTK4/Win32/macOS's *main windows* built `MentionController::Hooks` inline instead of via `RoomPane::wire_mention_hooks_` (only pop-out windows and Qt6's main window used that shared helper), so the new `resolve_room_avatar` hook never reached 3 of the 4 main-window composers regardless of cache state. All three now consolidate onto `wire_mention_hooks_`, matching the proven-working pop-out/Qt6 path. Separately, the avatar-arrives-after-insert case (composer pill is a baked bitmap, so a plain repaint can't refresh it) now patches in place via a new `TextArea::refresh_room_mention_avatar()` (all four composer backends) triggered by `RoomPane::notify_avatar_media_ready_()`, called from each shell's existing `on_media_bytes_ready_` — the same event-driven signal every other avatar consumer in the app already relies on — replacing an earlier timer-based poll loop (and its pre-existing per-user analogue) that didn't match how media arrival is handled anywhere else in the codebase. Linux (Qt6 + GTK4) build + full ctest, 1834/1834; unverified live this session. Windows/macOS share the code, unbuilt
...
- perf(media): animated WebP stickers loaded noticeably slower on macOS than the other platforms, since ImageIO's per-frame realization cost scales with a frame's position in the animation regardless of decode order or entry point used — a genuine ImageIO limitation, not fixable within it. macOS now decodes animated WebP via a vendored libwebp `WebPAnimDecoder` instead, on both the picker's whole-batch and the timeline's windowed decode paths; GIF/APNG are unaffected. macOS: user-verified live (~30-40x faster on previously-slow stickers, no color issues); Windows/Qt6/GTK4 unaffected (not on this code path)
...
- refactor(media): image caches (`PixmapCache`, `AnimImageCache`, `CompressedImageCache`, `MediaDiskCache`, `viewer_fullres_`, GIF/pill caches) now key on a typed `tk::CacheKey` instead of hand-prefixed strings (`"t{w}x{h}:"`, `"thumb::"`, `"blurhash::"`, etc.), closing a real collision risk that had already forced a manual `starts_with("thumb::")` workaround. `CacheKey::to_string()` reproduces the old prefixes exactly, so existing on-disk cache files stay valid. Windows build + full ctest, 1814/1814; Qt6/GTK4/macOS share the code, unbuilt this session
