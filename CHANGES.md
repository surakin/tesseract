...
- fix(accounts): the account-switcher popup clipped its last row and drew a square 1px border. After the sidebar strip/avatar grew (MSC4426), the shared `AccountPicker` row measured taller than the per-row height each of the four shells had hardcoded — the shells now size the popup from `AccountPicker::measure()` (GTK4/Win32 also weren't resizing it on an account-count change). The picker now paints its own rounded card and the host window is chrome-free (Qt6 translucent, GTK4 flat popover, Win32 rounded region, macOS clear NSPopover content). Linux (Qt6 + GTK4) build + full ctest, 1765/1765; Windows/macOS per-shell parts written from source, unverified on-device
