...
- feat(ui): Appearance → Layout now shows a live preview next to the message-layout combo box — two fake messages ("Hello my friend!" / "Hi! How are you?") rendered through the real Classic/Bubbles/IRC row renderer, updating immediately as the combo box changes. New `MessageLayoutPreview` widget hosts a real (but inert — no live room, no avatar/image providers) `MessageListView` fed two hand-built rows in a fixed-size swatch beside the combo/description column. Windows-verified (win32 shell); other shells share the code, unbuilt
- feat(threads): "mark all threads as read" button (Lucide `list-checks`) in the thread-list panel header, immediately left of the close button; disabled (greyed) whenever no thread is unread. New batched `Client::mark_all_threads_read(room_id)` FFI iterates the room's thread list and sends MSC3771 threaded receipts (`ReceiptThread::Thread`) for every thread with an unread foreign reply, concurrently, writing optimistic markers + firing `on_threads_updated` once so all the dots clear immediately. `send_thread_read_receipt` refactored to share the new `send_thread_receipt_pair` helper. Linux (Qt6 + GTK4) build + full ctest; macOS/Windows share the code, unbuilt. +2 C++ tests
- feat(threads): unread-thread indicator — a dot on the room-header threads button (and its overflow-menu fallback), accent-coloured when an unread reply mentions you, neutral otherwise, whenever any thread in the room has replies you haven't read; a matching per-row dot in the thread-list panel with the read row's title dimmed. Read state comes from MSC3771 threaded read receipts, which Tesseract now also sends: the open thread panel marks its thread read as it's viewed (`send_thread_read_receipt` FFI → `Room::send_single_receipt` with `ReceiptThread::Thread`), backed by an optimistic local marker so the dot clears before the sync echo. New `ThreadInfo::unread` / `mentions_me` FFI fields (enriched in `list_room_threads` via `Room::load_user_receipt`), `thread_is_unread` / `aggregate_threads` pure helpers. v1 scope: latest-reply `m.mentions` only (no push-rule eval); receipt-send-time vs reply-timestamp fallback comparison; cross-device read clears the local dot only when a new reply lands. Linux (Qt6 + GTK4) build + full ctest; macOS/Windows share the code, unbuilt. +6 C++ / +12 Rust tests
- fix(join): the Join / "Request to Join" button in the Add Room → Join tab (also the target of `matrix.to` / `matrix:` permalink joins) is disabled for the duration of the async join, so a second click can no longer fire a duplicate `join_room_async` / `knock_room_async`. Enters the already-declared `State::Joining`: preview card stays visible with a "Joining room…" / "Sending request…" status line; existing outcome handling re-enables or closes the dialog. +3 C++ tests
...
- fix(join): joins now route through via servers, so a room listed in a Space (or reached by permalink) that lives on another homeserver joins instead of failing with "join failed or cancelled". `resolve_route_via()` gathers routing servers in priority order — permalink `?via=`, the Space's `m.space.child` via list, then the room-id/alias domain (local state only, no network) — threaded through `join_room` / `knock_room` / `get_room_summary`, the cxx bridge, and `ShellBase`. `matrix_uri.rs` now retains `?via=` rather than stripping it; `join_room_async` surfaces the homeserver's real error instead of the generic string. Unverified end-to-end (no local federation rig)
- fix(freeze): the known-users roster sweep no longer freezes the client on a stalled `GET /rooms/{id}/members`. `get_room_members` now uses `room.members_no_sync()` (a pure store read) instead of `room.members()`, which fired an untimed member fetch for every freshly joined room — after joining a Space, one stalled request sat forever in `block_on` holding the shared FFI lock, and libc++'s writer-preferring `shared_mutex` then starved every later lock including the UI thread's. `poll_presence_now` also drops to a shared lock (spawns its kick like `backfill_task`) to remove the writer-priority starvation entirely. Built + tested on the presence/room-list/roster subset; unverified against the live repro
- feat(power): low power mode — Settings → General → Power: `Auto` (default) / `On` / `Off`. When active the client keeps only the sliding-sync long-poll running: it halts background timeline backfill, unread/favorite prefetch, bridge-status checks and the 2 s decoded-image GC timer (C++), and pauses the search-index crawl and per-room warm-check auto-pagination (SDK, via one new `Client::set_low_power_mode` FFI flag). Message sync, encryption sync and user-driven pagination are untouched. `Auto` resolves `on-battery || OS-energy-saver` with a ~20 s debounce. On a machine with no battery (`IPowerMonitor::has_battery()` — UPower device enumeration / `SYSTEM_POWER_STATUS.BatteryFlag` / `IOPSCopyPowerSourcesList`) the whole feature is hidden and inert: the Settings group is not shown, the status glyph never appears, and a persisted `On` is ignored (but left in `app_settings.json`). New `tesseract::IPowerMonitor` DI interface (mirrors `IScreenLock`) with per-platform probes — UPower + power-profiles-daemon D-Bus on Linux Qt/GTK, `GetSystemPowerStatus` + `RegisterPowerSettingNotification` on Win32, `NSProcessInfo` + IOKit `IOPowerSources` on macOS — plus `PowerPolicy`, a pure pref+signals resolver, and a subtle status-bar glyph (Lucide `battery-low`, rasterized + theme-tinted; the shells reach the generated icon bytes via a new `tk::low_power_icon_svg()` accessor rather than plumbing `icons.h` into each target). `presence_polling_enabled` now has one writer (`resolve_presence_polling_`) folding presence setting + window focus + low power. Linux (Qt6 + GTK4) build + full ctest; Windows/macOS share the code, unbuilt. +9 C++ / +1 Rust tests
...
- fix(tk): `Widget::set_enabled()` now requests a repaint of its own bounds when the flag actually changes. Previously it only stored the flag, so a widget disabled/enabled off the back of a scoped-repaint-only event (e.g. the compose bar's Send button, toggled from a text-changed callback that on Qt6 only invalidates the text field's own rect) stayed visually stale until an unrelated full repaint happened to touch its pixels. Linux (Qt6 + GTK4) build + full ctest. +1 C++ test
