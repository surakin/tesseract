...
- fix(macos): four compounding compose-bar text-area glitches surfaced while finishing the empty-placeholder inset work (e61dd669). (1) A restored per-room draft (da318969) rendered in a visibly different weight/size from freshly typed text on the same line — `NSTextViewNative::set_text()` edited `textStorage.mutableString` directly, leaving the inserted characters with no font attribute so TextKit fell back to its substitute font; it now sets an `NSAttributedString` built from `view_.typingAttributes` (what a real keystroke applies) and re-runs `reformat_emoji_runs()`. (2) The placeholder sat ~1–5px off from where real text begins and looked subtly different — it was an `NSTextField` laid out by `NSStringDrawing` while the real text is an `NSTextView` laid out by `NSLayoutManager` (different engines, different first-glyph origin, no amount of frame math closes it). Replaced with a non-editable `TKPlaceholderTextView : NSTextView` configured identically (same `textContainerInset`, `lineFragmentPadding`, font) and framed onto `scroll_`, so glyphs land in the same place by construction; `hitTest:` returns nil so a click on the empty field still focuses the text view under it. (3) That new text view double-counted `textContainerInset.height` against `natural_height()`, inflating the empty-state row by ~12px — `placeholder_natural_height()` now returns the bare wrapped-glyph height like the old `intrinsicContentSize` did. (4) The canvas-drawn blinking caret didn't appear until the first keystroke: `notify_focus_gained()` (which starts the blink timer) was wired only to `-textDidBeginEditing:`, which for a document `NSTextView` fires on the first *edit*, not on the programmatic `makeFirstResponder:` the compose bar uses for its default focus — `TKComposeTextView` now also drives it from `-becomeFirstResponder`/`-resignFirstResponder` (made idempotent), and `cursor_rect()` returns a real line-height rect for an empty document instead of the zero rect `boundingRectForGlyphRange` gives with no glyph. `set_rect()` also floors the applied-rect x so the captured text bitmap blits 1:1. User-verified live on macOS; other shells unaffected (backend-local)
- feat(shell): while the app-settings page is open, the main window title now stays plain `Tesseract` instead of `Tesseract - <room>`, and the room name comes back when settings closes (← Back, logout, or identity reset). Rather than repeat the "settings open?" guard and the current-room lookup across all four shells (the naive follow-up to 80b61bd8/26), the title string is composed once in a new `ShellBase::compose_window_title_()` — gated on a new `app_settings_open_` flag toggled via `set_app_settings_open_()` — and pushed to the OS through a single new `apply_window_title_ui_()` virtual that each shell implements with its one native call (`SetWindowTextW` + title-bar strip invalidation on Windows, `setWindowTitle` on Qt6, `gtk_window_set_title` on GTK4, `NSWindow.title` on macOS). The four per-shell helpers (`set_main_window_title_` / `updateWindowTitle_` / `update_window_title_` / `-updateWindowTitle:`) and their room-name plumbing at the `on_room_selected` / `on_rooms_updated_` / `on_left_room` call sites collapse to a bare `refresh_window_title_()`. Because the guard lives inside `compose_window_title_()`, the frequent `on_rooms_updated_` sync ticks can't flicker the title back to the room name while settings is open. Windows drops its own `settings_visible_` bool for the shared flag. New shared test `test_shell_window_title.cpp`. macOS build + full ctest verified (1 pre-existing unrelated SecretStore-migration failure); Qt6/GTK4/Windows written to the same shared design but not built here
- fix(gtk4): progressive video streaming (da488022) shipped with no live verification and was actually completely broken — not just for streaming, for all GTK4 video playback. `finish_pipeline_and_start_()` created the `audioconvert`/`videoconvert` elements with a `nullptr` name, so GStreamer auto-suffixed them (`videoconvert0`, `audioconvert0`); `on_pad_added_()` links `decodebin`'s dynamically-exposed pads via an exact-match `gst_bin_get_by_name(pipe, "videoconvert")`/`"audioconvert"` lookup that never found the auto-suffixed names, and its fallback (matching `gst_element_factory_get_longname()`, e.g. `"Colorspace converter"`) could never match either — every decoded pad went permanently unlinked. `decodebin`'s internal `multiqueue` absorbed a handful of buffers before the missing link surfaced, then `qtdemux`'s own streaming loop hit `not-linked` and the pipeline died with a bus ERROR, well before a user ever got the chance to press play. Confirmed via a `GST_DEBUG` capture of a live repro (same file played fine on Qt6) plus a standalone `gst-launch-1.0` naming test. Fixed by naming the elements explicitly and correcting the fallback to compare short factory names instead of the human-readable longname. User-verified live on GTK4
- fix(macos): progressive video streaming (f12e7fc3) also shipped broken — its `TkVideoStreamLoader` (`AVAssetResourceLoaderDelegate`) mishandled AVFoundation's content-information request two ways: it always answered with `contentLength = 0` (`begin_stream(mime, 0)` runs before any bytes or a known total arrive, so `_knownLength > 0 ? _knownLength : _buffer.length` evaluated to 0), and since the same pass also set `contentType`, the `!contentType` guard then blocked ever correcting it later — AVFoundation concluded the resource was empty and never started playback. Separately, a content-information-only request (`dataRequest == nil`) was never finished, so AVFoundation waited on it forever. `tryFulfill:` now leaves the content-information request pending until the real total length is known (`setTotalLength:`/`endWithHint:`), then populates it and calls `finishLoading` for the info-only case — resolving on the first streamed chunk, which carries the HTTP Content-Length. User-verified on macOS
- fix(shell): the room-list search field's debounce wiring (`on_changed` → debounce → `set_search_text` + a shell-level `refresh_room_list_()`) was duplicated across all four shells, and macOS was the only one that dropped the `refresh_room_list_()` half — its `_applySearchFilter` called only `set_search_text`. That call does two things search needs: swap `RoomListView` from the root-filtered set to the full `rooms_` set while a search is active (so search matches every room, not just root-level ones), and request a relayout — without it, macOS search neither repainted immediately (the list only updated on the next incidental pointer-move paint) nor searched past root rooms. Hoisted the whole wiring into `ShellBase::wire_main_app_widget_()`, where every other `RoomListView` callback already lives; `is_room_search_active_()` becomes a non-virtual `ShellBase` method backed by a new `room_search_text_` member, deleting the four per-shell pending-text members/overrides it replaces. User-verified on macOS
- fix(shell): the main window's title never actually followed the active room on Qt6/GTK4/macOS (80b61bd8) — `RoomPane::Deps::update_window_title` was wired on all four platforms, but `RoomPane` only ever calls it from `finish_init()` (a pop-out-only path, never run for the main window) and `on_room_info_updated()` (only invoked for secondary/pop-out windows), so the callback was live but never triggered for `main_room_pane_`. Windows worked because its implementation never actually went through `RoomPane`/`Deps` for the main window — it called its own `set_main_window_title_()` directly from `on_room_selected`/`on_rooms_updated_`/`.on_left_room`. Qt6 and GTK4 now do the same (`updateWindowTitle_()`/`update_window_title_()` called from those same three sites); macOS's `.on_left_room` needed an extra fix beyond that pattern — it closes the room's tab via `tab_close()` first, and when that empties the tab list the shared `on_tab_state_changed_ui_()` cascade skips calling back into `onRoomSelected:` (its `active_tab_idx_ < tabs_.size()` guard), so the title reset is now applied directly when `tab_close()` leaves no room selected. User-verified live on Qt6, GTK4, and macOS
- fix(tray): on Wayland (KDE Plasma) a tray-icon click on a visible-but-inactive window only lit up the panel task-manager entry instead of raising the window — the "raise a visible-but-inactive one" case from the tray click rework (251d9fa5). Not a Wayland limitation: Plasma's tray host hands the app a granted, compositor-issued xdg-activation token via the `ProvideXdgActivationToken` D-Bus call it makes just before `Activate`, and Tesseract was discarding it. On Qt6, Qt's own SNI adaptor already stashes that token in `XDG_ACTIVATION_TOKEN`, but `activateWindowWithToken_(QString{})` (the tray path) ignored the env var and instead self-issued a token with a stale input serial (the click went to plasmashell, not us), which KWin's focus-stealing prevention denies; it now consumes the env token first. On GTK4, `GtkSniTrayIcon`'s D-Bus interface didn't even declare `ProvideXdgActivationToken`, so the token call failed outright — it now handles that method, threads the token through the toggle callback, and `start_tray_if_needed_()` applies it via `gtk_window_set_startup_id()` before `gtk_window_present()` (the same mechanism the notification-click path already uses). Win32/macOS unaffected (native activation APIs). Confirmed live on Plasma/Wayland for both shells
- fix(tk): `NativeTextArea::natural_height()` reserved a shorter row while the compose bar was empty than once it had content — the empty-placeholder branch (b9c5ce93, added for the Create Room invite field) measured only the placeholder label and skipped the vertical inset/margin padding the non-empty branch adds, so once focused the text view's document overflowed its too-short frame (cursor low, a sliver of the vertical scrollbar peeking in above it). Fixed on the two affected backends (macOS `NSTextView`, GTK4 `GtkTextView`); Qt6 applies its margins outside the branch and Win32 delegates to a separate control, so neither needed a change
- fix(video/qt6): progressive video streaming (da488022) froze the whole app on essentially every streamed video — Qt's FFmpeg format probe reaches the growable source device via `avio_read` → `QIODevice::atEnd()` → `QIODevice::bytesAvailable()`, and `GrowableQIODevice::bytesAvailable()` held its mutex across the base `QIODevice::bytesAvailable()` call, which for a non-sequential device invokes the virtual `size()` and re-locked the same non-recursive mutex on the demux thread, wedging it while holding the lock; every subsequent UI-thread `feed()`/`set_stream_length()` then blocked on it. `bytesAvailable()` now returns only the bytes buffered past the read cursor with no chain to the base, and a new `atEnd()` override (releasing the lock before deferring to the base) takes over keeping the probe from concluding EOF mid-stream. GTK4's `GrowableGstBuffer` was unaffected (a standalone class, not a re-entrant framework subclass). Qt6 build + full ctest verified; fix confirmed live
- fix(tests): bb2e1897 moved the compose popups' (`QuickSwitcher`/`MessageSearchView`/`ForwardRoomPicker`) `set_query()`/`set_results()` relayout onto the coalesced, deferred `Host::mark_needs_relayout()` path, but four tests still asserted `relayout_count`/`repaint_count` synchronously — each now calls `host.fire_all_ui_tasks()` to run that UI-thread turn first. No production behavior changed
- fix(rooms): own messages could show up to 3× in the timeline after switching to a warm room — b5b72bb1's withheld-tail cap never taught the live `on_message_inserted`/`updated`/`removed` path that the SDK's indices stay relative to the full, untrimmed timeline while `room_view_` now holds a shorter one, so the mismatch dropped the local-echo "sent" transition (own messages stuck on the sending spinner) while `insert_message` clamped the same stale index into a duplicate tail row. `ShellBase`'s four live-update handlers and `RoomPane`'s pop-out equivalents now subtract `RoomPane::withheld_count()` from the incoming index and drop anything still targeting a withheld row; the pop-out dispatch path's `idx==0`/`SIZE_MAX` prepend/append sentinels are replaced with explicit `on_message_prepended()`/`on_message_appended()` methods so a translated index can't collide with a marker value
- feat(tk): `tk::Widget` now requests its own relayout automatically on `set_visible()`, `add_child()`, `remove_child()`, `clear_children()`, and `set_layout_hints()`, instead of every caller having to remember a manual `host()->request_relayout()` afterward — a codebase-wide audit found `ui/shared/tk/` had zero automatic layout invalidation, with the same hand-rolled follow-up call independently reproduced at ~150 call sites. New `Host::mark_needs_relayout()` is the coalesced primitive backing this (cheap to call repeatedly — the first call in a UI-thread turn schedules one deferred flush via `post_to_ui()`, folding in every call before it runs), since the existing `request_relayout()` runs a synchronous, uncoalesced measure+arrange pass. Also closes two related gaps found in the same audit: `tk::TextArea` now requests its own relayout internally when the native control's height changes (previously reimplemented by hand, down to a copy-pasted comment, at four separate call sites in `ComposeBar`/`CreateRoomView`/`RoomGeneralSection`), and `tk::ListView::invalidate_data()` gained an opt-in `container_may_resize` parameter so a self-sizing popup (`ForwardRoomPicker`/`MessageSearchView`/`QuickSwitcher`) no longer needs a second, easy-to-forget relayout call paired with it. Fixes a `MainAppWidget::show_room()` gap that was only safe by virtue of a relayout guarantee several call frames away, by making `RootLayoutWidget::set_active_pane()` request its own relayout on change. Also fixes a real app freeze this surfaced during testing: `RoomListView::arrange()` unconditionally shows the room-search field while `MainAppWidget::arrange()` force-hides it again whenever the sidebar isn't in view (narrow-mode/modal gating) — a harmless double `ShowWindow` every pass before this change, but now a self-sustaining relayout storm, since each flip is a real transition. Fixed generally (not just for this one case) by having each backend's relayout pass suppress `mark_needs_relayout()` scheduling for its own duration (`Host::begin_relayout_pass_()`/`end_relayout_pass_()`) — a widget mutated as a normal part of computing the current pass doesn't need a new one scheduled on top of it. Windows build-verified; Qt6/GTK4/macOS written by the same analogy (the relayout-pass guard touches each backend's own measure+arrange call site) but unverified — no toolchain here for those three
- feat(export): restyle the HTML room-history export to resemble the live timeline instead of a bare unstyled dump — hash-colored sender names and an initials-circle/avatar-photo per sender (fetched and cached once per unique sender, gated on the existing "Include images" option), consecutive-message grouping, reaction pill chips, a reply-quote card, muted styling for redacted/undecryptable/membership rows, and a light/dark palette that follows `prefers-color-scheme`, all pulled from the app's actual theme tokens and sender-color palette. `format::ExportSink::event()` gained an `EventContext` (previous event + resolved avatar path) to support grouping/avatars without shared mutable state; `images::download_images()`'s filename-collision set is now caller-owned so avatar names dedupe across the whole export, not just one window, and downloaded avatar bytes are extension-sniffed from magic bytes (a sender's display name carries no extension of its own, unlike a message image's original filename). Plain-text export is unchanged. Windows build+test-verified
- fix(export): the time-range cutoff's truncation logic was inverted — scanning an oldest-first event vec from the start for the first too-old event and `truncate()`ing to that position always matched index 0 whenever the cutoff was actually crossed, silently emptying the whole batch (a range-limited export could complete with zero messages); fixed to keep the in-range suffix and drop the too-old prefix, and to stop pagination as soon as the approximate oldest-so-far crosses the cutoff instead of always fetching a full window regardless of how narrow the requested range is. A range-limited or Stop-terminated export that fully satisfied its own boundary was also leaving its resume checkpoint behind, so the next export of that room wrongly showed the "previous export was interrupted" prompt for a run that had actually completed exactly as requested — now cleared like a true full-history export (a genuine Stop mid-walk still keeps its checkpoint, since that one really is resumable). The dialog's "Exporting back to {date}" line also always showed the room's creation date regardless of any chosen range; now reflects the actual effective cutoff, echoed on `RoomExportProgress` so it reads correctly whether the dialog just started the export or is reopening on one already in flight. Also fixes two missing relayouts in `ExportHistoryDialog` (declining a resume prompt via "Start new export", and the resume prompt appearing in the first place) that left buttons in stale, unclickable positions until an unrelated repaint happened to fix it. Windows build+test-verified
- feat(export): add a time-range selector (All history / 24 hours / 7 days / 30 days / 3 months / year) and a "Stop & save" action to the history-export dialog — Stop finishes the current window's writes and runs the normal assembly/zip pipeline with whatever was gathered so far, unlike Cancel, which discards it. Both ride on SDK machinery that already existed but was never driven from the UI: the range picker sets `RoomExportOptions::stop_at_ts_ms` (already wired end-to-end, just always 0 before now), and Stop is a new cooperative flag alongside `cancel` that steers the walk into the same "finish and assemble" path the time-range cutoff already used, rather than the discard-everything path Cancel takes. The chosen cutoff is persisted in the resume checkpoint (`stop_at_ts_ms` column, migrated in-place for existing installs) so a resumed export silently reapplies the original bound instead of drifting forward from a freshly recomputed "now". Windows build+test-verified
- perf(ui): a single room switch could fire up to 4 synchronous full-tree relayout passes, an unbounded row-height remeasure of the entire loaded timeline snapshot, and a full room-list rebuild on every sync tick regardless of relevance — now coalesced through the existing `schedule_relayout_` mechanism, with a switch capping what it hands `MessageListView` so `tk::ListView` measures only a viewport's worth of rows (the withheld older tail drains lazily via the existing scroll-up pagination path) and `RoomListView::set_rooms()` skipping its rebuild when the incoming room list is unchanged. (The withheld-tail cap regressed live message updates — see the 3× sent-message duplication fix above.)
- fix(ui): WCAG AA contrast audit of the light/dark `tk::Palette` — `text_secondary`/`text_muted`, `success`, `presence_online`/`presence_unavailable`, and `accent`/`unread_bg` (light) were all below the 4.5:1/3:1 thresholds for their actual text/icon usage; dark-mode `text_on_accent` (white button labels on the light-blue accent fill) only hit 2.63:1, fixed by switching to the same dark-navy tone already used for `unread_text` rather than darkening `accent` itself (which would have broken its already-passing use as link/icon text). Added a new `border_strong` token, applied narrowly to the handful of input-field edges (`LoginView`'s card/fields, the sidebar and picker search boxes) whose fill matches their surrounding card, so the stroke is the only thing that reads as their shape — regular `border`/`separator`/`chip_border` stay at their original decorative values everywhere else. Room-list selection in `RoomListView`'s invite/knock/unjoined rows was color-fill-only (unlike joined rooms, which already had an accent-colored left bar); those three row types now get the same bar, so selection state doesn't rely on `sidebar_selected`'s (deliberately subtle) contrast alone. Windows build+run-verified
- fix(windows): the status bar's font size didn't follow a live DPI change — `Theme::make_font()` sized fonts from a screen DC's `GetDeviceCaps(LOGPIXELSY)` instead of the main window's own DPI, a value per-monitor-DPI-aware processes capture once and never update, so `on_dpi_changed()`'s font-cache invalidation just rebuilt the same wrong size on `WM_DPICHANGED`. Now reads `GetDpiForWindow(g_main_hwnd)`. Also fixes the tray icon's right-click menu not scaling to the correct monitor's DPI: it was owned by a message-only window (`HWND_MESSAGE` parent), which has no monitor association for `TrackPopupMenuEx` to resolve DPI from — added a real (never-shown) top-level window, repositioned to the cursor point before each `show_menu()`, and used as the menu's owner instead. Windows build+run-verified
- feat(shell): add a Ctrl+Tab / Ctrl+Shift+Tab MRU room switcher, Alt-Tab-style — hold Ctrl, tap Tab/Shift+Tab to cycle a snapshot of `ShellBase::recent_room_ids_` (frozen at cycle-start so an incoming message can't reshuffle chips mid-hold), release Ctrl to jump to the highlighted room, Escape to cancel. New shared `MruSwitcher` widget (`ui/shared/views/`) reuses the Ctrl+K quick switcher's "Recent" chip-row rendering via a new extracted `room_chip_strip.h/.cpp` helper rather than duplicating it, and a new `tk::Host::set_on_ctrl_key_up()` hook centralizes commit-on-release for the backends that detect it at the Host level. Ctrl-release detection needed a genuinely native mechanism per platform, since it must keep tracking the key even while a native compose/search control holds real OS focus — same problem the Ctrl+K accelerator already works around for key-down: Windows uses a thread-scoped `WH_KEYBOARD` hook (mirroring the existing `WH_MOUSE` popup-dismiss hook); Qt6 installs an app-wide `QObject` event filter on `Host` (mirroring `QtPopupSurfaceHandle`'s identical outside-click filter); GTK4 attaches a `key-released` signal to the main window (alongside its existing window-scoped Escape handling) and calls `commit_mru_cycle()` directly rather than through the `Host` hook; macOS folds `NSEventMaskFlagsChanged` into the existing local key-event monitor and uses literal Control (not ⌘, unlike every other shortcut there — ⌘Tab is the OS's own App Switcher). Windows build+run-verified; Qt6/GTK4/macOS accelerators now written by the same analogy but unverified — no toolchain here for those three
- fix(ui): the emoji/sticker pickers, mention/slash/shortcode/gif compose autocomplete popups, the account picker, and any generic `ComboBox`/`LanguagePicker`/date-picker dropdown all captured their screen position once at open time and never recomputed or closed it on a window resize, leaving them stranded away from whatever control anchored them — reproducible in both the main window and pop-out room windows, on all four platforms. Each shell's resize handler now dismisses every open popup instead, via a new `RoomView::dismiss_popups()` wrapper, a new shared `hide_all_compose_popups()` helper (`ComposePopups.h`), `Host::dismiss_active_popup()`, and each shell's own account-picker close — mirroring the existing OS-focus-loss dismissal. Windows-verified; user-verified live on Qt6 and macOS; GTK4 still unverified
- feat(shell): show the active room's name in the main window's OS title ("Tesseract" / "Tesseract - " + room name), reverting when no room is selected — wired on all four platforms (`SetWindowTextW` + title-bar strip invalidation on Windows, `setWindowTitle` on Qt6, `gtk_window_set_title` on GTK4, `NSWindow.title` on macOS) via the existing `RoomPane::Deps::update_window_title` hook that pop-out `RoomWindow`s already used but `MainWindow` never wired up
- fix(windows): the custom title bar's window-title text was stuck near-black regardless of theme (unreadable in dark mode) — BetterText bakes each run's color into the document when it's inserted and only falls a run back to the theme brush when its style matches the control's *current* default exactly, and `CustomTitleBar::paint()` called `BetterTextSetText()` before `SetDefaultTextStyle()`/`SetTheme()` on the very first paint, tagging that first-ever run against BetterText's out-of-the-box default so it was never eligible for the fallback afterward. Style/theme now always apply before any text change within the same paint pass
- feat(windows): replace the stock title bar with a self-drawn Windows 11-style extended one (`WM_NCCALCSIZE`/`WM_NCHITTEST` + custom min/max/close buttons) matching the app's Mica/dark-caption theming instead of the leftover default chrome, on both `MainWindow` and `RoomWindow` popouts. Snap Layouts' hover flyout and right-click-for-system-menu are preserved via `DwmDefWindowProc` and a manual `GetSystemMenu` fallback (`DefWindowProc` stops auto-showing the system menu once the caption's non-client rect is zeroed). Caption buttons render Lucide SVG icons (rasterized once per size/color and cached as GDI+ bitmaps via a new `tk::rasterize_svg_rgba()`) rather than hand-drawn GDI primitives
- feat(tk): migrate every `tk::win32::Surface`'s swap chain from `CreateSwapChainForHwnd` to `CreateSwapChainForComposition`, backed by a shared `IDCompositionDevice` and a per-`Surface` `IDCompositionTarget`/`Visual` (with generation-tracked recreation on device loss) — DWM now composites each swap chain's actual current buffer directly instead of stretching a cached redirection bitmap during an interactive resize, fixing a visible "stretch" artifact on window resize across the whole Windows shell
- feat(bettertext): add a static-text control kind (`BetterTextSetStatic`) — read-only, no caret/selection/scrollbar, ellipsis-trimmed single line — and use it to render the title bar's window-title text via an offscreen, region-empty `BetterText` control captured into the title bar's GDI surface, giving the title bar real color-emoji (COLR/CPAL/SVG-in-font) rendering through the app's existing Noto Color Emoji font provider. Also fixes `DrawScrollbarThumb` drawing a stray overlay thumb for static controls when a font's natural line metrics exceed the control's fixed height (seen with the emoji fallback font), and a title bar flicker on window activation caused by two sequential GDI draws into the same live window DC that DWM's compositor could sample between (confirmed live via debugger) — both `MainWindow` and `RoomWindow` now paint the title bar through an off-screen DC with a single final `BitBlt`. Removes `TextRenderer.cpp`/`.h`, an earlier DirectWrite-to-HDC bridge that never achieved reliable text-field rendering and was superseded by BetterText, left behind with dead init/shutdown/DPI wiring after the UI moved onto the shared `tk::` toolkit
- fix(sdk): the crypto store was never closed on logout/Drop (matrix-org/matrix-rust-sdk#3270) — no reachable `close()` existed outside the crate. Prototyped as a fork patch (`surakin/matrix-rust-sdk`) adding `Store::close()`/`Client::close_stores()`, confirmed fixed against the real repro (logout now deletes the account's matrix store outright, no leftover files) — then discovered `matrix-sdk-base::BaseClient::close_stores()` already closes state/event-cache/media/crypto together in the pinned base rev (and upstream 0.18.0), and `Client::pause()` (a stock, already-public API meant for iOS background suspension) already forwards to it. Reverted the fork pin and call `client.pause()` directly in `logout()`/Drop instead — no fork dependency remains
- feat(media): progressive ("fast-start") video streaming, now on all four platforms — MP4/MOV files begin playing while still downloading instead of waiting for a full buffer, with a draggable buffered-range scrub bar and disk caching of completed downloads (re-opening a video skips the network). Windows uses a growable `IMFByteStream`; GTK4 feeds a GStreamer `appsrc` (`GST_APP_STREAM_TYPE_RANDOM_ACCESS`) via a mutex/condvar-backed growable buffer, replacing `giostreamsrc` for the streaming path; Qt6 feeds `QMediaPlayer::setSourceDevice()` via the same growable-`QIODevice` design; macOS implements `AVAssetResourceLoaderDelegate` over a growable in-memory buffer, closing out a resource-loader approach previously deferred as "complex," and now observes `AVPlayerItem.status` so decode/format errors surface instead of hanging the overlay silently. All four block only the platform's own decode/demux thread (never the UI thread) and cancel that wait before pipeline/player teardown so closing the lightbox mid-stream can't stall. Also fixes video/image lightbox overlays not cancelling their in-flight fetch on close, which let audio keep playing in the background after close
- fix(shell): a multi-commit arc closing out account logout/relogin reliability. Root cause: matrix-sdk never closed state/event-cache/media stores on Drop, leaving `-wal`/`-shm` handles open past logout — a new login's rename-then-copy-fallback into the still-locked account folder merged with stale files into "database disk image is malformed". Explicitly closing each closeable store (bounded, so a stuck close can't hang shutdown) surfaced two more races: a worker still holding the shared session (e.g. a queued presence poll) could keep the old store open past logout, now fixed by calling `request_stop()` up front and draining every in-flight (not just queued) task through a new `AccountManager` draining registry before logout returns; and backgrounding the now-slower `logout()` off the UI thread exposed a stale queued UI event processed after the old `Client` was destroyed (a dangling `Client*` dereference in `StickerPicker::refresh_packs()`), plus `RoomView`/`EmojiPicker`/`StickerPicker`/`SettingsController`'s own cached raw `Client*` never getting nulled out when logging out the last/only account — both now cleared on that path. Also fixes login/session-restore error messages being wiped by a `reset()` call ordered after `set_status_message()`, and a restore-failure error string that only showed an anyhow chain's outermost `.context()` label instead of the full cause chain
- feat(threads): add in-thread find/search (a `RoomSearchBar` embedded in `ThreadView`'s header, a `thread_root_id`-scoped FTS5 index column/migration, and a client-side persistent thread-list filter), and dim the main timeline when the thread list panel is open, not just when a single thread is open. Also fixes `ThreadListView`'s header scrolling away with its own content — it inherits `tk::ListView`, whose `paint()` fully overrides `tk::Widget::paint()` and never invoked the `paint_before_children()`/`paint_children()` hook chain, silently dead-coding the header re-mask and close button — and fixes `ThreadListView`/`ThreadView` being constructed via `std::make_unique` instead of `tk::create_widget`, which left `host()` null and would have silently prevented the new search fields (or any native control) from ever being created. A follow-up fix stops the thread-dim scrim and the jump-to-event nav-loading scrim from stacking into a darker-than-intended fill when both are active at once — the nav-loading overlay now skips its own scrim when the thread dim is already up
- fix(windows): the internal display's GPU driver mis-renders `IDC_IBEAM`'s legacy AND/XOR invert mask once a window presents via a flip-model swap chain (a regression from the earlier move to flip-model presentation) — confirmed display-dependent (white-on-white on the internal panel, correct on an external monitor on the same hybrid-graphics machine) by temporarily reverting that change, which isn't viable to keep reverted since it fixes a real DWM staleness bug. Now extracts the real system cursor's mask bits via `GetDIBits` and flattens them into a solid black/white cursor colored from the app's live theme instead of relying on OS-level inversion, falling back to the untouched system cursor whenever the source is already full-color (an accessibility color-cursor scheme) or extraction fails
- fix(avatars): timeline and sidebar/account-picker avatars stayed blank after a DPI/monitor change until an unrelated scroll or re-populate forced recovery — `set_current_scale_()` flushes the shared avatar cache on a scale change, but `MessageListView` only re-fetches on a diff against the last-visible set (which a scale change doesn't perturb) and `UserInfo` had no fetch-trigger at all. Added `MessageListView::reset_visible_avatar_tracking()` and an `on_avatar_needed` callback on `UserInfo` mirroring `RoomListView`'s existing self-healing pattern
- fix(windows): `WM_MOVE` persisted window geometry unconditionally, unlike its `WM_SIZE` neighbor which already skipped `SIZE_MINIMIZED` — `GetWindowRect()` on a minimized window returns Win32's tiny iconic placeholder rect, so closing the app while minimized persisted that rect and the next launch opened at a near-title-bar-only size
- fix(receipts): declare `ReceiptGridPopup`'s destructor instead of relying on an implicit one — its `unique_ptr<GridAdapter>` member is a forward-declared private nested class, so the compiler synthesized a destructor at the first odr-use site where `GridAdapter` was still incomplete, breaking the build
- feat(receipts): read receipts now carry a timestamp (sorted newest-first from the SDK), shown as hh:mm in the hover tooltip; a message read by more people than the 5-avatar inline cluster shows gets a "+N" overflow pill that opens `ReceiptGridPopup` — a scrollable grid (built on `tk::GridView`, like the emoji/sticker pickers) listing every hidden reader as an avatar disc with the same name+time tooltip on hover. Also fixes three spots that positioned around the receipt cluster without accounting for the overflow pill's own width, letting it overlap the hover action-icon toolbar, the pending-send indicator, and wrapped message body text
- fix(export): widen `format_short_date`'s stack buffer from 16 to 32 bytes — GCC can't prove `tm_year+1900` stays small and flagged a theoretical overflow in release builds
- build(ci): `package.yml`'s tag-triggered release job now publishes each GitHub Release with `prerelease: true`, so a `v*` tag lands as a pre-release for testing instead of going live immediately — promote manually (uncheck "Set as a pre-release" on the release) once the artifacts are verified
- fix(tk): the main window's opaque D2D surface used single-buffer BLT-model presentation (`DXGI_SWAP_EFFECT_SEQUENTIAL`), which has a known DWM reliability gap — `Present()` can return `S_OK` indefinitely while the compositor silently stops picking up new frames (seen after extended idle or switching a laptop between its integrated display and an external monitor), with no error and no crash, just a window that never visually updates again. Moved every surface to flip-model (`FLIP_DISCARD`, `BufferCount=2`); since that reuses the per-physical-buffer backlog tracking already used by transparent overlay surfaces (only eventually consistent on its own — a scoped repaint credits the *other* buffer with catching up on its own next unrelated repaint, invisible for something as infrequent as a caret blink but not for a control that scoped-invalidates on every keystroke), `Host::on_paint()` now runs its paint body up to twice per `WM_PAINT`, so the second pass targets the other buffer and repaints exactly what its backlog owes it before the message loop dispatches anything else
- feat(tk): DPI/HiDPI display-scale awareness across all four platforms — a shared scale-change signal (`Widget::apply_scale_change`/`on_scale_changed`, `Surface::apply_scale_change`) that each backend fires on a live DPI/backing-scale change (Win32 `WM_DPICHANGED`, macOS `NSWindowDidChangeBackingPropertiesNotification`, Qt6 `DevicePixelRatioChange`/`screenChanged`, GTK4 `notify::scale-factor`), fixing `NativeTextField`/`NativeTextArea` image captures that went stale or stayed permanently under-rasterized (GTK4) after a scale change, plus a Win32 `CallWindow` that had no `WM_DPICHANGED` handling at all. Avatar/thumbnail media requests now scale with the current factor (`ShellBase::current_scale_`/`set_current_scale_()`) so server-generated thumbnails stay sharp on HiDPI instead of being fetched at a fixed 1× size and upscaled; a live scale change clears the in-memory thumbnail/image caches (entries aren't keyed by requested size). Linux (GTK4 + Qt6) builds clean and passes the full suite; Win32/macOS written to the same pattern but unverified — no toolchain here
- fix(media): media fetches from every room and homeserver shared two lane-wide `PriorityGate`s (fg/bulk), each with a single AIMD `dynamic_limit` — a batch of stalled requests to one dead or flaky homeserver could collapse that shared limit toward its floor and throttle all unrelated media from other rooms/servers, recovering only slowly via AIMD's additive increase (or not at all while the dead origin kept retrying). A new per-origin registry (`media_gate_registry.rs`) wraps the otherwise-unchanged `PriorityGate` so each origin (the homeserver behind an `mxc://`, or the target host of a direct URL fetch) gets its own lazily-created gate and can only ever throttle itself; a small lane-wide semaphore still caps aggregate concurrency across origins, and idle origins are reaped after a couple of minutes. `get_url_preview_async` is keyed by the client's own homeserver (the actual network destination for that call), not the previewed URL's host
- fix(export): fixed several compounding bugs in room-history export found via live testing — a stale checkpoint was silently resumed even when the dialog's "Start new export" was chosen; the oldest-event tracker captured the newest event in a window instead of the oldest (matrix-sdk-ui's `window_events_vec` is oldest-first), corrupting the displayed date and resume checkpoint; the in-progress message count double-counted matrix-sdk-ui's virtual items (date dividers, read markers) and was derived from live, not-yet-reconciled pagination state, measured inflating to >2x the real total — dropped in favor of an indeterminate bar; the anti-flash guard measured from dialog-open instead of from gathering-finalized, so it didn't cover the actual fast tail after a slow gathering phase; and progress ticks didn't reliably trigger a repaint/relayout, leaving the dialog looking frozen for seconds. Also renders HTML export images at the same scale-to-fit size the live timeline uses, linked to the full-resolution file
- fix(tests): `catch_discover_tests()` captured the test binary's `--list-tests` output with no explicit encoding, so CMake ≥ 3.15 fell back to the ANSI code page (no console attached to a piped subprocess) and mangled any non-ASCII test name into mojibake before it reached the generated CTest file — `ctest` reported "No test cases matched" for around 14 tests despite Catch2 printing the correct UTF-8 name. Patches the vendored `extras/CatchAddTests.cmake` via `FetchContent_Declare`'s `PATCH_COMMAND` so it survives a clean fetch; all 1481 tests now pass under `ctest --output-on-failure`
- fix(tk): `Surface::end_paint()` only treated `DXGI_ERROR_DEVICE_REMOVED`/`_RESET` as device loss, so a hung device (the common TDR case, e.g. switching a laptop between displays) was never recognized and the window silently stopped repainting forever — now uses the existing `is_device_lost_hr()` helper, which also covers `DEVICE_HUNG`. Also closes a crash loophole in `ensure_target()`'s recovery path: a failed target reacquisition after device recreation is now swallowed and logged instead of propagating uncaught out of `WM_PAINT`, with `begin_paint()` returning a null `Canvas*` so the caller skips painting and retries next repaint
- feat(export): add a full room-history export (plain text or HTML, optionally with images, optionally zipped) via a new "Export History" button in the room info panel. Walks the room on an isolated, detached focused timeline in memory-bounded windows (`sdk/src/client/history_export/`), persists a resumable checkpoint, and downloads/packages images; `HistoryExportController`/`ExportHistoryDialog` on the UI side, plus a new determinate/indeterminate `tk::ProgressBar` widget and a persistent click-to-return status-bar message. Windows-verified via local build+test; macOS/Qt6/GTK4 written by close analogy but not yet build-verified there
- fix(bettertext): the compose bar's caret-visibility check compared scroll bounds against raw DIP floats, but `ClientRectDip`'s physical-pixel round-trip made a perfectly-fitting control's visible height/width land a hair below DirectWrite's unrounded content metrics — misread as overflow, nudging `scroll_y` up and immediately snapping back down on the next call, ping-ponging every keystroke (most visible on backspace). Compares at physical-pixel granularity instead; also fixes the same truncating-division parity flip in `BetterTextArea`/`BetterTextField::set_rect`'s vertical centering, and adds pixel-granularity hysteresis to `BetterTextArea::refresh_height` so sub-pixel layout noise doesn't trigger a spurious resize
- docs(claude): require confirming the selected fix with the user before implementing when investigating a reported bug, and ask whether the user wants to set breakpoints before continuing if the first fix attempt fails
- fix(tray): relaunching the app while hidden to the tray correctly detected the running instance and exited the new process, but never showed the existing window again — Windows only restored a minimized (`IsIconic`) window, not one hidden via `SW_HIDE`, so it now also calls `ShowWindow(SW_SHOW)` when the found window isn't visible. macOS had no way at all for the duplicate-launch process to un-hide a tray-hidden window in the other process (unlike a Dock reopen or the tray's own click); added an `NSDistributedNotificationCenter` message so it can ask the running instance directly, mirroring the cross-process activation Windows gets from `WM_COPYDATA` and Qt6/GTK4 get from their `ActivationListener` socket. Windows-verified; user-verified live on macOS
- fix(tray): tray-icon clicks now show a hidden window, raise a visible-but-inactive one, and hide an active one across all four shells, instead of a naive visibility toggle. Windows needed two fixes verified via live debugging: a persistent `tray_owner_considered_active_` flag (updated from `WM_ACTIVATE` by checking whether the new foreground process is actually Explorer) distinguishes "deactivated by this click" from "user already switched to Explorer" or "another app is genuinely active", since Explorer's taskbar always briefly takes the foreground on its own tray-icon clicks; and `WM_LBUTTONDBLCLK` was firing the toggle in addition to `WM_LBUTTONUP`, tripling the toggle count on a double-click. macOS now checks `window.isKeyWindow` instead of app-wide `NSApp.isActive`; Qt6 and GTK4 (which lacked an active-check or hide path at all) now mirror the same three-state logic, with Qt6's duplicated toggle lambda factored into one shared helper
- fix(tests): 22 test cases across five files declared their `TestSurface`/`Stage` (owning a `tk::d2d::Backend`, whose destructor calls `CoUninitialize()`) *after* the widget tree it painted through — locals/members destruct in reverse declaration order, so the COM apartment tore down before the widget tree's D2D/WIC-rasterized icon bitmaps and text layouts, segfaulting inside `ComPtr::Release()`. Reordering each surface ahead of its widget tree (matching the already-correct tests) fixes all 22 without touching product code
- fix(tests): `ctest` launches Catch2 tests by name, but the CRT's narrow `main()` builds `argv` from the wide command line via the process's ANSI code page, silently collapsing any non-ASCII `TEST_CASE` name to `?` before Catch2 sees it — reported as a false "No test cases matched" ctest failure. Catch2 already ships a `wmain` entry point and a lossless `wchar_t` command-line path gated on `UNICODE`/`_UNICODE`, which FetchContent's Catch2 targets don't define by default; defining them and setting `/ENTRY:wmainCRTStartup` (to resolve the entry-point ambiguity against `audiopus_sys`'s embedded `opus_compare` `main()`) enables it
- feat(compose): unsent text, a staged attachment, and the caret position now round-trip per room (keyed by room id, not tab lifetime) instead of being wiped on every switch — a room-keyed store in `ShellBase`, `ComposeBar::take_pending()`/`restore_pending()` to move the staged attachment without copying it, and `NativeTextArea::set_cursor_byte_pos()` on all four backends to restore the caret. Also fixes a staged attachment surviving a room switch unclear (risking a send to the wrong room), and macOS never clearing compose text/attachments on switch at all
- fix(bettertext): `GetClientRect` returns physical pixels regardless of DPI awareness, but the control's D2D device context operates in DIPs via `SetDpi()` — feeding the raw physical-pixel rect into DirectWrite's wrap width, padding, and scroll-into-view math made the effective wrap width wider than the visible area on any non-100%-scaled display, so long compose-bar lines ran under the buttons before wrapping
- fix(tk): `CreateSwapChainForHwnd` can fail with `DXGI_ERROR_DEVICE_REMOVED` if the GPU/driver dies between device creation and a surface's first paint, before any frame has `Present()`'d to hit the existing mid-session recovery path — previously an uncaught exception out of `WM_PAINT` that crashed the app on startup. Now retries once with a freshly recreated device before giving up
- feat(crash): add an optional local crash handler (Settings → Advanced → Diagnostics, off by default, experimental) — vendors `backward-cpp` for native SIGSEGV/SIGABRT/SEH handling plus a Rust panic hook, both writing a plain-text stack trace to `data_dir()/crashes/`; nothing is transmitted anywhere. Gated behind a new CMake option so disabled builds compile to inert stubs; also adds Release debug-info generation (PDB/dSYM/`-g1`, Cargo line-tables-only) behind the same flag so a captured trace has something to symbolicate against. `backward-cpp` prints its Windows stack trace via `std::cerr` rather than the POSIX path's C `stderr`, so the redirect retargets both
- fix(tk): a prior fix (fec13410) made `Win32PopupSurfaceHandle::set_rect()` scale its anchor rect DIP→physical, correctly fixing `ComboBox`/`SearchablePicker`/`PopupMenu` — but `BetterTextArea::cursor_rect()` returned physical pixels instead of the DIP the `host.h` contract promises, so every caller's existing "already physical" workaround now double-scaled on top of the new conversion, breaking the mention, slash-command, shortcode, and GIF popups above 100% DPI. Fixed `cursor_rect()` to return genuine DIP like the Qt/macOS backends, and dropped the now-redundant manual scaling at its four call sites
- fix(macos): the room-list search field's clear (×) button is drawn/hit-tested in shared code but only works once the shell wires `on_search_clear` — Windows, GTK, and Qt did; macOS never did, so the button was clickable but inert. User-verified live on macOS
- fix(tk): `Win32PopupSurfaceHandle::set_rect()` passed the caller's DIP-space anchor rect straight into `ClientToScreen`/`SetWindowPos` with no DPI conversion, undersizing `ComboBox`/`SearchablePicker`/`PopupMenu` dropdowns (clipping trailing rows) above 100% scaling
- fix(tk): `request_repaint_rect()`/`invalidate_anim_damage()` built their `RECT` straight from DIP-space widget rects without the existing `dip_to_phys()` conversion every other screen-coordinate path uses, so on scaled displays the invalidated rect ended up too small/mispositioned and the resulting `WM_PAINT` missed the pixels that actually changed — visible as compose-bar text (and animated-image partial repaints) not appearing until an unrelated full-window repaint caught it up
- fix(sdk): `audiopus_sys`'s dynamic-linking override was scoped to macOS only, so Windows fell back to static linking — embedding a second copy of Opus alongside the one already in `webrtc-sys`'s bundled `libwebrtc.a`, which MSVC's `link.exe` rejects with `LNK2005` (hit by `cargo test`'s harness). Widened to `cfg(any(target_os = "macos", windows))` using `audiopus_sys`'s prebuilt dynamic `opus.dll`/`opus.lib`; verified via `cargo test -p tesseract-sdk-ffi` (474 passed) and confirmed a no-op for the shipped app (`dumpbin /dependents` shows no `opus.dll` import from either build)
- fix(sdk): `TransportsResponse` deserialized a `"transports"` field, but the real MSC4195 `/rtc/transports` response key (confirmed against MSC4519/MSC4515) is `"rtc_transports"` — the same name `signaling.rs` already uses for the MSC4143 member-event content. Because the field had `#[serde(default)]`, the mismatch failed silently: every server fell through to the `.well-known` `rtc_foci` fallback, hiding the call UI on servers that only expose `/rtc/transports`. Also accepts registry-prefixed unstable type strings (e.g. `msc4195.livekit`) per MSC4519's naming convention
- fix(qt): `QtNativeTextArea::natural_height()`'s placeholder branch wrapped its scratch measurement at `textWidth()`, which only reflects the field's real width once Qt has processed an actual resize — called before the first `set_rect()` ever runs, it measured against a freshly-constructed `QTextEdit`'s default width, producing a bogus height clamped to the compose bar's max until the first keystroke snapped it back down. Falls back to measuring the placeholder unwrapped when `textWidth()` isn't yet a real positive value
- fix(rooms): `RoomInfoPanel`'s settings/close buttons and `ComposeBar`'s attachment-remove badge let scrolled/underlying content show through their transparent-at-rest Icon-variant fill — given an opaque `Button::FillOverride` instead of a hand-drawn background. Also adds `tk::ComboBox::set_tooltip()`, used on the Create Room visibility combo to explain that Public/Private is a creation-time preset with finer-grained options available afterward in Room Settings
- fix(rooms): the Create Room invite field's placeholder was long enough to wrap to two lines, but every `NativeTextArea` backend measured only the (empty) real document for its natural height, so the box never grew — all four backends now measure the placeholder itself when the document is empty. Adds a reusable `tk::GroupBox` widget to visually cluster the invite list, reason field, and hint label. Also fixes `ComboBox`/`SearchablePicker` dropdowns showing square white corners on Windows around their rounded content — the popup's top-level window painted an opaque rectangle regardless of the rounded content inside it — via a new `PopupSurfaceHandle::set_corner_radius()` that clips the popup window to a matching rounded region (`SetWindowRgn`, padded 1px to avoid clipping the border stroke)
- feat(rooms): add room knocking support (MSC2403) — request to join a knock/knock-restricted room from the Join dialog (with an optional reason), track and cancel pending requests from a new "Requests to Join" room-list section, and let admins/moderators review and accept/deny/deny-and-ban requests from Room Info, uniformly across all four shells including pop-out room windows. Also fixes federated knocks failing silently: `Client::knock()` was passed an empty `via` server list, leaving the local homeserver with no route to a room it doesn't already know — now falls back to the room id/alias's own domain and surfaces the SDK's real error message
- feat(rooms): show a "Start calls" permission in the Room Settings Permissions tab — MatrixRTC call-member state events (`org.matrix.msc4143.rtc.member`, falling back to the legacy `org.matrix.msc3401.call.member`) were authorized like any other state event but had no exposed power-level control. Adds a 10th `RoomPermissions` field, read from either event-type override (falling back to `state_default`) and written to both keys together on save; the new "Calls" group is hidden when the server has no call backend, mirroring the call button's own gating
- fix(tk): follow-up correctness fixes surfaced testing 0a9b5265's migration of native text controls to render into the canvas — Win32's `caret_rect()` was dividing an already-DIP value by `dip_scale()` a second time, halving the caret size above 100% DPI; Qt6's `QTextEdit::scrollContentsBy()` and signal-blocked `set_text()` calls never told the capture pipeline anything had changed, leaving `cached_image_` stale (smeared scroll content, overlapping prefill/clear); the compose bar's text-area rect had zero vertical inset (unlike the button row), sizing the native control flush to the card's border once content grew past the floor height; and the Win32 compose box was entirely non-scrollable (real/touchpad wheel didn't recapture the canvas, no keyboard scroll-into-view, no visible scrollbar thumb), now fixed with a new `forward_wheel`/`BetterTextScrollBy`, `EnsureCaretVisibleVertically`, and a content-drawn overlay thumb — plus two structural bugs (an undocumented border inset under-sizing the control by a few px, and a missing repaint call after `HandleChar` that left the canvas one keystroke stale)
- feat(login): show a clear "No Internet Connection" dialog on cold start when the machine is offline, instead of the raw backend error text `show_restore_error()` surfaces for real connectivity-present failures — a new `tk::Host::is_network_available()` one-shot connectivity probe (`QNetworkInformation` / `GNetworkMonitor` / `INetworkListManager` / `SCNetworkReachability`) gates the cold-start session restore, skipping the restore network call entirely when offline. Qt6/GTK4 build clean and pass the full suite; Win32/macOS backend code unverified from this Linux session
- fix(rooms): cold-start room restore had gotten slow — `subscribe_room()` blocked synchronously on a seed-event-count check, `refresh_pinned_for_current_room_()`'s permission checks blocked the UI thread, and `push_rooms_()` could queue a backlog of background-warming work for every other room ahead of the restore's own subscribe call. Moved the seed-check into the already-spawned task, dispatched the permission checks off the UI thread, and skip background-warming dispatches while a restore is pending (bounded so an unreachable room can't gate warming forever). Also, room layout (active room + open tabs) was only ever persisted at window close with no tracking of the write — added a debounced auto-save so layout survives a crash within a couple seconds, plus a blocking save path used only when closing the last open window so shutdown waits for the write to land
- perf(tk): `rebind()` cleared the D2D emoji-bitmap and brush caches on every `Surface::begin_paint()` even though they belong to the device's resource domain (unaffected by `SetTarget()`) rather than per-paint state — every room-list row with an emoji paid a fresh synchronous PNG decode on each repaint instead of once per distinct glyph
- fix(sdk): six background-task bookkeeping methods (`start_background_backfill`, `start_bridge_status_check`, `start_unread_prefetch`, etc.) took the exclusive FFI lock purely to write an `AbortHandle` field, and could hold it during a genuine UI-thread stall (confirmed via a Superluminal blocking capture) while dispatched from background workers on the room-switch path. Wrapped the relevant task fields in `parking_lot::Mutex` (matching the existing `thread_timelines`/`thread_lists` pattern) so these methods take `&self` and run concurrently with other FFI readers
- fix(rooms): the room search field never declared its own background, falling back to the root (white) background instead of the rounded search card's fill drawn behind it, and was sized flush with that card so its opaque native capture squared off the rounded corners
- fix(rooms): the overflow menu's Delete action was gated purely on `is_own`, so moderators/admins with sufficient power to redact others' events under `m.room.power_levels.redact` never saw the option — added a `can_redact_in_room` query (mirroring `can_pin_in_room`) threaded into `MessageListView`'s delete gate alongside the existing check
- fix(rooms): favorite rooms subscribed to at startup showed stale/empty content until manually opened, since `compute_unread_prefetch_set()` only selected rooms by unread state — favorites are now always included in the prefetch set regardless of read/mute state or the unread cap. Separately, `subscribe_room()` only builds a live timeline from whatever is already cached locally; it now checks the built timeline's event count and kicks off a bounded background backfill immediately if thin, instead of waiting for the UI to notice an empty list and paginate reactively
- perf(tk): a Windows CPU profile caught startup eagerly constructing `RoomSettingsView`'s whole native-field subtree (3 `TextField`s + 1 `TextArea`, each a real `CreateWindowExW`) for a settings panel nobody has opened yet — native construction for `TextField`/`TextArea` now happens on first `set_visible(true)` or `arrange()` instead of in the constructor, benefiting any hidden native field anywhere in the app; setters used before creation now buffer their value and replay it once the control exists
- fix(replies): `ensure_reply_details_()` dedups reply-preview fetches to one attempt per event id, but matrix-sdk-ui never re-resolves an `m.in_reply_to` preview on its own — if the quoted message wasn't loaded locally on the first attempt, the quote block stayed stuck on "Original message unavailable" forever, even after the quoted message later scrolled into view via backward pagination. Added a retry that clears the dedup guard and re-fetches for any still-unresolved reply row whose target shows up in a newly-inserted, appended, or paginated-in batch
- fix(rooms): the Create Room dialog's topic and invite fields were fixed-height `TextArea`s with no auto-grow wiring, so content beyond a couple of lines was invisible/unreachable without scrolling — now grow with their natural content height, capped in `arrange()` so neither can push the fixed-size card's buttons out the bottom, mirroring `RoomGeneralSection`'s existing topic-field pattern
- fix(tests): the i18n tests hardcoded `/tmp` for writing MO catalogs, which isn't a valid path on Windows — all five tests silently failed to load their catalog there and `set_locale()` fell back to identity mode, failing on Windows. Uses a portable temp dir instead
...
- feat(shell): add an idle-TTL eviction tier (30 min) for warm room/thread timelines — the warm-subscription LRU permanently exempts the active room, every open tab, every pop-out-pinned room, and every favorite from eviction, so a long session with a few tabs or favorites open accumulates unbounded live timelines (each retaining every event ever loaded for the life of the subscription). The new orthogonal tier reclaims any room or thread timeline not genuinely on-screen right now regardless of tab/favorite/pinned status, reusing the same `unsubscribe_room`/`unsubscribe_thread` teardown the warm-LRU already relies on and riding the existing 30 s presence tick (no new timer)
...
- feat(tk): render native text controls into the canvas instead of overlaying them — `NativeTextField`/`NativeTextArea` grow an optional `rendered_image()`/`rendered_image_rect()`/`set_on_repaint_needed(Rect)` set (default no-op, backward compatible) letting a backend render itself into an offscreen buffer instead of compositing on screen directly; `tk::TextField`/`TextArea` draw that image via `Canvas::draw_image()` when present, so the control participates in clip/opacity/transform like any other canvas content. Backends whose real control loses OS input/cursor handling as a side effect of going invisible get `forward_pointer_down`/`drag`/`up` and `set_hovering()` to restore click/drag-select/hover-cursor behavior explicitly; where the native control can suppress its own caret paint, the canvas draws the caret itself
- fix(ci): scope the signing-cert secret to only the steps that use it, reading a presence flag from a dedicated check step's output instead of the `secrets` context directly (GitHub rejects reading it in an `if:` at static validation time)
- fix(windows): hide `main_app_surface_` until session restore completes — it was never hidden at creation, unlike `settings_surface_`, so the room list sidebar was visible immediately alongside `BrandView` on Windows instead of only after restore completes
- refactor(tk): make `SettingsPage`/`KnownPacksList` real `ScrollableBase` widgets instead of hand-rolled, subtly-different scroll copies (neither of which could inherit `ScrollableBase` since they already inherited `tk::VBox`); picks up shared thumb geometry/paint, kinetic momentum, clamping, and real thumb-drag support for the first time. Folds the one-method `tk::ScrollableRegion` interface into `ScrollableBase` itself, since it existed solely as a workaround for these two classes
- refactor(tk): make child painting automatic instead of manually-invoked — `Widget::paint()` now runs `paint_before_children()`/`paint_children()`/`paint_after_children()` itself instead of requiring every override to remember to call `paint_children()`; adds `z_order()` so a widget can be pinned to paint last without depending on `add_child()` order, and gives `tk::Button` self-contained icon rendering. Converts ~65 `paint()` overrides across the shared toolkit and views
- refactor(windows): remove the dead pre-BetterText EDIT/RichEdit control code (`Win32NativeTextArea`, `Win32RichEditArea`, RichEdit IIDs, the `IProvideFontInfo` COM shim, the emoji-codepoint classifier, the caret timer, `win32_textserv2_compat.h`, and the `imm32` link dependency) — predates the BetterText-backed native text controls and is no longer instantiated anywhere
- fix(ui): raise the per-image scaled-surface LRU cap from 4 to 8 on both the GTK/Cairo and Qt canvas backends — a shared avatar mxc URL is drawn at many distinct fixed sizes across views, so a 4-entry cache thrashed on every repaint tick for popular avatars
- feat(windows): add MSIX packaging (Store + direct) alongside NSIS — wires the packaged-app runtime (`Win32PackageContext` identity/AUMID detection, packaged `StartupTask` autostart, packaged Jump List, AUMID-aware toast notifications) up to a matching build/CI layer: an MSIX manifest + App Installer descriptor, generated visual assets, CMake stage/pack/sign/validate targets for both editions, and a CI job producing signed/unsigned `.msix`/`.appinstaller` artifacts
- feat(windows): integrate taskbar shell features — a process-wide `ITaskbarList3` coordinator shared by main/room/call windows applies unread/mention overlay icons across every registered top-level window and restores state after `TaskbarButtonCreated`; context-sensitive thumbnail-toolbar controls for next-unread navigation and active calls; attachment/voice-message upload progress reported through taskbar progress state; classic localized Win32 Jump Lists for quick switching, message search, settings, and recently visited rooms, backed by typed launch arguments, cold-start deferral, and single-instance `WM_COPYDATA` forwarding for multi-account room routing
- ci: add deterministic cross-platform screenshot generation — a CI-only mode (guarded by `TESSERACT_ENABLE_SCREENSHOT_MODE`) with a shared deterministic room/timeline fixture and embedded CC0 avatars, light/dark capture entry points for all four shells, and a workflow that builds and captures Linux screenshots under Xvfb plus a best-effort macOS AppKit job
- fix(arch): add `git` as an explicit `makedepend` (Corrosion fetches itself via `FetchContent(GIT_REPOSITORY ...)` at configure time, and a clean chroot build only has `base-devel`+`makedepends`) and disable LTO (`options=('!lto')`) for AUR builds, since Arch's default `-flto=auto` breaks `ring` (pulled in via rustls/webrtc-sys) at link time — a known upstream issue (briansmith/ring#1444)
- feat(linux): show room/DM avatars in KRunner and GNOME Shell search results, resolved via a synchronous disk-cache lookup (no network fetch) and attached as a per-result icon instead of the generic kind-based icon, with the themed icon name kept as a fallback
- build: trim third-party debug info to shrink `target/` size — caps non-workspace deps (rustls, tokio, webrtc-sys, ...) to line-tables-only, keeping file:line backtraces while dropping per-variable debug data; our own `sdk/src/` code keeps full debug info
- fix(linux): fix several GTK4 rendering bugs (sticker/emoji pickers not animating without mouse movement, cubic image resampling redone from scratch on every draw dominating CPU, room-list names/previews not truncating with an ellipsis, the back-pagination spinner not advancing without mouse movement, a stale "Topic (optional)" placeholder staying visible) and add `tk::single_instance`, a shared flock+Unix-socket primitive so launching the GTK4 build while the Qt6 build is already running (or vice versa) is detected and forwarded instead of opening a second instance against the same matrix-sdk store
- feat(macos): add Now Playing and Spotlight search integration, extending the Linux MPRIS/search work to macOS via the same shared `MediaPlaybackHub`/`SearchBackend` classes — `MacNowPlaying` (`MPNowPlayingInfoCenter`/`MPRemoteCommandCenter`) for voice/audio playback, and `MacSpotlightSearch` (indexes rooms/contacts into `CSSearchableIndex`, debounce-reindexed on roster changes)
- feat(linux): add MPRIS, GNOME Shell search provider, and KRunner integration — a new headless `SearchBackend` backs room+contact search across every open window; `GtkMprisPlayer`/`QtMprisPlayer` expose voice/audio-message playback as `org.mpris.MediaPlayer2(.Player)`, fed by a new `MediaPlaybackHub` and `TimelineMediaController` play/pause/resume/seek controls; a GNOME Shell search provider (GTK4) and KRunner plugin (Qt6) surface room/DM/contact results as thin D-Bus adapters over `SearchBackend`

## v0.8.17 — 2026-07-31

### Summary

- ci: merge duplicate DEB/RPM and AppImage build steps into one Linux job, gated on independent toggles in the manual workflow; also fixes `build-platform.yml`'s AppImage output missing the emoji-font bundling step
- feat(packaging): add a Flatpak/Flathub manifest (`packaging/flatpak/`, Qt6 variant), vendoring Rust deps offline and Rust's own 1.97.0 toolchain since Flathub's KDE runtime pairs with an older rustc than matrix-sdk's MSRV; validated with a real local `flatpak-builder` build, plus a manual-only CI validation job
- fix(macos): fire hover events for non-activating popup panels (`PopupMenu`, `ComboBox`/`SearchablePicker` dropdowns) by switching their tracking area to `NSTrackingActiveInActiveApp`, since `NSTrackingActiveInKeyWindow` never fired for panels that are deliberately never made key
- chore(packaging): track the AUR-ready `tesseract-matrix`/`tesseract-matrix-git` PKGBUILDs in-repo instead of gitignoring them, as the source of truth copied into the separate AUR git repos at publish time
- fix(packaging): correct the Arch PKGBUILD's license (was MIT, repo is GPLv3), drop an untested `aarch64` arch entry, remove a redundant `provides`, and fill in placeholder CPack contact/help URLs
