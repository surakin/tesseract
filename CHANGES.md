...
- feat(rooms): MSC2346 bridge detection now parses the bridge event's `network`/`protocol` content, so the room-info panel badge shows the actual bridged network's name and avatar (e.g. "WhatsApp") instead of a generic "Bridged" label; falls back to the label when the event carries no displayname. Added a "This room isn't actually bridged" override checkbox in Room Settings → General (local-only, persisted in the existing `im.gnomos.tesseract` account data, applies immediately) for rooms where auto-detection is wrong. All three room-info badges (Encrypted, history visibility, Bridged) now draw a real Lucide icon (`lock-keyhole`, `eye`, `cable`) instead of an emoji glyph, so they line up on one shared baseline instead of depending on how a platform's colour-emoji renderer happens to size/place its glyphs. Windows build; user-verified live
