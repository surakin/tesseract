...
- fix(mentions): a timeline mention pill sharing a line with real text could inflate that line's height on Windows — `build_rich_text`'s per-range path sized the pill's `BlankInlineObject` baseline/height split via `tk::role_line_metrics()`, which depends on `TextLayout::ascent()` always reporting the true font ascent; the D2D backend's `ascent()` deliberately returns the *full* line height instead (so Segoe UI Emoji glyphs center correctly elsewhere), so the split silently collapsed to "whole box is ascent, zero descent" — exactly the shape the code's own comment said it was avoiding. Switched to the existing `real_line_metrics()` helper (already used for the compose box's native pills, via `IDWriteTextLayout::GetLineMetrics()`), which was never wired into the timeline path. Windows build + full ctest, 1814/1814; Qt6/GTK4/macOS unaffected (D2D-only bug)
- refactor(media): every decoded-image cache (`PixmapCache`, `AnimImageCache`, `CompressedImageCache`, `MediaDiskCache`, plus `ShellBase::viewer_fullres_` and each shell's GIF-picker/mention-pill caches) is now keyed by a typed `tk::CacheKey` (namespace tag + id + optional size) instead of a bare `std::string` with hand-rolled prefixes (`"t{w}x{h}:"`, `"fullres:"`, `"thumb::"`/`"video_thumb::"`, `"blurhash::"`, `"tile:"`, `"gifsrc:"`) — those prefix collisions were a real foot-gun (`ShellBase.cpp` and `.h` both special-cased `starts_with("thumb::")` by hand to dodge one). `CacheKey::to_string()` reproduces each old prefixed string exactly, so on-disk cache files from a previous session stay valid. New `ui/shared/tk/cache_key.h`/`.cpp`; all four platform shells, `RoomPane`, the view-layer prefetch collectors, the desktop-search D-Bus adapters, and every affected test updated to match. Windows build + full ctest, 1814/1814; Qt6/GTK4/macOS share the code, unbuilt this session
- perf(media): animated GIFs/WebP/stickers loaded slower than they needed to — the shell's actual media-decode path bypassed the sized decode entirely on Windows/GTK4/macOS (full native resolution, no downscaling, ever) and every platform decoded the whole animation before displaying anything. Each backend now downscales per-frame as it decodes (independent width/height limits, not a collapsed square bound) and streams frame 0 to the screen immediately while the rest decode in the background; the lightbox's full-res view for animated media got its own decode path so it no longer shares (and gets capped by) the small inline-timeline copy. The shared decode/IO thread pool is now sized from `hardware_concurrency()` (clamped 2-8) instead of a fixed 2 threads, and the size-mapping/streaming-callback logic that used to be copy-pasted per shell is now one shared `ShellBase` implementation. Windows build + full ctest, 1814/1814 (+5) (1 pre-existing unrelated failure); user-verified live on Qt6, GTK4, and macOS — including the macOS per-frame scaling path's known ImageIO channel-swap failure mode (see the `scale_cgimage` comment in `canvas_cg.cpp`), not reproduced on a real account with many animated stickers.
- feat(pickers): the emoji/sticker picker's category tabs are now keyboard-focusable, via a new `TabbedGridPicker::TabStrip` child widget mirroring `tk::TabView`/`TabBar`/`SideTabView`'s single-focusable-container shape — Left/Right move a keyboard cursor and Enter/Space commits it (`GridView`'s two-step model, not TabView's immediate-commit one). Tabs also gained full `Role::TabList`/`Role::Tab` accessibility, including a new `tab_label()` naming each one for screen readers. macOS build + full ctest, 1805/1805 (+3) (1 pre-existing unrelated failure); user-verified live. Qt6/GTK4/Windows share the code, unbuilt this session
- fix(pickers): pressing Tab inside the emoji/sticker picker (or the read-receipt popup) escaped to the app's wider focus order and closed the popup — those widgets are `register_popup()`'d but deliberately kept out of `RoomView`'s own tree, so `next_focusable()` couldn't find them and fell back to the first focusable widget anywhere, whose `request_focus()` then dismissed the "outside" popup. `RoomView::paint()` now scopes Tab traversal to whichever picker/popup is open, mirroring its existing handling of the five overlay panels. macOS build + full ctest, 1805/1805 (+3) (1 pre-existing unrelated failure); user-verified live. Qt6/GTK4/Windows share the code, unbuilt this session
- fix(profile): the user-profile card's Pronouns/Timezone/Bio/Status/Activity rows drew their label and value at the same top-`y` despite using different font roles (`Small` vs `Body`), so the differing ascents made them look vertically offset; values also never wrapped (`wrap` left at its default `false`, `trim = Ellipsis`), silently cutting off anything past one line regardless of the space below. `UserProfilePanel::layout_ext_rows_()` now baseline-aligns label/value via `TextLayout::ascent()` and wraps values (capped at 4 lines, mirroring `RoomInfoPanel`'s topic-truncation pattern), sizing the row/card from the real measured height instead of a fixed 22px estimate. Linux (Qt6 + GTK4) build + full ctest, 1802/1802; user-verified live. Windows/macOS share the code, unbuilt
- fix(media): the image viewer opened a profile picture at the wrong size and then jumped to being enormous once the full-res avatar decoded — real dimensions aren't known up front for avatars (no Matrix metadata), so the viewer sizes off whatever's currently decoded (thumbnail first, then full-res), but `zoom_` was only ever clamped against the fit ratio, never recomputed; a zoom fit for the small cached thumbnail then got multiplied against the much larger full-res image once it replaced the thumbnail in cache. Now tracks the dimensions a fit was computed against and redoes the fit whenever the resolved image's actual size changes, and opens sized to cover ~75% of the viewport (upscaling allowed) instead of the thumbnail's native pixel size. Mouse wheel zoom is a no-op only during the true loading-placeholder phase (nothing decoded yet). That upscaling also crashed: a small thumbnail (e.g. 96x96, or smaller) covering 75% of a real screen can need more than the fixed 8x manual-zoom ceiling, so `fit_zoom_` could exceed `kZoomMax` and violate `std::clamp`'s `lo <= hi` precondition; both clamp sites now widen the ceiling to match whenever that happens. Linux build + full ctest, 1802/1802 (+5); user-verified live on Qt6. Windows/macOS share the code, unbuilt
- feat(mentions): timeline and composer mention pills now share one renderer (new `tk/pill.h`, `render_pill_bitmap`) instead of two separate drawing paths, so they match in shape, color (theme accent), and leading avatar. macOS build + full ctest, 1795/1795 (1 pre-existing unrelated failure); Qt6/GTK4/Windows share the code, unbuilt this session
- fix(mentions): composer pills dropped the redundant `@` from their label (the pill shape already says "mention") and no longer vary in height per mention — macOS read the font at the text-insertion point (`NSTextView.font`), which drifts with local formatting, instead of the compose box's own stable body font. macOS build + full ctest, 1795/1795 (1 pre-existing unrelated failure); Qt6/GTK4/Windows share the `@`-drop, unbuilt this session
- fix(mentions): timeline pills on a wrapped (multi-line) message row rendered visibly stretched — `paint_span_images` sized the bitmap from `TextLayout::ascent()`, which reports the *whole layout's* height across every line rather than one line's ascent, so a 3-line message inflated the pill's height ~2x versus what layout time had actually reserved for it. Now uses the same stable, line-count-independent font metrics layout time already uses. macOS build + full ctest, 1796/1796 (+1) (1 pre-existing unrelated failure); Qt6/GTK4/Windows unaffected (they never read layout.ascent() here)
- fix(mentions): a timeline pill was only clickable across roughly its left half — macOS's `link_at()` used `CTLineGetStringIndexForPosition`, a caret-placement API that snaps to whichever side of a character's width the click is nearer, and a pill is one very wide character (a `CTRunDelegate`-backed placeholder); the right half snapped past the pill's own index. Now hit-tests via geometric x-offset containment, matching how `selection_rects()` already computes pill bounds. macOS build + full ctest, 1797/1797 (+1) (1 pre-existing unrelated failure); Qt6/GTK4/Windows use different hit-testing mechanisms — not verified to have (or not have) the same issue
- fix(mentions): timeline pills rendered pixelated next to the composer's crisp ones, and measurably taller than the surrounding text — the live canvas's `scale_factor()` read an unscaled CTM (inherited layer-backing from an ancestor's `wantsLayer = YES`) that the pill bake trusted for its own resolution, and layout time fed CoreText's `CTRunDelegate` a degenerate all-ascent/zero-descent split instead of the font's real one. Both now use explicit real values — `window.backingScaleFactor` for scale, `real_line_metrics()` for the ascent/descent split. macOS build + full ctest, 1796/1797 (1 pre-existing unrelated failure); user-verified live
- fix(mentions): reply-quote previews never rendered mention pills — the quote card built its body via `html_to_spans()` directly instead of going through `prepare_spans()`, so it classified the mention span correctly but skipped `substitute_image_placeholders()`/`paint_span_images()`, the two steps that actually reserve and paint the pill bitmap. Also fixed `QtRichTextLayout::line_count()`, which reported paragraph (hard-break) count instead of visually wrapped line count, silently defeating a wrap-dependent pill regression test. Linux build + full ctest, 1797/1797; user-verified live (Qt6)
- fix(mentions): on Qt6 HiDPI displays, a pill's background and text rendered at the wrong scale (only its top-left logical-sized region painted) while its avatar rendered correctly — `QtOffscreenSurface` set the backing `QImage`'s devicePixelRatio *after* attaching a `QPainter` to it, too late for `QPainter::begin()`'s one-time device-metrics snapshot; the avatar path independently re-read the DPR live so it self-corrected. Now set on the `QImage` before it's moved in. Linux build + full ctest, 1797/1797; user-verified live (Qt6 HiDPI)
- fix(gtk4): on HiDPI displays, every native text field/area (composer, search boxes — pills included, since they're embedded inside the same widget) rendered its content at a quarter its correct size, anchored top-left — `GtkNativeTextField`/`GtkNativeTextArea::refresh_image()` tags its device-pixel-sized capture surface with `cairo_surface_set_device_scale()` (needed for `gsk_render_node_draw` to render into it correctly), but never clears it afterward; `canvas_cairo.cpp`'s generic image-blit path (shared with plain decoded images, which never carry this metadata) honors that scale again when compositing via `cairo_set_source_surface`, compounding with its own explicit resize to shrink the content by scale². Reset to 1:1 once the capture itself is done. Linux build + full ctest, 1797/1797; user-verified live (GTK4 HiDPI)
- feat(rooms): Room Settings' Permissions tab warns (non-blocking, unlike the existing self-lockout guard) when a staged change would leave no other room member able to edit permissions — new `Client::room_best_other_power_level`, mirroring `room_own_power_level` but scanning other joined members. Windows build + full ctest, 1791/1791 (+3) C++ / 667 Rust; unbuilt/unverified live this session. Qt6/GTK4/macOS share the code, unbuilt
- feat(rooms): Room Settings gains a Leave button in its footer (destructive-styled, confirmed via the same dialog as the room-info panel's Leave). Windows build + full ctest, 1788/1788 (+4); user-verified live. Qt6/GTK4/macOS share the code, unbuilt
