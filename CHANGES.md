...
- fix(windows): the main window (and pop-out room windows) could show the native Win32 caption alongside the custom-drawn titlebar. Caption hiding relies entirely on `WM_NCCALCSIZE` firing before first paint, but the no-saved-geometry fallback size could exactly match the size already passed to `CreateWindowExW`; `SetWindowPos` with an unchanged size/position is a no-op that Windows never re-fires `WM_NCCALCSIZE` for. Most visible over RDP (which commonly defaults its virtual display to 96 DPI), but reproducible locally too by clearing the saved geometry. Both windows now force one `SWP_FRAMECHANGED` call before showing. Windows build + user-verified live
- fix(gtk4): all pointer input (clicks, scroll, hover) was dead across the entire GTK4 UI except the native text fields — the `TkAccessNode` AT-SPI marker is a full-bleed `GtkOverlay` child, and with `can-target` at its GtkWidget default of TRUE `gtk_widget_pick()` returned it for every point, so the sibling `GtkDrawingArea` beneath it never saw an event. Marker nodes are now created `can-target=FALSE`. Broken since the a11y bridge landed. GTK4 build + user-verified live; no shared/Qt6 changes, test count unchanged (1766)
- refactor(tk): `CanvasFactory` now has one text-layout virtual, `build_rich_text`; `build_text` is a non-virtual shim that wraps the string in a single `TextSpan`. Each backend short-circuits a lone unformatted span to its cheap single-line path, so plain labels are unchanged while emoji sizing is now uniform across all four platforms instead of Linux-only. Emoji segmentation moved from `views/html_spans` to `tk/emoji_segmentation`, dropping a `tk/`→`views/` include. All four platforms build; Linux full ctest, 1766/1766 (+1) C++ + 652 Rust; user-verified on Qt6 + GTK4
