...
- fix(emoji): emoji-only messages rendered at normal size instead of 2x ("BigEmoji") on Windows — `canvas_d2d.cpp`'s per-run emoji sizing always derived from `FontRole::InlineEmoji`, ignoring the `BigEmoji` role the body was styled with, so it stomped the layout's correctly-sized text format back down for every glyph. Fixed to match the Qt6/GTK4 role-aware pattern; applied the same missing guard to macOS's `canvas_cg.cpp`. Windows build + full ctest, 1770/1770 (4 pre-existing unrelated failures reproduced against baseline) + user-verified live; macOS unbuilt
