...
- fix(win32): the accessibility bridge no longer crashes on shutdown — `UiaDisconnectProvider` can pump a reentrant `WM_GETOBJECT` into the still-live window, which could reach back into the bridge mid-teardown and invalidate its provider map. Registry entry is dropped before `detach()` runs, and `detach()` iterates a moved-out copy of the map as a second guard. Windows build-verified; runtime-unverified against the original crash
