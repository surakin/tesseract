...
- fix(macos): the room-list search field's clear (×) button is drawn/hit-tested in shared code but only works once the shell wires `on_search_clear` — Windows, GTK, and Qt did; macOS never did, so the button was clickable but inert. Unverified (no macOS toolchain here)
