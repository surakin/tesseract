...
- feat(settings): "Restart now" relaunches Tesseract after a language change; the new process waits for the old one's single-instance lock (`--relaunch`). Qt6 build + ctest; unverified live. GTK4/Windows/macOS unbuilt
- feat(i18n): French translation (`i18n/fr.po`, Element Web terminology); pronoun language names translated. `msgfmt -c` clean
- fix(calls): a missing, busy or blocked camera is now detected and explained (in calls, the lobby and `/selfie`) instead of sending green video; turning video back on retries the camera. Qt6 build + ctest 1993/1993; user-verified live. Windows/macOS unbuilt
- fix(accounts): the account picker now lists every signed-in account (it kept the row count from its first open, so a later login was missing) and shows up to 8 rows, scrolling beyond that. Qt6 build + ctest 1987/1987; user-verified live. GTK4/Windows/macOS share the code, unbuilt
- feat(security): one dialog now handles all of encryption. Setup creates a recovery key in one click (passphrase is an optional link), the key can be saved to a file and can't be lost to a stray click, and new devices choose between another device, the recovery key, or resetting encryption. Emoji verification and incoming requests ("Was this you?") run inside the dialog; the old verification banner is now a reminder strip that reopens it and can be snoozed for 3 days. Verification handling moved from the four shells into `ShellBase` + `EncryptionFlowController`; "Use another device" is offered via matrix-sdk's `has_devices_to_verify_against` (new FFI), and the reminder follows recovery-state changes live via a new `on_recovery_state_changed` callback. While a fresh login's sync is held behind the dialog, an encryption-only sliding sync (new `start_encryption_sync`) now uploads this device's keys and carries verification traffic, so "Use another device" and requests from other devices work from the dialog without starting the room-list sync. A request arriving on a background account now switches to that account and raises the window (or is declined on it while the dialog is mid-setup). The reset-approval step names the account to approve as (the browser approves for whichever account it is signed in as), offers "Copy link" for a private window, and after 60 s hints that a different account may have been approved. Qt6 build + ctest 1985/1985, cargo 719/719; user-verified live on Qt6 (the code-review fixes that followed — stuck Confirming step, reset-cancel on a gated login, account switch mid-verification, declined outgoing requests, owner-only key file — , the background-account routing and the reset-approval hints are unverified live). GTK4/Windows/macOS shell code removed, unbuilt
...
- fix(tk): flag-tag emoji (e.g. the Scottish flag) now render on Windows in the timeline, title bar and composer; each tag sequence is shaped as one glyph run and drawn as an inline object, since `IDWriteTextLayout` splits the tag characters into their own run. Windows build; user-verified live
- fix(tk): mention pills in the reply quote-card preview render again — macOS's elided single-line `CTLayout::selection_rects()` returned no rects once the layout swapped its CTFrame for a cached CTLine, so a pill's reserved box was never painted into; the quote card's plain-text fallback (no `formatted_body`) also now pill-ifies a literal @room, matching the main body. macOS build + ctest 79/79 (pill/mention/quote subset); user-verified live. Qt/GTK4/Windows unbuilt (Qt's separate rich-text path was checked and needs no equivalent fix)
...
- fix(tk): flag-tag emoji sequences (e.g. the Scottish flag) rendered as a bare black flag in-app despite showing correctly in the title bar — emoji-run segmentation didn't treat Unicode tag characters (U+E0000–U+E007F) as part of the preceding cluster. macOS build; user-verified live; Qt6/GTK4 share the segmentation fix, unbuilt

## v0.8.25 — 2026-09-24

### Summary

- build(ci): releases are now two-stage. A tag push builds everything (the Windows job now also packs the Store MSIX) into a pre-release, and marking it released runs the new `publish-release.yml`, which publishes the docs site, the Flatpak repo (from the tested bundle) and the Store submission; winget is wired in but disabled until its first submission is live. Replaces `msix-store-publish.yml` and `winget-publish.yml`. YAML-parsed only; unverified in CI
...
- feat(security): the sidebar avatar warns when the session is unverified, with a new "Verify this session…" menu item to reopen setup. Windows build; user-verified live. Qt6/GTK4/macOS share the code, unbuilt
...
- fix(packaging): MSIX installs (Store and direct edition) no longer check GitHub for new releases, since the Store or App Installer handles their updates. The check and its Settings toggle turn off at runtime when the app runs packaged, so one Windows build serves NSIS and MSIX. Windows build; unverified live
...
- fix(spaces): a just-created space could open as a normal chat room with a stuck "Empty room" name, or block leaving it — the room didn't exist in the local room-list cache yet when the app navigated to it right after creation, so the is-a-space check silently missed and nothing ever retried it once sync caught up. `ShellBase::push_rooms_` now re-runs that check and re-pushes the space's own info on every sync update. Windows build; user-verified live
- fix(spaces): the "Leave Space" button inside Space Settings did nothing — `SpaceRootView`'s embedded `RoomSettingsView` instance never had its `on_leave_room` callback wired up. Windows build; user-verified live
- fix(spaces): the room list's "back" button (exiting drilled-into-space browsing) could swap the main pane to the space's own room instead of keeping the space root shown, when that space was still the active room. Extracted the shared, corrected logic into `ShellBase::space_back_command_()`, replacing four near-identical, independently-buggy per-shell copies. Windows build; user-verified live. Qt6/GTK4/macOS share the code, unbuilt
- polish(rooms): Create Room's "Creating…" state now hides the whole form and shows a centered spinner, instead of an inline label that pushed the buttons down. Windows build; user-verified live
- polish(rooms): Create Room's status/error row now always reserves its height, so the Create/Cancel buttons no longer shift when an error appears. Windows build; user-verified live
- fix(spaces): a space's topic with no links in it didn't show at all in the space root view — `autolink_plain_to_spans` returns an empty span list (not a plain-text one) when there's no URL to link, and `TopicLinkLabel` had no fallback for that case. Windows build; user-verified live.
- fix(spaces): the space name/avatar above the room list now highlights on hover like other clickable rows, via a `SpaceNavWidget::dispatch_pointer_move` override. Windows build; user-verified live.
- fix(rooms): editing a room's topic now updates the room list live instead of needing a restart — `topic`/`topic_html` were missing from `room_list_fingerprint`. Cargo 711 (+2); Windows build; user-verified live.
- feat(spaces): space room management — drag rooms between a candidates list and the space's children grid in `SpaceRootView` to add/remove them (Enter/Delete keyboard equivalents too), gated on `m.space.child` send permission. New Rust/FFI/Client mutation API (`can_edit_space_children`, `add_room_to_space_async`, `remove_room_from_space_async`) reusing the existing `resolve_route_via`/`send_state_event_for_key`/`on_room_action_complete` machinery — no new callback. Top of the view redone as avatar+alias on the left, scrollable linkified topic on the right. First consumer of the in-app drag-and-drop framework. Linux (Qt6 + GTK4) build + full ctest 1906/1906, cargo 711 (+7); user-verified live. No automated coverage yet for the two new widgets themselves (`SpaceAddRoomList`/`SpaceChildRoomGrid`). Windows/macOS share the code, unbuilt
- feat(tk): add `tk::ScrollView`, a generic "wrap any widget to make it scrollable" container (`ScrollableBase` + a single owned child, natural-height probe via `measure()`); built for the space topic label, usable anywhere a one-off scrollable wrapper was previously hand-rolled. Linux (Qt6 + GTK4) build + full ctest 1906/1906
- fix(rooms): the room list didn't update when a room was added to or removed from a space — `m.space.child` is a state change on the *space's* room, not the child's, so nothing in `room_list_fingerprint` could ever notice it. The fingerprint now includes a synthesized `space_children_summary` of each room's valid (non-empty `via`) space-child state. Cargo 711 (+2)
- feat(tk): add an in-app (widget-to-widget) drag-and-drop framework — `Host::begin_drag`/`DragPayload`/`DragVisual`, claim-bubble drop-target discovery (`on_drag_enter`/`on_drag_over`/`on_drag_leave_target`/`on_drop`), a shared click-vs-drag threshold helper (`DragGestureTracker`), and a Host-owned floating drag visual painted above everything. Synthetic/in-process only — no OS drag-source APIs touched. Renamed the existing OS-file-drop hover API (`on_drag_hover`→`on_native_drag_hover`, etc.) so the two systems can't be confused. Framework only, no product call site yet; first consumer will be dragging rooms onto Spaces. Linux (Qt6 + GTK4) build + full ctest 1906/1906 (+8); Windows/macOS share the code, unbuilt
- feat(rooms): call rooms (MSC3417) get a dedicated "Call Rooms" room-list section (after Rooms, below Favorites/Unread/Inactive priority) plus full lifecycle wiring — auto-join on first switch into one, auto-float (not hang up) on navigating away, auto-restore on return, and leave-then-join when switching directly between two active calls; Docked/DockedExpanded mode controls now hide while away from the call's room. Linux (Qt6 + GTK4) build + full ctest, 1898/1898 (+15); unverified live this session. Windows/macOS share the code, unbuilt
...
- fix(calls): cross-homeserver calls showed no media because we only joined our own homeserver's SFU. We now speak Element Call's `multi_sfu` mode (publish on our SFU, subscribe to every member's SFU), with per-member key delivery, MSC4140 delayed leave, and hardened key-event checks. Cargo 704 (+28); user-verified live against Element Web on matrix.org. Sticky-event (`matrix_2_0`) memberships not supported yet
- fix(rooms): the room-info panel's Media row (and avatar lightbox, topic links, member rows, Requests-to-join row) stopped responding to clicks after the scrollable-body split — hit-tests compared a body-relative x against world-x rects. Direct-painted rects are now stored body-relative and converted to world at paint time; regression test added. Linux Qt6 build + full ctest, 1876/1876 (+1); user-verified live. Windows/macOS/GTK4 share the code, unbuilt
- fix(media): opening a few autoplay videos could push memory past 1 GB — each inline player held its whole clip plus decoder state even off-screen, paused players sat in the retired pool forever, and a stopped Qt/GTK/macOS player kept its copy of the clip. Videos now join the image mark-and-sweep GC (`TimelineVideoPlaylist::touch`/`retire_unseen`, marked by the GC's forced repaint on the main and pop-out windows): players unseen for 2 generations are retired and revive when scrolled back, retired ones are destroyed after 30 s idle, and Qt `stop()`, GTK `stop()` and macOS `play()` release the clip bytes. `VideoPlayer::memory_bytes()` added and video memory now counted in Settings' in-memory cache total. Windows media-engine copy of the clip is still held until the next `play()`. Linux Qt6 build + full ctest, 1875/1875 (+5); unverified live. GTK4/Windows/macOS unbuilt this session
- fix(media): Settings → About "In-memory cache" under-counted — `PixmapCache` froze each image's size at store time, missing the pre-scaled/GPU copies backends add later (and budgeting eviction against that stale figure), and the total ignored animation decode-session state (retained encoded bytes, compositing canvases), the full-res viewer cache, warmed voice clips, GIF-picker previews and the per-host/per-view pill caches. `PixmapCache` now sums live sizes and exposes `total_bytes_all_instances()`; `AnimDecodeSession::memory_bytes()` added on all four backends; hit/miss stats now include the compressed tier. Linux Qt6 build + full ctest, 1870/1870 (+4); unverified live. GTK4/Windows/macOS unbuilt this session
- feat(diagnostics): new Activity Monitor window (Settings → Advanced → Open Activity Monitor) listing every background job — Rust tasks/watchers/backfill/media gates, C++ worker pools and housekeeping timers — with state, last run, run count and errors, refreshed live while open. Job registries in Rust (`activity.rs`) and C++ (`ActivityRegistry`), new generic `AuxWindowBase` secondary window per shell. Linux Qt6 build + full ctest, 1866/1866, +4 Rust tests; unverified live. GTK4/Windows/macOS windows written by analogy, unbuilt.
- feat(media): uploaded images now have EXIF/GPS, XMP, IPTC and text metadata stripped losslessly (JPEG/PNG/WebP), in Original quality too and on animated WebP, file sends and avatar uploads — previously only Compressed mode dropped it, as a re-encode side effect. New `strip_meta.rs` on `img-parts` + `little_exif`; JPEG/WebP keep only the Orientation tag; PNG and GIF orientation/metadata not preserved (GIF unstripped). Linux Qt6 build + ctest 1856/1856, cargo strip_meta tests (+5); unverified live
- fix(compose): photos sent in Compressed mode arrived sideways because the JPEG re-encode dropped EXIF orientation without applying it. Each `encode_for_send` now bakes it in (Qt `setAutoTransform`, GTK `apply_embedded_orientation`, WIC flip-rotator, ImageIO transform); reported width/height now match the displayed orientation. Qt6 build + ctest; unverified live. GTK4/Win32/macOS unbuilt this session
- fix(emoji): shortcode popup missed common names like `:coffee:` and `:whiskey:` — `emoji_aliases.inc` was a ~60-entry hand-picked subset of gemoji. Now generated by `emoji_aliases.gen.py` from the full gemoji shortcode set (~900 aliases beyond CLDR names, sourced from the Rust `emojis` crate) plus `whiskey`/`whisky`/`bourbon` for 🥃. Linux (Qt6 + GTK4) build + emoji ctests (+1); unverified live. Windows/macOS share the code, unbuilt
- feat(rooms): the user profile panel's avatar is now shown at 160px (up from 72px) and fetches the actual source image instead of the shared small thumbnail, so it no longer looks blurry. Reuses the image viewer's existing full-resolution fetch/cache (`ShellBase::ensure_viewer_fullres_`/`viewer_image_lookup_`) rather than a new avatar-specific pipeline. Windows build; user-verified live. Qt6/GTK4/macOS share the code, unbuilt this session
- refactor(rooms): Room Settings → General now uses `tk::HBox`/`VBox` layout instead of one widget's hand-rolled position math, matching every other settings tab. The bridge-detection override moved out to its own new "Bridge" tab with an explanatory paragraph, shown only for rooms actually flagged as bridged. Windows build; user-verified live
- feat(rooms): the room-info panel's Bridged badge now shows the actual network name/avatar (e.g. "WhatsApp") when MSC2346 provides one, and all three badges use real Lucide icons instead of emoji. Room Settings gained a local override checkbox for rooms where the bridge auto-detection is wrong. Windows build; user-verified live
...
- fix(ci): macOS release builds (package.yml, build-platform.yml, compile-check.yml) now install the `llvm-tools-preview` rustup component, so `rust-objcopy` can load its `libLLVM.dylib` dependency instead of dyld-aborting. Previously non-fatal but silent, leaving debug info embedded in shipped binaries.
- feat(theme): the "System" accent option now resolves to a real, live-updating OS accent color on Qt6, GTK4, and macOS too, not just Windows. Derivation shared via a new `tk::apply_system_accent()`; each shell only implements the platform-specific read (`ShellBase::os_accent_color_()`). Linux (Qt6 + GTK4) build + full ctest, 1855/1855 (+5); user-verified live on Qt6 (KDE Plasma). Windows/macOS unbuilt this session
- feat(rooms): the room info panel's member list now has a real, draggable scrollbar instead of mouse-wheel-only scrolling. Linux (Qt6 + GTK4) build + full ctest, 1850/1850; unverified live this session. Windows/macOS share the code, unbuilt
- fix(compose): `/selfie` froze the app on Windows instead of showing the camera overlay, since the shared modal-hide gate didn't account for the camera/screen-picker overlays, triggering a relayout loop. Windows build + full ctest, 1824/1824; user-verified live. Qt6/GTK4/macOS share the fix, unbuilt this session
- feat(rooms): the room info panel's member list is now sorted by power level (admins/mods first, alphabetical within each tier) instead of matrix-sdk's arbitrary local-store order. `power_level` threaded through the FFI via `RoomMember::power_level()` (ruma's `RoomPowerLevels::for_user`, so room v12+ privileged creators sort correctly above ordinary admins). Linux (Qt6 + GTK4) build + full ctest, 1850/1850; cargo test 667/667; unverified live this session. Windows/macOS share the code, unbuilt
- fix(rooms): the room info panel's member-list scroll speed was ~20x faster than every other scrollable view — `RoomInfoPanel::on_wheel` applied a stray `* 20.0f` multiplier on top of a wheel delta the platform host already scales to pixels. Now matches `tk::ScrollableBase`'s plain 1:1 delta. Linux (Qt6 + GTK4) build + full ctest, 1850/1850; unverified live this session. Windows/macOS share the code, unbuilt
- fix(media): an animated sticker/GIF that scrolled off-screen and back could come back permanently blank — the pre-paint prefetch pass decoded it but always discarded the result (by design, deferring to the row's own lazy fetch), and could keep re-decoding and re-discarding it every pass without ever handing off. `run_media_prefetch_impl_()` now hands off to `ensure_media_image_()` directly instead of just releasing the shared guard. Linux (Qt6 + GTK4) build + full ctest, 1850/1850; user-verified live cross-platform. Windows/macOS share the code, unbuilt
- fix(media): an animated sticker/GIF in the timeline could go permanently blank after the app window was hidden for more than ~30s and restored — `run_image_gc_()`'s animated-cache TTL sweep ran on a timer regardless of window visibility, unlike the animation tick, so a hidden window's unpainted entries silently expired. Now skipped while no window is visible anywhere in the app. Linux (Qt6 + GTK4) build + full ctest, 1850/1850; reported live cross-platform, not yet re-verified live. Windows/macOS share the code, unbuilt
- fix(theme): on Windows, picking any accent theme other than Blue had no visible effect, since the OS-accent override applied unconditionally regardless of the chosen theme. Added a `System` accent option (now the default) so that override only applies when explicitly chosen. Linux (Qt6 + GTK4) build + full ctest, 1850/1850 (+3); Windows fix unverified live this session (no toolchain available). macOS unaffected
- feat(theme): three new accent-color themes (Forest, Sunset, Violet) alongside Blue, picked from a new "Accent color" combo in Settings → Appearance; each is WCAG AA contrast-checked. Linux (Qt6 + GTK4) build + full ctest, 1847/1847 (+13); unverified live this session. Windows/macOS share the code, unbuilt
- polish(mentions): a pill's leading avatar now nearly fills its rounded left cap (1px inset from the pill's own edges) instead of floating inset inside the chip with visible padding around it. Shared `tk::render_pill_bitmap`/`measure_pill` (`ui/shared/tk/pill.cpp`), so every pill everywhere picks it up. Linux (Qt6 + GTK4) build + full ctest, 1834/1834; user-verified live on Qt6. Windows/macOS share the code, unbuilt
- feat(mentions): typing `@room` in the composer and accepting it from the mention popup now shows the room's own avatar in the inserted pill too, matching received/timeline `@room` pills. Linux (Qt6 + GTK4) build + full ctest, 1834/1834; unverified live this session. Windows/macOS share the code, unbuilt
- feat(mentions): `@room` mention pills now show the current room's own avatar, the same avatar the room header uses (cache-peeked, fetched on miss). Applies to both spellings of a self-mention (plain text, and the spec-invalid `<a href="https://matrix.to/#/@room">` link some senders emit) — a Room-kind pill that's an actual permalink to a *different* room still gets no avatar. Linux (Qt6 + GTK4) build + full ctest, 1834/1834; unverified live this session. Windows/macOS share the code, unbuilt
- fix(mentions): a plain `m.text` `@room` message (no `formatted_body`, as some other Matrix clients send it — relying on MSC3952 `m.mentions.room` instead of HTML markup) rendered as literal text instead of a mention pill. The existing bare-text `@room` scanner, `split_room_mentions()`, was only ever invoked from inside the HTML-parsing path; it's now exposed and called from the plain-text fallback branches too, without ever parsing `m.body` as HTML. Linux (Qt6 + GTK4) build + full ctest, 1834/1834 (+3); unverified live this session. Windows/macOS share the code, unbuilt
- feat(notifications): new global "Mentions & Keywords" controls in the Notifications settings tab — separate on/off for @-mentions, @room, and general messages, plus a master "Notify on keywords" switch and an editable keyword list (wrapping chip layout), all backed by matrix-sdk's push-rule API. Keyword add/remove applies optimistically instead of re-fetching the server list, which raced with a concurrent `/sync` response clobbering the just-added keyword. Linux (Qt6 + GTK4) build + full ctest, 1831/1831; cargo test, 667/667 (+3); user-verified live on Windows. macOS unbuilt this session
- fix(mentions): a mention pill's mxid kept a literal `%3A` instead of `:` when a sender percent-encoded the whole matrix.to target, breaking avatar lookup and the mxid shown when the pill was clicked open. The URL→mxid helper only decoded a leading `%40`; it now delegates to the same `Client::parse_matrix_link` decode/validate already used for pill classification. macOS build + full ctest, 1831/1831 (+2) (1 pre-existing unrelated failure); user-verified live. Qt6/GTK4/Windows share the code, unbuilt this session
- feat(media): active low power mode now freezes animated images/stickers/emoji at their current frame in the timeline and the emoji/sticker pickers, resuming playback only while the pointer hovers that specific item; a new `AnimImageCache::set_paused()` gate backs this. Image/video viewers are exempt and always play. Linux (Qt6 + GTK4) build + full ctest, 1829/1829 (+5); user-verified live. Windows/macOS share the code, unbuilt this session
- feat(rooms): new "Browse" tab in Add Room — a searchable, paginated public-room directory (own homeserver or another via federation), with join-rule badges and a Join/Go action. Linux (Qt6) build + full ctest, 1824/1824 C++ + 664 Rust; user-verified live on Qt6. GTK4 blocked by a pre-existing, unrelated build break; Windows/macOS unbuilt this session
- perf(media): animated WebP stickers loaded noticeably slower on macOS than other platforms; they now decode via a vendored libwebp `WebPAnimDecoder` instead of ImageIO. macOS: user-verified live (~30-40x faster on previously-slow stickers); Windows/Qt6/GTK4 unaffected (not on this code path)
- perf(media): long animated stickers/GIFs/WebP (>40 frames) now decode a bounded rolling window instead of the whole animation up front, via a new per-platform `tk::AnimDecodeSession`. Windows build + full ctest, 1824/1824 (+9); user-verified live. Qt6/GTK4/macOS implemented against the same interface, unbuilt this session
- perf(media): new sticker/GIF loads skip the windowed decode session while Low Power Mode is active, falling back to a single whole-animation decode instead of ongoing per-frame top-ups. Windows build + full ctest, 1824/1824; unverified live this session. Qt6/GTK4/macOS share the code, unbuilt
- perf(pickers): the sticker/emoji picker's pre-paint prefetch now scopes to `GridView`'s viewport instead of warming every item in the active tab/search results. Windows build + full ctest, 1824/1824; user-verified live. Qt6/GTK4/macOS share the code, unbuilt this session
- fix(media): closed several races that could redundantly re-decode the same sticker concurrently, or let an unwindowed prefetch decode win the store race for an on-screen animation. Windows build + full ctest, 1824/1824; user-verified live
- fix(mentions): a timeline pill sharing a line with text could inflate that line's height on Windows — the D2D backend's `ascent()` reports full line height rather than true ascent, so the pill's baseline/descent split collapsed to zero. Now uses the existing `real_line_metrics()` helper instead of `role_line_metrics()`. Windows build + full ctest, 1814/1814; Qt6/GTK4/macOS unaffected (D2D-only)
- refactor(media): image caches now key on a typed `tk::CacheKey` instead of hand-prefixed strings, closing a real key-collision risk; existing on-disk cache files stay valid. Windows build + full ctest, 1814/1814; Qt6/GTK4/macOS share the code, unbuilt this session
- perf(media): animated GIFs/WebP/stickers now downscale per-frame while decoding and stream frame 0 immediately instead of decoding the whole animation before showing anything; the shared decode/IO pool sizes from `hardware_concurrency()` instead of a fixed 2 threads. Windows build + full ctest, 1814/1814 (+5) (1 pre-existing unrelated failure); user-verified live on Qt6/GTK4/macOS
- feat(pickers): the emoji/sticker picker's category tabs are now keyboard-focusable (Left/Right to move, Enter/Space to commit), with full `TabList`/`Tab` accessibility. macOS build + full ctest, 1805/1805 (+3) (1 pre-existing unrelated failure); user-verified live. Qt6/GTK4/Windows share the code, unbuilt this session
- fix(pickers): pressing Tab inside the emoji/sticker picker or the read-receipt popup escaped to the app's wider focus order and closed it, since those popups live outside `RoomView`'s own tree. `RoomView::paint()` now scopes Tab traversal to whichever picker/popup is open. macOS build + full ctest, 1805/1805 (+3) (1 pre-existing unrelated failure); user-verified live. Qt6/GTK4/Windows share the code, unbuilt this session
- fix(profile): the user-profile card's extended rows (Pronouns/Timezone/Bio/etc.) misaligned label/value baselines and never wrapped, truncating long values. Now baseline-aligned and wrapped (capped at 4 lines), sized from the real measured height. Linux (Qt6 + GTK4) build + full ctest, 1802/1802; user-verified live. Windows/macOS share the code, unbuilt
- fix(media): the image viewer opened a profile picture at the wrong size then jumped huge once the full-res avatar decoded, since zoom was never recomputed after the fit target changed; also fixed a `std::clamp` violation where a small thumbnail's zoom ceiling could be exceeded on a large screen. Viewer now re-fits on resolution change and opens at ~75% of viewport. Linux build + full ctest, 1802/1802 (+5); user-verified live on Qt6. Windows/macOS share the code, unbuilt
- feat(mentions): timeline and composer mention pills now share one renderer instead of two separate drawing paths, so they match in shape, color, and leading avatar. macOS build + full ctest, 1795/1795 (1 pre-existing unrelated failure); Qt6/GTK4/Windows share the code, unbuilt this session
- fix(mentions): composer pills dropped the redundant `@` from their label and no longer vary in height per mention, since they now read the compose box's stable body font instead of the text-insertion point's (which drifted). macOS build + full ctest, 1795/1795 (1 pre-existing unrelated failure); Qt6/GTK4/Windows share the `@`-drop, unbuilt this session
- fix(mentions): timeline pills on a wrapped message row rendered visibly stretched, since sizing read the whole layout's height instead of one line's ascent. Now uses stable, line-count-independent font metrics. macOS build + full ctest, 1796/1796 (+1) (1 pre-existing unrelated failure); Qt6/GTK4/Windows unaffected
- fix(mentions): a timeline pill was only clickable across roughly its left half on macOS, since `link_at()` used a caret-snapping API unsuited to one wide placeholder character. Now hit-tests via geometric x-offset containment. macOS build + full ctest, 1797/1797 (+1) (1 pre-existing unrelated failure); Qt6/GTK4/Windows not verified for the same issue
- fix(mentions): timeline pills rendered pixelated and taller than surrounding text on macOS — the canvas read an unscaled CTM for its own resolution, and layout fed CoreText a degenerate ascent/descent split. Both now use explicit real values (`backingScaleFactor`, `real_line_metrics()`). macOS build + full ctest, 1796/1797 (1 pre-existing unrelated failure); user-verified live
- fix(mentions): reply-quote previews never rendered mention pills, since the quote card bypassed `prepare_spans()`'s image-placeholder step; also fixed `QtRichTextLayout::line_count()` counting paragraphs instead of wrapped lines. Linux build + full ctest, 1797/1797; user-verified live (Qt6)
- fix(mentions): on Qt6 HiDPI displays a pill's background/text rendered at the wrong scale (avatar was fine), since the backing `QImage`'s devicePixelRatio was set after `QPainter::begin()` had already snapshotted it. Now set first. Linux build + full ctest, 1797/1797; user-verified live (Qt6 HiDPI)
- fix(gtk4): on HiDPI displays every native text field/area (pills included) rendered at a quarter size, anchored top-left, since a device-scale tag on the capture surface leaked into the generic image-blit path and compounded with its own resize. Reset to 1:1 after capture. Linux build + full ctest, 1797/1797; user-verified live (GTK4 HiDPI)
- feat(rooms): Room Settings' Permissions tab warns when a staged change would leave no other member able to edit permissions. Windows build + full ctest, 1791/1791 (+3) C++ / 667 Rust; unbuilt/unverified live this session. Qt6/GTK4/macOS share the code, unbuilt
- feat(rooms): Room Settings gains a Leave button in its footer. Windows build + full ctest, 1788/1788 (+4); user-verified live. Qt6/GTK4/macOS share the code, unbuilt
- fix(pins): the pinned-messages banner kept showing a pinned message's original text after it was edited, since edits weren't a notable-update trigger. Now resolves the latest edit and re-triggers via `EventCache`'s update stream. 664 (+6) Rust / 1781 C++ tests; user-verified live
- fix(previews): URL preview cards stretched their image to a fixed 56x56 square regardless of aspect ratio, since the event's already-parsed `image_w`/`image_h` went unused. Legacy previews now contain-fit within that slot; MSC4095 sender-bundled previews get a new image-on-top layout (320x200, aspect-fitted) with content-dependent card height. Linux (Qt6 + GTK4) build + full ctest, 1781/1781; user-verified live on Qt6. Windows/macOS share the code, unbuilt
...
- refactor(shell): LoginView and SettingsView are now built on first need and torn down once no longer needed, instead of living for the whole session — both are rarely used per session. Also fixes a crash this uncovered: `SettingsController` outlives Settings being closed, but `SettingsView` wired its callbacks with unguarded `this` captures, so a still-in-flight fetch could complete into freed memory; its destructor now unwires them. Windows build + full ctest, 1783/1783; user-verified live. Qt6/GTK4/macOS share the pattern, unbuilt
...
- feat(message_list): render MSC4095 bundled URL previews (`com.beeper.linkpreviews` / `m.url_previews`) — sender-bundled link-preview cards, encrypted thumbnails included, shown without a homeserver `/preview_url` call; multiple stack, an empty array opts out, and the homeserver fetch stays the fallback when no bundled field is present. Receiving side only. Linux (Qt6 + GTK4) build + full ctest, 1781/1781 (+1) C++ + 658 (+6) Rust; user-verified on Qt6 (previews from mautrix bridges); Windows/macOS share the code, unbuilt
- feat(rooms): the room-list / conversation separator is now a drag handle — resize between an icon-only minimum and a cap of half the window or the widest on-screen room row (measuring every room froze startup on large accounts). Icon-only mode hides the search field, buttons and section headers and collapses the account strip to its avatar, with a hover flyout showing each room's full row; a grip on the separator toggles it, and width + collapsed state persist to `app_settings.json`. Linux (Qt6 + GTK4) build + full ctest, 1780/1780 (+14), user-verified on Qt6; Windows/macOS share the code (new Win32 `Cursor::SizeWE`), unbuilt
- fix(message_list): switching message layout style (Classic/Bubbles/IRC) shifted the timeline's scroll position, since row heights differ between styles but `on_display_prefs_changed()` only invalidated cached layouts without compensating `scroll_y_`. Now routed through the existing `preserve_top_through` row-anchoring mechanism already used for history prepends and async row-growth. Windows build + user-verified live; Linux/macOS share the code, unbuilt
- fix(media): scrolling the room list could leave avatars stuck on initials for a long time even though the bytes were already on disk — the pre-paint disk-cache prefetch posted its disk-read+decode tasks onto the same 2-thread, strict-FIFO pool shared with the real network media-fetch pipeline and other background work, so a burst of prefetch tasks could queue behind a slow fetch indefinitely. Prefetch now runs on its own dedicated worker pool; `PixmapCache`/`AnimImageCache` also gained internal locking (mirroring the existing `CompressedImageCache` pattern) instead of relying on a UI-thread-only convention. Windows build; Linux/macOS share the code, unbuilt this session
- test(rooms,message_list): fixed 4 pre-existing test failures unrelated to any change this session — the room-list context-menu tests still expected the pre-"Mark as read" 4-item menu (now 6, with the new item and its separator); the table copy-selection test's drag loop broke out of its "walk into the next row" step immediately (its own word-select anchor already made `has_selection()` true) and used a fixed small x that only ever selected 1 of 5 characters into "omega". Windows build + full ctest, 1770/1770
- fix(emoji): emoji-only messages rendered at normal size instead of 2x ("BigEmoji") on Windows — `canvas_d2d.cpp`'s per-run emoji sizing always derived from `FontRole::InlineEmoji`, ignoring the `BigEmoji` role the body was styled with, so it stomped the layout's correctly-sized text format back down for every glyph. Fixed to match the Qt6/GTK4 role-aware pattern; applied the same missing guard to macOS's `canvas_cg.cpp`. Windows build + full ctest, 1770/1770 + user-verified live; macOS unbuilt
- fix(tray): the tray context menu never listed accounts to switch between — `rebuild_tray_()` only ran from each shell's constructor/destructor, before the tray icon exists (it's created lazily after login), and the item list was built per open *window* rather than per account, so a single-window multi-account session only ever saw "Show App"/"Quit". Tray rebuilds now also fire right after the tray icon is created and on every account switch/logout, and the menu lists every logged-in account (raising its popout window, or switching it into the current window, on click). Windows build + user-verified live; Qt6/GTK4/macOS share the code, unbuilt this session
- feat(rooms): the room list's right-click menu gains a "Mark as read" item, disabled once the room has no unread/notification count. Reuses the existing `mark_room_as_read` FFI call already used on room open. Windows build + user-verified live; Linux/macOS share the code, unbuilt
- fix(windows): the main window (and pop-out room windows) could show the native Win32 caption alongside the custom-drawn titlebar — caption hiding relies on `WM_NCCALCSIZE` firing before first paint, but the no-saved-geometry fallback size could exactly match the size already passed to `CreateWindowExW`, and `SetWindowPos` with an unchanged size/position never re-fires it (most visible over RDP's 96 DPI default). Both windows now force one `SWP_FRAMECHANGED` call before showing. Windows build + user-verified live
- fix(gtk4): all pointer input (clicks, scroll, hover) was dead across the entire GTK4 UI except the native text fields, broken since the a11y bridge landed — the `TkAccessNode` AT-SPI marker is a full-bleed `GtkOverlay` child, and with `can-target` at its GtkWidget default of TRUE `gtk_widget_pick()` returned it for every point, so the sibling `GtkDrawingArea` beneath it never saw an event. Marker nodes are now created `can-target=FALSE`. GTK4 build + user-verified live; no shared/Qt6 changes, test count unchanged (1766)
- refactor(tk): `CanvasFactory` now has one text-layout virtual, `build_rich_text` — `build_text` is a non-virtual shim wrapping the string in a single `TextSpan`, and each backend short-circuits a lone unformatted span to its cheap single-line path, so plain labels are unchanged while emoji sizing becomes uniform across all four platforms instead of Linux-only. Emoji segmentation also moved from `views/html_spans` to `tk/emoji_segmentation`, dropping a `tk/`→`views/` include. All four platforms build; Linux full ctest, 1766/1766 (+1) C++ + 652 Rust; user-verified on Qt6 + GTK4
- fix(accounts): the account-switcher popup clipped its last row and drew a square border. The four shells now size it from `AccountPicker::measure()` instead of a row height hardcoded before the MSC4426 sidebar grew, and it paints its own rounded card on a chrome-free host window. Linux build + full ctest, 1765/1765; Windows/macOS parts unverified on-device
- fix(qt6): an ellipsized label containing emoji (room name, dialog title, sender name) wrapped to two lines instead of truncating, since the emoji-sizing change routes such labels through `build_rich_text`, which didn't force a single line on Qt6. It now sets `NoWrap` with emoji-aware truncation; both Linux backends also clamp any `!wrap` layout to one line's height so a larger emoji can't shift a row. Linux build + full ctest, 1765/1765 (+2); Windows/macOS unaffected
...
- fix(pickers): the emoji/sticker picker's shortcode tooltip no longer lingers after a pick dismisses the picker without the pointer moving. Linux (Qt6 + GTK4) build + full ctest, 1757/1757
- feat(profile): MSC4426 user status — a self-set emoji + short text status (Settings › Account, shown on the profile card) plus an automatic "In a call" indicator (`m.call`) while in a MatrixRTC call; profile-field read/write migrated off hand-rolled reqwest onto matrix-sdk's typed API. Linux (Qt6 + GTK4) build + full ctest, 1757/1757 (+4) C++ + 652 (+11) Rust; SDK migration and `m.call` need live verification
- refactor(rooms): MSC3030 focused-timeline completion-side restore (historical-mode gate, scroll-to-focus-event, return-to-live scroll-to-bottom) moved from `ShellBase` onto `RoomPane::on_timeline_reset`, keyed by the pane's own room id — a pop-out that jumps to a date/permalink now gets the same restore-on-reset behavior the main window always had, instead of only beginning the focused subscription and never re-arming the gate. Linux (Qt6 + GTK4) build + full ctest, 1753/1753 (+3)
- fix(media): pop-out windows now load thumbnails for old (backward-paginated) messages. `fetch_media_pipeline_`'s delivery gate only ever recognized the main window's current room or an open gallery as "live", silently dropping any media fetch for a pop-out showing a different room — already-cached thumbnails still rendered, which is why only scrolling into never-before-fetched history exposed it. Linux (Qt6 + GTK4) build + full ctest, 1750/1750 (+2); user-verified live
- refactor(rooms): MSC3030 focused-timeline/date-jump initiation moved from `ShellBase` onto `RoomPane`; also removed redundant `on_date_jump` rewiring in all four platform shells that silently overrode `RoomPane`'s own wiring for the main window. Linux (Qt6 + GTK4) build + full ctest, 1748/1748; user-verified live; Windows/macOS unbuilt
- refactor(rooms): compose-draft save/restore moved from `ShellBase` onto `RoomPane` — the "apply" half previously existed nowhere on `ShellBase` itself, only duplicated across all four platform shells' room-switch handlers. Linux (Qt6 + GTK4) build + full ctest, 1748/1748; user-verified live; Windows/macOS unbuilt
- refactor(rooms): pinned-events-banner refresh unified onto `RoomPane`, replacing `ShellBase`'s own copy and `RoomPane`'s separate one for pop-outs with a single implementation that now consistently dispatches the pin/redact permission check off the UI thread. Linux (Qt6 + GTK4) build + full ctest, 1748/1748; user-verified live
- refactor(rooms): reply-quote resolution moved from `ShellBase` onto `RoomPane`; `build_rows_` now resolves a reply-quote against the row's actual room instead of always assuming the current one, fixing a latent bug where a pinned-open gallery or a pop-out on a different room never resolved its reply previews. Linux (Qt6 + GTK4) build + full ctest, 1748/1748; user-verified live
- ci(flatpak): bump the Flatpak `org.kde.Platform`/`org.kde.Sdk` runtime from 6.9 (EOL on Flathub) to 6.11, and drop the `org.freedesktop.Sdk.Extension.golang` install plus the `golang`/`perl` apt packages from the flatpak/build-platform/package/update-screenshots CI workflows — unused since aws-lc-sys's cc builder stopped needing Go or Perl. CI-only change, unverified until it runs
...
- build(ci): new manually-triggered `Compile Check` workflow builds Tesseract on Windows, macOS (arm64), and Linux (Qt6) with the release presets, compile only — no packaging, no artifact upload. Meant as a fast, cheap sanity check to run before `package.yml`'s real publish pipeline
- feat(spaces): the space summary view gains a "Leave Space" button. It reuses the existing generic leave-room confirm/command chain, now tagged so a successful leave also steps back out to the parent space (or room list root) instead of stranding the UI on the now-gone space's summary — mirrors the room list's own back-button navigation. Linux (Qt6 + GTK4) build + full ctest, 1748/1748; macOS/Windows share the code, unbuilt
- fix(spaces): a room left while it was a child of a Space no longer vanishes from that space's "available to join" list. `space_children()`'s membership filter checked only that the room was known to the local store, not that it was still joined, so a left room's own state kept it wrongly counted as joined and hidden from both halves of the joined/unjoined split. Rust build clean; no C++ changes needed
- fix(macos): switching the theme setting to "System" right after an explicit Light/Dark choice now restyles the whole app immediately instead of only the titlebar (contents stayed on the old theme until restart). `os_color_scheme_()` read `NSApp.effectiveAppearance` while it was still pinned to the prior explicit choice; it's now cleared first so the read reflects the live OS appearance. macOS build-verified, user-verified live
- fix(media): the pre-paint prefetch shared its in-flight guard with the real network-fetch path, so on a cold disk cache the prefetch's disk-only check could block the very next paint's lazy fetch from ever starting a download — room-list avatars (and other media) stayed stuck until clicked. Prefetch now single-flights against its own guard set and only reads, never inserts into, the real fetch path's. Full ctest, 1748/1748 (+2); user-verified live on Linux
- fix(media): the pre-paint prefetch (see below) never deduped its dispatched keys against the real fetch pipeline's in-flight guard, so a key that couldn't resolve within one frame got redispatched on every subsequent paint, flooding the shared decode pool and starving real fetches — visible on macOS as the pending-fetch count climbing while nothing rendered. Now guarded the same way `ensure_media_image_`/`ensure_room_avatar_` already are. User-verified live on macOS
- feat(media): images, stickers, reactions, and avatars already on disk now decode before each paint instead of on first draw, bounded by a 2ms deadline so a slow decode can't stall a frame. Linux (Qt6 + GTK4) build + full ctest, 1748/1748 (+19); user-verified live on Linux and macOS
...
- fix(threads): pasting/dropping an image, video, audio file, or document while a thread is open now sends it into the thread instead of the room. Linux (Qt6 + GTK4) build + full ctest, 1721/1721
- fix(threads): an open thread now paginates its own older messages on scroll-to-top instead of only ever showing its newest batch. Linux (Qt6 + GTK4) build + full ctest, 1721/1721; user-verified live
- fix(ui): bubble layout now reserves room for the read-receipt cluster, so a wide message from another user no longer hugs out under it. Linux (Qt6 + GTK4) build + full ctest, 1721/1721 (+1)
- build(sdk): bump the matrix-rust-sdk fork to `1d1c0cb` (~280 upstream commits) — dehydrated devices (MSC3814), automatic call status (MSC4426), RTC transport discovery, room retention, and MSC2545 image-pack stabilization. Required follow-on fixes: ruma repinned to the fork's own git rev (matrix-sdk moved off crates.io ruma mid-range), rusqlite 0.37 → 0.40, `image_packs.rs` migrated off the now-removed unstable-msc2545 ruma feature (per-image `usage` overrides dropped; pack-level `usage` still applies), `qr_grant.rs`'s new mandatory `continuation_sender` handshake, and `thread.rs`'s updated `load_user_receipt` signature. 1719 C++ / 638 Rust tests pass; user-verified QR login and sticker/emoji packs live
- fix(compose): the compose box's text area could get stuck at roughly double height under HiDPI scaling until the first edit — `set_rect()` applied a new width to the native control but never re-queried its natural height against it. Now re-measures whenever the width actually changes
- fix(threads): bubble layout's thread-reply preview chip capped itself to the bubble's own hugged content width, truncating short messages with long thread previews almost immediately. It now sizes off the full row width like Classic/IRC, hugs its own measured text instead of always stretching, and own (right-anchored) bubbles anchor/grow the chip from the right instead of the left. 1719 C++ ctest pass; user-confirmed live on Linux
- fix(threads): sending a sticker into an open thread no longer lands it in the room. The main window read a stale, separate copy of thread-panel state; `RoomPane` is now the single source of truth for both the main window and pop-outs. Linux debug + release build, full ctest + cargo test; user-verified live
- fix(threads): a sticker sent into a thread now shows up in the thread view — stickers converted through a path that never extracted MSC3440 thread metadata, so incoming stickers always got an empty thread root. User-verified live against Synapse + element-web
- fix(tests): `wipe_local_stores`'s Rust tests built temp-dir names from `SystemTime`'s `Debug` output, which embeds a colon — invalid in a Windows path component, so `create_dir_all` failed there. Derived from nanos-since-epoch instead
- deps: fix RUSTSEC-2026-0258 (h2 unbounded empty DATA frames) and RUSTSEC-2026-0274 (rtrb double-free/UAF in `ReadChunk::commit`), plus an unsound `event-listener` Send/Sync impl and a yanked chacha20 release. `webrtc-sys-build`/`livekit-*` stay pinned to the last combination verified to build, since the advisory-driven `cargo update` also wanted to bump those and broke the build

## v0.8.20 — 2026-09-04

### Summary

- fix(threads): the per-thread unread dot now survives an app restart — read markers persist to a new per-device `thread_read_state.db` instead of relying on Synapse to echo threaded receipts back. Linux (Qt6 + GTK4) build + full ctest; macOS/Windows share the code, unbuilt. +7 Rust tests
- fix(tk): a disabled widget is now opaque to input — pointer, key, and drop dispatch stop at it instead of falling through to whatever's behind or inside it, and clicking one no longer clears keyboard focus. Linux (Qt6 + GTK4) build + full ctest; macOS/Windows share the code, unbuilt. +6 C++ tests
- feat(macos): the application menu bar now has an entry for every app-level keyboard-shortcut action, previously only reachable via an invisible `NSEvent` monitor. macOS build-verified, user-confirmed working
- fix(win32): the accessibility bridge no longer crashes on shutdown — a reentrant `WM_GETOBJECT` could reach back into the bridge mid-teardown and invalidate its provider map. Windows build-verified
- feat(tk): scrolling the mouse wheel over a closed `ComboBox` cycles the selected option, matching native combobox behavior. Windows build-verified, user-confirmed on Qt6, GTK4, and macOS too
- fix(ui): bubble layout no longer reserves blank space under a portrait sticker — measure and paint passes disagreed on the sticker's height cap. Linux (Qt6 + GTK4) build + full ctest. +1 C++ test
- fix(ui): a bubble now hugs its content with no minimum width, so a one-word own message sits flush to the right edge instead of padded inside a wider bubble. Linux (Qt6 + GTK4) build + full ctest. +1 C++ test
- fix(tk): widget visibility now cascades, so a widget under a hidden ancestor reports hidden — fixes the thread-list/thread-view search fields staying painted after the panel closed. Linux (Qt6 + GTK4) build + full ctest. +4 C++ tests
- feat(ui): Appearance → Layout shows a live preview beside the message-layout combo box, updating as the selection changes. Windows-verified; user-verified on Qt6, GTK4, and macOS too
- feat(threads): thread-list panel header gains a "mark all threads as read" button. Linux (Qt6 + GTK4) build + full ctest; user-verified on macOS/Windows too. +2 C++ tests
- feat(threads): unread-thread indicator — a dot on the room-header threads button and thread-list rows, driven by new MSC3771 threaded read receipts (which Tesseract now also sends). Linux (Qt6 + GTK4) build + full ctest; user-verified on macOS/Windows too. +6 C++ / +12 Rust tests
- fix(join): the Join / "Request to Join" button is disabled for the duration of the async join, so a double click can't fire a duplicate request. +3 C++ tests
- refactor(settings): ~20 duplicated SettingsView callbacks across all four shells now live once in `ShellBase`; each shell keeps only its genuinely platform-specific parts. Windows build-verified; user-verified on Linux and macOS too
- fix(join): joins now route through via servers, so a Space child room on another homeserver joins instead of failing with "join failed or cancelled". User-verified end-to-end
- fix(freeze): the known-users roster sweep no longer freezes the client on a stalled member fetch — it now reads from the local store instead of an untimed network request. Built + tested on the presence/room-list/roster subset
- feat(power): low power mode — Settings → General → Power: `Auto` (default) / `On` / `Off`, halting background work (backfill, prefetch, GC, search indexing) while keeping message sync live. Linux (Qt6 + GTK4) build + full ctest; user-verified on Windows and macOS too. +9 C++ / +1 Rust tests
- refactor(timeline): the three message layouts' divergent behavior (monospace body, IRC prefix, day separators, reply rendering) is now a hook on `MessageRowRenderer` instead of scattered global-setting branches. Behaviour-preserving. Linux (Qt6 + GTK4) build + full ctest; user-verified live
- feat(ui): IRC layout shows a reply's context as a dim scrollback line above the reply instead of the framed quote card. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows too. +2 C++ tests
- fix(ui): a short grouped message is no longer padded up to the hover-action-pill height — the pill now overflows upward into the row above instead. Linux-verified. +4 C++ tests
- perf(settings): opening the Settings view no longer blocks the UI thread — the search-index stats read and "launch at login" OS query now run off-thread. macOS-verified; other shells share the code, unbuilt
- feat(ui): third message layout — `IRC` in Appearance → Layout, a monospaced mIRC look-alike with per-person coloring and IRC-style system lines. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows too. +6 C++ tests
- fix(ui): compose bar's Send button now shows an icon instead of a text label, matching the other compose-bar icon buttons. Linux (Qt6 + GTK4) build; user-verified visually
- fix(tk): `Widget::set_enabled()` repaints its own bounds on a real change, so a toggled widget no longer stays visually stale until an unrelated repaint. Linux (Qt6 + GTK4) build + full ctest. +1 C++ test
- feat(ui): message layout is now a choice — Appearance → Layout combobox, `Classic` (default) or `Bubbles`. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows too. +14 C++ tests
- fix(ui): bubble layout — the sender name is no longer clipped to the hugged bubble width, and a message with rich formatting no longer forces the bubble to full width. +2 C++ tests
- fix(ui): the main window, room pop-outs, and call windows now all enforce a minimum size, so none can be resized down to unusable dimensions. Windows-verified; user-verified on macOS/Qt6/GTK4
- feat(linux): the AUR package's update checker queries the AUR RPC API instead of GitHub Releases. +1 Rust test
- fix(arch): Arch/AUR builds no longer double-inject hardening flags that conflicted with makepkg's own
- fix(qt6): an update-available notification no longer blanks the status bar
- feat(ui): the reply-quote card renders the replied-to message's inline formatting instead of its raw markdown-ish plain body. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows. +2 C++ tests
- feat(ui): Markdown tables render as a column-aligned grid, with per-cell and cross-cell text selection/copy. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows. +11 Rust / +29 C++ tests
- fix(tk): native text controls inset 2px inside their border box by default, so the opaque native capture no longer paints over rounded borders on Qt6. Linux-verified. +2 C++ tests
- fix(windows): the native text control recovers its D3D device on `DEVICE_HUNG`, not just `DEVICE_REMOVED`/`DEVICE_RESET` — the common TDR case on hybrid-graphics laptops. Unbuilt (written from Linux)
- build(flatpak): regenerate `cargo-sources.json`, add missing metainfo releases, and add a PR-triggered CI job that builds the manifest offline. Manifest build tested and works
- perf(build): Linux builds use jemalloc as the process-wide allocator, tuned to return freed pages to the OS — addresses RSS climbing far past the live heap in long media-heavy sessions. Linux-verified
- feat(media): the room media gallery is backed by a persistent per-room index instead of re-scanning the SDK timeline on every open; also fixes gallery thumbnails that never loaded when not already cached by the chat timeline. Linux-verified; user-verified on macOS/Windows. +9 Rust / +3 C++ tests
- perf(media): decoded-image caches are garbage-collected to the on-screen working set instead of growing unbounded, plus a 32 MB compressed-bytes tier for fast scroll-back re-decode. Qt6-verified; user-verified on GTK4/macOS/Windows. +17 C++ tests
- fix(settings): "Clear all caches" now wipes state/event-cache/media stores and re-fetches the room list, keeping the crypto store and session so you stay logged in. User-verified on Linux, macOS, Windows, GTK4
- fix(tk): `ConfirmDialog` no longer clips body text past ~4 lines
- fix(macos): the full-screen media viewer hides the bottom status strip, matching the other shells. macOS-verified; user-confirmed
- fix(media): closing a full-screen image viewer by clicking outside no longer flashes the image at 1:1 zoom. macOS-verified; user-confirmed
- fix(search): the search index's one-time history backfill restarts after "Clear all caches" instead of leaving old history un-searchable
- fix(tk): `Label` text/style changes schedule a coalesced relayout instead of waiting for an unrelated repaint
- fix(settings): the indexed-size figure shown while indexing is re-measured every poll tick instead of frozen at its open-time value
- i18n: wrap previously-bare strings in Settings → Privacy → Search. New `es.po` / `pseudo.po` entries
- docs: three code comments called the FTS search index `app_cache.db`; it lives in `search_index.db`
- feat(media): full-screen mode for the image and video viewers, edge-to-edge with auto-hiding controls. Qt6/GTK4 verified; user-verified on Win32/macOS
- fix(qt6): `QAccessible::Switch` (Qt 6.11+) now falls back to `CheckBox` below 6.11, matching the project's Qt 6.4 floor
- fix(oauth): sign-in against servers that reject a ported loopback redirect URI at dynamic client registration now works; the error path also surfaces the real registration failure instead of a generic message. Linux (Qt6) build + full ctest; user-verified live against continuwuity
- fix(oauth): clicking Cancel while waiting on the browser redirect no longer freezes the app — a self-deadlock over the exclusive FFI lock. Linux (Qt6 + GTK4) build + full ctest; user-verified live
...
- fix(macos): progressive video streaming also shipped broken — the resource loader mishandled AVFoundation's content-length request, so it never started playback. Fixed to report the real length once known and finish info-only requests properly. User-verified on macOS
- fix(shell): macOS's room-list search didn't refresh the list or search past root-level rooms — its wiring dropped a required refresh call the other shells made. Hoisted the shared search/debounce wiring into `ShellBase` for all four shells. User-verified on macOS
- fix(shell): the main window's title didn't follow the active room on Qt6/GTK4/macOS — the update hook was wired but never actually invoked. Fixed with a direct per-shell call at each room-change site. User-verified on Qt6, GTK4, and macOS
- fix(tray): on Wayland/KDE, a tray-icon click on a visible-but-inactive window only lit the taskbar entry instead of raising it — Tesseract was discarding the compositor's granted activation token. Fixed on Qt6 and GTK4 to consume and apply it. Confirmed live on Plasma/Wayland
- fix(tk): `NativeTextArea::natural_height()` reserved a shorter row for an empty compose box than once it had content, so the focused text view overflowed its frame. Fixed the empty-placeholder measurement on macOS and GTK4 to match
- fix(video/qt6): progressive video streaming froze the app on nearly every streamed video — `GrowableQIODevice::bytesAvailable()` deadlocked by re-locking its own mutex via a base-class call. Fixed to avoid the re-entrant lock. Verified live and via full ctest
- fix(tests): four popup relayout tests still asserted synchronously after a prior commit moved relayout onto a deferred path — updated to flush the UI-thread task queue first. No production behavior changed
- fix(rooms): own messages could show up to 3x in the timeline after switching to a warm room — the withheld-tail cap desynced live insert/update/remove indices from the trimmed timeline. Fixed by adjusting indices for the withheld count across all four shells' live-update paths
- feat(tk): `tk::Widget` now requests its own relayout automatically on visibility/child/layout-hint changes instead of relying on ~150 hand-written follow-up calls. Also fixes a real relayout storm found during testing (the room-search field fighting its own visibility) via a per-pass suppression guard. Windows build-verified; other platforms written by analogy, unverified
- feat(export): restyle HTML room-history export to resemble the live timeline — sender avatars/colors, message grouping, reaction chips, reply cards, and a light/dark palette matching the app's theme. Plain-text export unchanged. Windows build+test-verified
- fix(export): the time-range cutoff's truncation logic was inverted, silently emptying a range-limited export whenever the cutoff was actually crossed; it also cleared completed-export checkpoints incorrectly and showed the wrong cutoff date in the dialog. All three fixed, plus two missing relayouts that left dialog buttons stale. Windows build+test-verified
- feat(export): add a time-range selector and a "Stop & save" action to the history-export dialog — Stop finishes and assembles whatever was gathered so far rather than discarding it like Cancel. The chosen cutoff now persists in the resume checkpoint. Windows build+test-verified
- perf(ui): a room switch could trigger up to 4 synchronous full-tree relayouts, a full timeline row remeasure, and a full room-list rebuild on every sync tick — now coalesced, capped to a viewport's worth of rows, and skipped when the room list is unchanged
- fix(ui): a WCAG AA contrast audit of the light/dark palette fixed several under-threshold text/icon/accent colors, added a `border_strong` token for input fields whose fill matches their card, and gave invite/knock/unjoined room rows the same selection accent bar joined rooms already had. Windows build+run-verified
- fix(windows): the status bar's font didn't follow a live DPI change because it read a stale screen DC DPI instead of the window's own — fixed to use `GetDpiForWindow`. Also fixes the tray context menu not scaling to the correct monitor's DPI. Windows build+run-verified
- feat(shell): add a Ctrl+Tab / Ctrl+Shift+Tab MRU room switcher, Alt-Tab-style, via a new shared `MruSwitcher` widget and a `tk::Host::set_on_ctrl_key_up()` hook. Windows build+run-verified; other platforms written by analogy, unverified
- fix(ui): popup pickers and autocomplete dropdowns didn't recompute or close their position on window resize, leaving them stranded — each shell's resize handler now dismisses all open popups. Windows-verified; user-verified on Qt6 and macOS; GTK4 unverified
- feat(shell): show the active room's name in the main window's OS title, reverting when no room is selected — wired on all four platforms via the existing pop-out-window title hook the main window never used
- fix(windows): the custom title bar's text was stuck near-black in dark mode because style/theme were applied after the first text was set, so BetterText's font-fallback color logic never took effect. Fixed by applying style/theme before any text change
- feat(windows): replace the stock title bar with a self-drawn Windows 11-style extended one matching the app's Mica/dark-caption theming, on both the main window and pop-out windows, preserving Snap Layouts and the system menu
- feat(tk): migrate Win32 swap chains to composition-based presentation (`CreateSwapChainForComposition`), fixing a visible stretch artifact during interactive window resize
- feat(bettertext): add a static-text control kind and use it to render the title bar's window title, giving it real color-emoji rendering. Also fixes a stray overlay scrollbar thumb on static controls and a title-bar flicker on activation. Removes the dead pre-BetterText `TextRenderer.cpp`/`.h`
- fix(sdk): the crypto store was never closed on logout, leaving `-wal`/`-shm` handles open and corrupting a subsequent login's store. Fixed by calling the SDK's existing `client.pause()` on logout/Drop instead of carrying a forked patch
- feat(media): progressive ("fast-start") video streaming on all four platforms — video begins playing while still downloading, with a scrub bar and disk caching of completed downloads. Also fixes video/image overlays not cancelling in-flight fetches on close, letting audio keep playing
- fix(shell): a multi-commit arc fixing account logout/relogin reliability — closes SDK stores on logout to prevent corrupted stores on relogin, drains in-flight background tasks before logout returns, nulls out cached raw `Client*` pointers across views, and fixes error messages being wiped or truncated during login/restore failures
- feat(threads): add in-thread find/search and dim the main timeline whenever the thread list panel is open, not just a single thread. Also fixes `ThreadListView`'s header scrolling away with its content, both thread views being constructed without a `host()`, and the thread-dim/nav-loading scrims stacking into a darker-than-intended fill
- fix(windows): the GPU driver mis-rendered the I-beam cursor's invert mask on some internal displays after the move to flip-model presentation, showing white-on-white. Fixed by flattening the cursor into a solid black/white bitmap colored from the app's theme instead of relying on OS-level inversion
- fix(avatars): timeline and sidebar avatars stayed blank after a DPI/monitor change until an unrelated repaint forced recovery — added explicit re-fetch triggers for `MessageListView` and `UserInfo` matching `RoomListView`'s existing self-healing pattern
- fix(windows): `WM_MOVE` persisted window geometry even while minimized, using Win32's tiny iconic placeholder rect — closing the app while minimized corrupted the saved window size. Fixed to skip persisting while minimized
- fix(receipts): declare `ReceiptGridPopup`'s destructor explicitly instead of relying on an implicit one, which broke the build due to an incomplete forward-declared member type
- feat(receipts): read receipts now show a timestamp and a "+N" overflow pill opening a scrollable grid of every reader beyond the inline avatar cluster. Also fixes three spots that overlapped the overflow pill with other UI
- fix(export): widen `format_short_date`'s stack buffer from 16 to 32 bytes to silence a theoretical overflow flagged in release builds
- build(ci): the tag-triggered release job now publishes each GitHub Release as a pre-release, promoted manually once verified
- fix(tk): the main window's opaque D2D surface used single-buffer presentation, which DWM can silently stop compositing after extended idle or a display switch, leaving the window frozen with no error. Moved all surfaces to flip-model presentation, with `Host::on_paint()` now running up to two paint passes per `WM_PAINT` to keep both buffers current
- feat(tk): DPI/HiDPI display-scale awareness across all four platforms — a shared scale-change signal fixes stale/under-rasterized native-control captures and scales avatar/thumbnail fetch size with the current DPI factor. Linux verified via full suite; Win32/macOS written to the same pattern, unverified
- fix(media): media fetches from all rooms/homeservers shared two lane-wide rate gates, so a stalled or dead homeserver could throttle unrelated media from other servers. Fixed with a per-origin gate registry so throttling stays isolated to its own origin
- fix(export): fixed several compounding bugs in room-history export — a stale checkpoint resumed incorrectly, the oldest-event tracker picked the wrong end of the window, progress counts overcounted and used an indeterminate bar instead, the anti-flash guard measured from the wrong point, and progress didn't reliably repaint. Also renders HTML export images at the live timeline's scale-to-fit size
- fix(tests): `catch_discover_tests()` mangled non-ASCII test names into mojibake under CMake's ANSI code-page fallback, causing ~14 tests to be silently skipped by ctest. Patched the vendored CMake script to force UTF-8; all 1481 tests now pass
- fix(tk): `Surface::end_paint()` only recognized device-removed/reset as device loss, not a hung device (the common TDR case), so the window could silently stop repainting forever. Now also recognizes device-hung, and swallows a failed target-reacquisition instead of crashing
- feat(export): add full room-history export (plain text or HTML, optionally with images, optionally zipped) via a new "Export History" button in the room info panel, with a resumable checkpoint and a new progress bar widget. Windows-verified; other platforms written by analogy, unbuilt
- fix(bettertext): the compose bar's caret-visibility check compared unrounded DIP scroll bounds against a rounded physical-pixel client rect, causing scroll position to ping-pong on every keystroke. Fixed to compare at physical-pixel granularity, plus related rounding fixes in vertical centering and resize hysteresis
- docs(claude): require confirming the selected fix with the user before implementing a bug fix, and offer to set breakpoints if the first fix attempt fails
- fix(tray): relaunching the app while tray-hidden correctly detected the running instance but never showed its window again on Windows or macOS. Fixed both to un-hide the existing window; macOS gained a cross-process notification to request it. Windows-verified; user-verified on macOS
- fix(tray): tray-icon clicks now correctly show/raise/hide across all four shells instead of a naive toggle. Windows needed two additional fixes for foreground-detection and double-click double-toggling; macOS, Qt6, and GTK4 gained the same three-state active/visible logic
- fix(tests): 22 test cases declared their D2D-backed test surface after the widget tree it painted, so COM tore down before D2D resources were released, segfaulting on exit. Reordered declarations to fix all 22
- fix(tests): ctest reported false "No test cases matched" failures because the CRT's narrow `main()` mangled non-ASCII test names via the ANSI code page. Enabled Catch2's existing wide-`main()` entry point to fix it
- feat(compose): unsent text, a staged attachment, and caret position now persist per room instead of being wiped on every switch. Also fixes an unclear staged-attachment survival across room switches and macOS never clearing compose state on switch
- fix(bettertext): `GetClientRect` returns physical pixels while the control's DirectWrite layout operates in DIPs, so wrap width was computed too wide on any non-100%-scaled display, letting compose-bar text run under the buttons before wrapping. Fixed to convert consistently
- fix(tk): swap-chain creation could fail with a removed device before any frame had ever presented, previously crashing the app on startup. Now retries once with a freshly recreated device
- feat(crash): add an optional local crash handler (off by default) that writes a plain-text native/Rust panic stack trace to disk; nothing is transmitted anywhere. Also adds Release debug-info generation behind the same flag so traces can be symbolicated
- fix(tk): a prior DPI fix for popup anchor rects broke mention/slash-command/shortcode/GIF popups above 100% DPI because `BetterTextArea::cursor_rect()` still returned physical pixels, double-scaling on top of the new conversion. Fixed to return genuine DIPs like the other backends
- fix(macos): the room-list search field's clear (x) button was drawn and hit-tested but inert on macOS because the shell never wired its callback, unlike the other three platforms. User-verified on macOS
- fix(tk): `Win32PopupSurfaceHandle::set_rect()` passed a DIP-space anchor rect with no DPI conversion, undersizing dropdown popups above 100% scaling and clipping trailing rows. Fixed to convert properly
- fix(tk): repaint/damage-rect invalidation built its `RECT` from DIP-space widget rects without DPI conversion, so on scaled displays the invalidated region missed changed pixels — visible as compose-bar text and animated-image partial repaints lagging behind. Fixed to convert consistently
- fix(sdk): `audiopus_sys`'s dynamic-linking override only applied to macOS, so Windows statically linked a second copy of Opus that conflicted with `webrtc-sys`'s bundled copy at link time. Widened the override to include Windows; verified via `cargo test` and a binary dependency check
- fix(sdk): `TransportsResponse` deserialized the wrong JSON key for the MSC4195 `/rtc/transports` response, silently falling back to `.well-known` on every server and hiding call UI on servers that only expose the direct endpoint. Fixed the field name and added support for registry-prefixed unstable type strings
- fix(qt): `QtNativeTextArea::natural_height()`'s placeholder measurement used a not-yet-resized field width on first layout, producing a bogus height until the first keystroke corrected it. Fixed to fall back to an unwrapped measurement when the real width isn't known yet
- fix(rooms): `RoomInfoPanel`'s buttons and the compose bar's attachment-remove badge let scrolled content show through their transparent-at-rest fill. Given an opaque fill instead. Also adds a tooltip to the Create Room visibility combo box
- fix(rooms): the Create Room invite field's placeholder could wrap to two lines but the field never grew, hiding content — all `NativeTextArea` backends now measure the placeholder for natural height when empty. Also adds a `tk::GroupBox` widget and fixes square corners on Windows dropdown popups
- feat(rooms): add room knocking support (MSC2403) — request to join, track/cancel pending requests, and let admins review/accept/deny requests, across all four shells. Also fixes federated knocks failing silently due to a missing server routing hint
- feat(rooms): show a "Start calls" permission in Room Settings Permissions, exposing power-level control over MatrixRTC call-member state events that was previously unexposed
- fix(tk): several follow-up fixes for native text controls rendering into the canvas — a doubled DPI division halving the Win32 caret, stale captured images after Qt6 scroll/programmatic text changes, zero vertical inset in the compose bar's text-area rect, and a non-scrollable Win32 compose box (now with wheel, keyboard, and scrollbar support)
- feat(login): show a clear "No Internet Connection" dialog on cold start when offline, instead of a raw backend error, using a new one-shot connectivity probe per platform. Linux verified via full suite; Win32/macOS unverified
- fix(rooms): cold-start room restore had gotten slow due to synchronous seed-event checks, blocking permission checks, and background-warming dispatches competing with the restore itself — all moved off the critical path. Also adds debounced auto-save of room layout so it survives a crash
- perf(tk): `rebind()` cleared the D2D emoji-bitmap and brush caches on every paint even though they're device-scoped, forcing a fresh PNG decode per emoji on every repaint instead of once per glyph
- fix(sdk): six background-task bookkeeping methods took the exclusive FFI lock just to write an `AbortHandle` field, occasionally holding it during a UI-thread stall on the room-switch path. Moved those fields to their own mutex so the methods no longer block other FFI readers
- fix(rooms): the room search field fell back to the root background instead of the rounded search card's fill, and was sized flush with the card, squaring off its rounded corners
- fix(rooms): the overflow menu's Delete action was gated purely on message ownership, hiding it from moderators/admins with sufficient power to redact others' messages. Added a power-level-aware check
- fix(rooms): favorite rooms subscribed to at startup showed stale/empty content because the prefetch set only considered unread state — favorites are now always included. Also kicks off a background backfill immediately when a newly subscribed room's cached timeline is thin
- perf(tk): startup eagerly constructed `RoomSettingsView`'s native text-field subtree before the panel was ever opened — native construction now happens lazily on first show/arrange instead of at construction
- fix(replies): a reply-preview quote stuck on "Original message unavailable" never retried once the quoted message loaded later via pagination. Added a retry that re-fetches for any still-unresolved reply whose target newly arrives
- fix(rooms): the Create Room dialog's topic and invite fields were fixed-height with no auto-grow, hiding content beyond a couple lines. Now grow with content, capped so buttons stay visible
- fix(tests): the i18n tests hardcoded `/tmp` for writing MO catalogs, which isn't valid on Windows, silently failing all five there. Uses a portable temp dir instead
- fix(build): drop the MinGW cross-compile presets — `webrtc-sys`'s prebuilt Windows library is MSVC-only with no MinGW support, so cross-compiling can't link. Verifying Win32 UI changes now requires an actual Windows machine
- feat(shell): add a 30-minute idle-TTL eviction tier for warm room/thread timelines — the existing warm-LRU permanently exempted active/tab/pinned/favorite rooms from eviction, letting a long session accumulate unbounded live timelines. The new tier reclaims anything not actually on-screen, riding the existing presence tick
- feat(sdk): use MSC4491 atomically when the homeserver advertises support — attaches the invite reason directly inside `createRoom` instead of a two-step create-then-invite fallback
- feat(rooms): add optional invite reasons to room creation, `/invite <user> [reason]` (via a new quote-aware slash-command tokenizer), and quick switcher DM creation — attached via the per-invite `reason` field
- feat(rooms): show an invite's reason (if set) on `InviteCard`, for both DM and group variants
- feat(tk): render native text controls into the canvas instead of overlaying them, so they participate in clip/opacity/transform like any other canvas content. Backends whose real control loses OS input on going invisible get explicit forwarding for pointer/hover/caret behavior
- fix(ci): scope the signing-cert secret to only the steps that use it, reading presence via a dedicated check step's output instead of the `secrets` context directly, which GitHub rejects in that context
- fix(windows): hide `main_app_surface_` until session restore completes, matching `settings_surface_`'s existing behavior — previously the room list was visible alongside the brand splash before restore finished
- refactor(tk): make `SettingsPage`/`KnownPacksList` real `ScrollableBase` widgets instead of hand-rolled scroll copies, picking up shared thumb geometry, kinetic momentum, and real thumb-drag support
- refactor(tk): make child painting automatic instead of requiring every `paint()` override to call `paint_children()` itself; adds `z_order()` for paint-last pinning. Converts ~65 overrides across the shared toolkit and views
- refactor(windows): remove the dead pre-BetterText EDIT/RichEdit control code, no longer instantiated anywhere since the move to BetterText-backed native text controls
- fix(ui): raise the per-image scaled-surface LRU cap from 4 to 8 on the GTK/Cairo and Qt canvas backends — a shared avatar drawn at many sizes was thrashing the cache on every repaint
- feat(windows): add MSIX packaging (Store + direct) alongside NSIS, with matching packaged-app runtime support (identity/AUMID detection, autostart, Jump List, toast notifications) and CI build/sign/validate targets
- feat(windows): integrate taskbar shell features — unread/mention overlay icons, thumbnail-toolbar controls, upload progress, and localized Jump Lists, coordinated across main/room/call windows
- ci: add deterministic cross-platform screenshot generation — a CI-only mode with a shared fixture room/timeline, light/dark capture entry points for all four shells, and a workflow building/capturing Linux under Xvfb plus a best-effort macOS job
- fix(arch): add `git` as an explicit makedepend and disable LTO for AUR builds, since Arch's default LTO breaks `ring` at link time
- feat(linux): show room/DM avatars in KRunner and GNOME Shell search results, resolved from a synchronous disk-cache lookup with the themed icon as fallback
- build: trim third-party debug info to shrink `target/` size, capping non-workspace deps to line-tables-only while keeping full debug info for our own code
- fix(linux): fix several GTK4 rendering bugs (pickers not animating without mouse movement, redundant cubic image resampling, room-list text not truncating, a stuck pagination spinner, a stale placeholder) and add `tk::single_instance` so launching both builds against the same store is detected and forwarded
- feat(macos): add Now Playing and Spotlight search integration, extending the Linux MPRIS/search work to macOS via the same shared classes
- feat(linux): add MPRIS, GNOME Shell search provider, and KRunner integration — a new headless search backend, MPRIS playback exposure for voice/audio messages, and D-Bus adapters surfacing room/DM/contact results

## v0.8.17 — 2026-07-31

### Summary

- ci: merge duplicate DEB/RPM and AppImage build steps into one Linux job, gated on independent toggles. Also fixes AppImage output missing emoji-font bundling
- feat(packaging): add a Flatpak/Flathub manifest (Qt6 variant), vendoring Rust deps and the Rust toolchain offline. Validated via a local `flatpak-builder` build and a CI validation job
- fix(macos): popup panels (`PopupMenu`, `ComboBox`/`SearchablePicker`) never fired hover events because their tracking area required key-window status, which non-activating panels never get. Fixed by using an active-in-app tracking mode
- chore(packaging): track the AUR-ready PKGBUILDs in-repo as the source of truth copied into the separate AUR git repos
- fix(packaging): correct the Arch PKGBUILD's license (MIT to GPLv3), drop an untested `aarch64` entry, remove a redundant `provides`, and fill in placeholder CPack URLs
- fix(linux): route Wayland desktop notifications through the legacy D-Bus interface instead of the XDG portal, which lacked required features on Plasma. Also unifies notification-id correlation into a shared helper and makes GTK4 always show a fresh popup like Qt6
- fix(ui): make Ctrl+C/Cmd+C copy a message-timeline text selection — the composer's native text field never released keyboard focus when a selection started, swallowing the key. Fixed to release focus to the canvas on selection start
- fix(ui): make hover/press on media-viewer chrome/transport buttons actually visible via a new opt-in `tk::Button::FillOverride`
- feat(ui): show a floating date badge pinned to the top of the message timeline while scrolled up through history
- fix(rooms): stop re-fetching 100 events from the server every time a room is revisited, not just the first time
- fix(macos): correct two wrong API calls (`SMAppService.mainAppService`, `UNTextInputNotificationAction`'s initializer) introduced by the launch-at-login and notification quick-reply features
- feat(notifications): add desktop notification quick-reply (Windows toast input, macOS, Linux KDE/portal) across all four shells
...
- fix(sdk): restore the two-token User-Agent shape so MAS names sessions correctly instead of showing "Unknown device"
- fix(ui): stop `wire_main_app_widget_()` from silently overwriting `RoomPane`'s retry/abort-send handlers with a dead copy; surface a real failure as a toast instead of doing nothing
- fix(ui): convert the media-viewer chrome/transport buttons to real `tk::Button` widgets, giving them hover/press/keyboard feedback for free
- feat(sdk): harden `matrix-sdk` init (bounded retries/timeout, auto key-backup) and encrypt new sessions' local SQLite store
- fix(ui): dismiss open popups on clicks that land in a native text field/area and bypass canvas hit-testing; wire outside-click auto-dismiss for popup surfaces on GTK/Win32/macOS (previously Qt-only)
- refactor(ui): render `PopupMenu` in a genuine OS popup surface instead of a canvas overlay so it z-orders correctly above native controls; fixes 3 bugs found in the migration
- fix(ui): hide the compose bar's native text field under pop-out-window overlays, matching the main window's existing behavior
...
- fix(ui): make `PopupMenu` always render on top via `paint_overlay()` instead of ordinary paint order, so the room-list context menu isn't painted over
- feat(ui): add a right-click context menu to room list rows (Open in tab / Open in window / Leave room)
- fix(ui): `RoomSettingsView`/`ImagePackEditorView::arrange()` no longer redundantly call the base `tk::Widget::arrange()`, which corrupted the image-pack list's scroll position on every relayout
- fix(ui): the room/space image-pack editor's shortcode/pack-name field now re-syncs against the list's live scroll offset every paint, instead of only on a full relayout
- fix(ui): give the personal image-pack editor a self-owned shortcode text field — clicking a tile's shortcode previously showed no editable field at all
- fix(ui): stretch the Subscribed Packs list to fill the Settings page's full height instead of a fixed ~200px viewport
- fix(tests): fix a Unity-build anonymous-namespace `Widget` collision between two test files
- refactor(ui): unify ~16 independent ad hoc lifetime-guard patterns onto a single `tk::EnableWeakSelf<T>` mixin; also closes 3 confirmed unguarded raw-`this`-across-async-boundary gaps
- fix(ui): skip the room-list rebuild when a presence poll reports the same state as last time, instead of rebuilding on every 60s tick
...
- fix(ui): construct `BrandView` via `create_root_widget()` instead of plain `make_unique()`, fixing a null `host()` that stalled the startup splash animation; also backgrounds two more blocking calls and caps each restored account's Tokio runtime to 2 worker threads
...
- fix(macos): force an independent sample-buffer copy in `AVAssetReaderTrackOutput`, fixing an intermittent R/B channel swap during GIF-strip playback
- refactor(ui): introduce `RoomPane`, a shared class for per-room display logic previously duplicated across `ShellBase` and `RoomWindowBase` on all four platforms; fixes ~9 real bugs surfaced along the way
- refactor(win32): remove the dead `Win32NativeTextField` class, never constructed since the BetterText migration
...
- fix(macos): stop pinning `NSApp.appearance` to a concrete value on every theme apply, which broke System-mode OS dark/light detection and live theme-change reaction; fixed on pop-out windows too
- fix(macos): migrate the main window's `/command` popup onto the shared `SlashCommandController`, closing an MSC4391 parity gap with the pop-out window
- fix(ui): capture the `BrandView` pointer before `set_root()` instead of downcasting afterward, which could yield a stale pointer and crash on the first startup-restore status update
- fix(windows): add `/bigobj` for MSVC to fix a unity-build "too many sections" error
- fix(macos): expose `ShellBase::RestoreResult` to `MacShell` via a `using` declaration so the restore-callback lambda actually resolves
- docs: make the landing page's interactive simulator and marketing sections mobile-friendly
- feat(ui): add a subtle animated spinning tesseract wireframe background in `BrandView`, backed by a new `Canvas::draw_line` primitive on all four backends
- chore(gitignore): scope-ignore Corrosion's probe-crate `Cargo.lock`, un-ignored incidentally when the root lockfile started being tracked
- fix(build,macos): tolerate `ar d` failing on a vendored x86_64 WebRTC archive missing objects the arm64 one has; fix a signature mismatch in the AVFoundation video-decode backend
- fix(qt): preserve alpha in native-menu QSS colors instead of flattening to opaque, which rendered highlighted menu items as black-on-black
- fix(startup): stop freezing the window during account restore — `BrandView` now shows a live status message and spinner instead
- fix(video): self-heal generated video thumbnails evicted from the in-memory cache instead of showing a permanent placeholder icon
- fix(video): fix client-side video-thumbnail generation, persist thumbnails to disk, implement first-frame extraction on Windows via Media Foundation, and fetch only a byte-range prefix instead of the whole video
- feat: implement MSC4391 simplified in-room bot commands — command discovery/autocomplete, a positional argument-entry flow, and structured invocation sending, across all four shells
- feat(ui): add adaptive narrow-window layout — collapse room-list/room-view below 600px, derive a real minimum window width, and collapse header actions into an overflow menu when they don't fit
- fix(ui): hide the profile fields section entirely when the server doesn't support it, instead of showing disabled inputs
- fix(linux): stop Ctrl+C from freezing the app on shutdown; harden shutdown so an in-flight send doesn't wait out the full network timeout
...
- fix(ci): point `rust-cache` at the actual Cargo workspace root instead of the `sdk` member, so the lockfile participates in the cache key
- fix(cmake,ci): strip stray protoc-plugin objects from the linked WebRTC archive; mirror the macOS Opus static-link fix to the platform build workflow
...
- fix(client,ui): stop re-fetching the whole profile after a field save, which could silently revert a sibling field
- fix(ui): defer pronoun-editor saves to blur/tab-switch/settings-close instead of every micro-edit, and fix a new row's language defaulting to a raw locale string
- fix(ui): gate the room-header calls button on the user's actual power to send call-member state, not just server/bridge status
- feat(ui): add a multi-language pronoun editor and searchable IANA timezone picker to account settings
...
- feat(ui): merge Join Room and a new Create Room feature into one shared `AddRoomView` modal with a real tabbed header
- fix(ui): stop a popup's own trigger button from dismissing then immediately reopening its popup on the same click
- fix(tk): make popup pickers resize to fit on every filter keystroke, not just incidentally via arrow-key navigation
...
- feat(tk): add trackpad momentum (kinetic) scrolling on all four platforms
...
- feat(ui): polish room list, compose bar, and timeline visuals — stronger active-room highlight, square-icon compose-bar buttons, unified chrome
...
- feat(ui): rebuild emoji/sticker pickers as in-window widgets, replacing duplicated native-popup-window code per shell; adds an eased hover/fade polish pass
...
- feat(compose): support editing Image/File/Video captions via matrix-sdk's `EditedContent::MediaCaption`, preserving the original media instead of clobbering it
- fix(packaging): declare an emoji font as a runtime dependency on Linux (.deb/.rpm/Arch)
- feat(tk): compute Tab traversal order from widget rects (top-to-bottom, left-to-right) instead of insertion order
- fix(tk): gate key-handling widgets on `has_focus()`; give the Appearance theme-picker real keyboard access
...
- fix(views): stop the recovery-key/passphrase fields in `EncryptionSetupOverlay` losing focus on every repaint; add a same-value native-forward guard to text-field/area visibility
...
- fix(main-app): scope Tab traversal to `MainAppWidget`-level overlays instead of leaking across them
...
- feat(tk): add a real keyboard-focus system, migrate native text-entry surfaces to self-positioning widgets, and default-focus the composer when nothing else needs attention
...
- feat(compose): `/location` slash command sends your current OS location as an m.location event immediately, with no confirmation step
- feat(calls): remove the `TESSERACT_ENABLE_CALLS` build flag; MatrixRTC voice/video calls are now permanent and always-on across all four platforms
- fix(tests): fix a link failure in the test executable exposed by calls always being enabled — attach calls-only platform libs to the right archive interface
- feat(settings,views,tk): add a "Developer mode" toggle to Advanced settings, gating a "Copy event source" message-menu item; consolidate three duplicated toast implementations into one
- fix(calls): stop the incoming-call banner reappearing for long-ended calls by filtering stale member events; macOS now hangs up on quit instead of leaving stale server-side state
...
- fix(cache): bound or prune six unbounded caches found in a memory-usage audit (Rust media-fetched/backoff tables, five `ShellBase` maps, per-platform animated-image frame decode, Windows emoji-bitmap cache)
- fix(tk): harden `clear_children()`/`remove_child()` against reentrant/self-destroying widget callbacks via weak tracking and deferred subtree destruction
- fix(windows): update vendored BetterText, fixing the password field's masking dot showing the wrong glyph
- feat(login): add legacy username/password login (`m.login.password`) for self-hosted homeservers without an OIDC/MAS provider, behind a new build flag (default on)
- feat(ui): dispatch file drop and drag-hover through the widget tree instead of a flat per-Surface callback, so each drop target claims and highlights its own drop zone; fixes native text fields swallowing file drags before the Surface saw them
- fix(login): hide the homeserver field's drawn border once the password form is showing
- feat(compose): show the full slash-command list on no match instead of hiding the popup, and add scrolling so commands past the old 8-row cap are reachable
...
- feat(tk): let multiple `FormLayout`s share one label-column width via a new `FormLayoutGroup`, fixing misaligned combo boxes across the Room Settings → Permissions tab
...
- fix(views): wire drag-drop into the personal image pack editor in Settings, which never had file-drop wired on its own native surface
...
- fix(windows): stop the emoji/sticker picker's shortcode tooltip from freezing every animation in the app while visible — the tooltip refresh now only repaints when its text/anchor actually changed
- fix(macos): implement inline-image box reservation in the CoreText canvas backend, fixing custom MSC2545 emoji not rendering inline in the timeline at all on macOS
- feat(image-packs): surface MSC2545 packs from any Space (direct or nested-ancestor) the current room belongs to, in the emoji/sticker pickers and shortcode popup, alongside personal/current-room/subscribed scopes
- refactor(tk): replace the hand-maintained per-shell native-field theming list with a generic `tk::Widget::apply_theme()` tree traversal, fixing several fields the old list missed entirely
- feat(tk): add a generic `tk::Host`-owned tooltip system (dwell-delay, custom-drawn) and migrate all hand-rolled hover/tooltip sites onto it, deleting the old per-platform native tooltip code
...
- feat(popout): close 13 feature-parity gaps between the main window and pop-out room windows (file save, jump-to-original, pin/unpin, edit-last-message, retry/abort send, inline video/GIF autoplay, forward-message picker, room media gallery, and more), plus routing incoming-call banners to whichever window has the room open
...
- perf(idle CPU): fixed the app burning CPU while idle with the window hidden
...
- Refactored `MainAppWidget`'s widget tree onto shared traversal/keyboard dispatch primitives
...
- fix(macos): fixed a bitwise operation between mismatched types in the screen-capture code
- fix(sdk): `backfill_room_silent()` now drains its Timeline's diff-processing stream to quiescence before dropping it
- fix(call): mute, video-mute, and screen-share state now survive switching the call overlay between Docked and Floating
...
- fix(unread): state events (e.g. another member's avatar change) can no longer produce a stray unread
...
- fix(ui): fixed a pointer-move regression from the widget-routing refactor above
...
- fix(packaging): Arch PKGBUILD no longer fails with "cannot stat" errors, by dropping the redundant manual `mv`/`sed` rename steps
...
- fix(gtk4): make the GTK4 shell usable at all — three fixes to a shell that aborted on startup so none of its paths had ever run: the tray drops `libayatana-appindicator3` (which pulled libgtk-3 into the process and made `gtk_init()` abort) for a pure `org.kde.StatusNotifierItem` + `com.canonical.dbusmenu` implementation over GDBus; the user-strip right-click menu now hit-tests the lower-left strip instead of only sticker rects, so Settings / Add-Account / Log-Out / Quit fires; and image send flattens onto opaque white before JPEG encoding, since the compress path failed on any pixbuf with an alpha channel.
...
- perf(anim): stop full-window repaints for animated images — animated GIF/sticker/emoji images drove the 60 Hz frame timer to repaint the whole surface forever, because `anim_cache_` is never evicted and the timer only stopped when it was empty. `AnimImageCache` gains a visibility-gated timer (`any_visible()` lets each shell stop it once nothing animated is on screen) plus partial repaints via a new `PaintCtx::AnimDamageSink` that Qt6/macOS use to invalidate just the animated rects (`update(QRect)` / `setNeedsDisplayInRect:`); GTK4 drops the wasteful per-frame relayout instead, and Win32's D2D present is unchanged.
...
- fix(macos): unblock the x86_64 build broken by the stale CommandLineTools libc++ — cxx 1.0.194's `cxx.h` unconditionally includes `<ranges>` under C++20 (absent from CommandLineTools libc++ v8000), fixed by substituting the macOS SDK's modern libc++ via `-nostdinc++` + `-isystem` for CXX and OBJCXX; the newer libc++ also made `path& operator=(string_type&&)` ambiguous at six `pending_login_temp_dir_ = {}` sites (replaced with `.clear()`), needed the missing `-framework UniformTypeIdentifiers` link, and required capturing `send_message`'s `tesseract::Result` as `auto` to match the other shells.
...
- feat(presence): publish outgoing Matrix presence, pairing with the receive-side dots shipped in `d652c95` — a new shared `PresenceTracker` runs a deadline-driven FSM (Online while engaging with the app, Unavailable after 5 min of no input + no window focus, Offline on logout), wired per shell via pointer/wheel activity, a focus tap (`changeEvent`/`notify::is-active`/`WM_ACTIVATE`/`windowDidBecomeKey,Resign`), a 30s tick timer, and a pre-logout hook. Transitions publish via `Client::set_presence(PresenceState)` → `set_presence` FFI → the Matrix presence endpoint on a detached worker thread. 114 Rust tests + 411 C++ tests
