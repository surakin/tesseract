...
- polish(rooms): Create Room's "Creating…" state now hides the whole form and shows a centered spinner instead of an inline label that pushed the buttons down; the status/error row's height is now always reserved so the Create/Cancel buttons never move when an error appears. Windows build; user-verified live
