...
- fix(threads): sending a sticker into an open thread no longer lands it in the room timeline. The main window's four shells wired the thread panel's callbacks (open/close/send) straight to `ShellBase`'s own, separate thread-panel state machine — a leftover from before that logic moved to `RoomPane` — silently overriding `RoomPane`'s equivalent wiring, so `RoomPane`'s own panel state (which `send_sticker_()`'s thread check reads) was never updated for the main window. Removed `ShellBase`'s duplicate implementation entirely (state, subscribe/pagination side effects, and the five `on_thread_*`/`on_threads_button_clicked` entry points); `RoomPane` is now the single source of truth for both the main window and pop-outs, with `ShellBase`'s remaining thread-aware code (event routing, find-in-thread search, backfill pagination) reading through it. Also makes the Rust sticker-send path set the `m.thread` relation explicitly instead of relying on an SDK auto-tag heuristic. Linux debug + release build, 1719/1719 ctest (incl. a rewritten `test_shell_thread_panel.cpp` exercising the real `ShellBase`↔`RoomPane` integration), 643/643 cargo test; user-verified live
- fix(threads): a sticker sent as a thread reply (or as the message that starts a thread) now shows up in the thread view at all — it previously never appeared anywhere, room or thread. `m.sticker` events convert through a separate code path from `m.room.message` (image/video/audio/file all share the message path and were unaffected) that never extracted MSC3440 thread metadata, so every incoming sticker always got an empty thread root regardless of its actual `m.relates_to`. User-verified live against Synapse + element-web

## v0.8.20 — 2026-09-04

### Summary

- fix(threads): the per-thread unread dot now survives an app restart. `send_thread_read_receipt` now threads the acked reply id through the FFI instead of re-deriving the target from the (often-unsubscribed) `ThreadListService`, and read markers now persist to a new `thread_read_state.db` since Synapse doesn't echo threaded receipts back on the live sliding-sync path. Read state is per-device: the local store isn't synced or cleared across your other devices. Linux (Qt6 + GTK4) build + full ctest; macOS/Windows share the code, unbuilt. +7 Rust tests
- fix(tk): a disabled widget is now opaque to input — pointer, key, and drop dispatch (plus hit-test and Tab traversal) stop at a disabled widget instead of relying on its handler to reject the event, so clicks/hover/drops no longer fall through to what's behind or inside it. Clicking a disabled widget no longer clears keyboard focus. Linux (Qt6 + GTK4) build + full ctest; macOS/Windows share the code, unbuilt. +6 C++ tests
- feat(macos): the application menu bar now has an entry for every app-level keyboard-shortcut action (Settings…, Add Room…, Find/Search Your Messages, Quick Switcher, room navigation), previously only reachable via an invisible `NSEvent` monitor. es.po updated. macOS build-verified, user-confirmed working
- fix(win32): the accessibility bridge no longer crashes on shutdown — `UiaDisconnectProvider` can pump a reentrant `WM_GETOBJECT` into the still-live window, which could reach back into the bridge mid-teardown and invalidate its provider map. Registry entry is dropped before `detach()` runs, and `detach()` iterates a moved-out copy of the map as a second guard. Windows build-verified; original crash was an intermittent teardown race, not reliably reproducible on demand — user is monitoring post-release rather than blocking on a forced repro
- feat(tk): scrolling the mouse wheel over a closed `ComboBox` cycles the selected option, matching native Win32/Qt/GTK combobox behavior. No-op while expanded, disabled, or empty. Windows build-verified, user-confirmed working; user-confirmed on Qt6, GTK4, and macOS too
- fix(ui): bubble layout no longer reserves blank space under a portrait sticker. The row-height (measure) pass sized the sticker against the full shaping width while the paint pass used the hugged bubble-column width, and the sticker branch used that width as *both* the width and height cap — so a taller-than-wide sticker drew shorter than its reserved row, leaving ~100px of empty "caption" space below it. All three passes now go through one `sticker_fit_box_()` helper whose height cap is always the constant `kStickerSize`. Linux (Qt6 + GTK4) build + full ctest. +1 C++ test
- fix(ui): bubble layout — a bubble now hugs its content with no minimum width, so a one-word own message ("ok") sits flush to the right edge instead of left-aligned inside a wider-than-needed bubble. Dropped the 48px `kBubbleMinW` floor from the bubble content-width and natural-width clamps (kept only as the absurdly-narrow-row floor in `shaping_width`); replies keep their `kQuoteMinW` floor and reaction/thread furniture sizes itself. Linux (Qt6 + GTK4) build + full ctest. +1 C++ test
- fix(tk): widget visibility now cascades — `Widget::visible()` is `visible_ && parent_visible_` (cached), so a widget under a hidden ancestor reports hidden. `tk::TextField`/`TextArea` hide their native OS control via a new `on_effective_visibility_changed_()` hook, fixing the thread-list and thread-view search fields that stayed painted after the panel closed. Adds `own_visible()`; `visible_in_tree()` kept as a synonym. Linux (Qt6 + GTK4) build + full ctest. +4 C++ tests
- feat(ui): Appearance → Layout shows a live preview beside the message-layout combo box — two sample messages rendered through the real Classic/Bubbles/IRC row renderer, updating as the selection changes. New inert `MessageLayoutPreview` widget (a real `MessageListView`, no live room). Windows-verified; user-verified on Qt6, GTK4, and macOS too
- feat(threads): thread-list panel header gains a "mark all threads as read" button (Lucide `list-checks`, left of the close button), disabled when nothing is unread. New batched `Client::mark_all_threads_read(room_id)` FFI sends MSC3771 threaded receipts for every unread thread at once. Linux (Qt6 + GTK4) build + full ctest; user-verified on macOS/Windows too. +2 C++ tests
- feat(threads): unread-thread indicator — a dot on the room-header threads button (and its overflow menu), accent-coloured when an unread reply mentions you, plus a per-row dot in the thread-list panel. Read state comes from MSC3771 threaded read receipts, which Tesseract now also sends: the open thread panel marks its thread read as it's viewed (`send_thread_read_receipt` FFI + optimistic local marker). New `ThreadInfo::unread` / `mentions_me`, `thread_is_unread` / `aggregate_threads` helpers. v1 scope: latest-reply `m.mentions` only; receipt-send-time vs reply-timestamp fallback; a cross-device read clears the local dot only on the next reply. Linux (Qt6 + GTK4) build + full ctest; user-verified on macOS/Windows too. +6 C++ / +12 Rust tests
- fix(join): the Join / "Request to Join" button (Add Room → Join tab, and `matrix.to` / `matrix:` permalink joins) is disabled for the duration of the async join, so a double click can't fire a duplicate `join_room_async` / `knock_room_async`. Preview card stays visible with a "Joining room…" / "Sending request…" status line. +3 C++ tests
- refactor(settings): the ~20 identical SettingsView callbacks (persist to `Settings::instance()` or forward to an existing handler) and the avatar/profile-field controller wiring, previously duplicated byte-for-byte across all four shells, now live once in `ShellBase::wire_settings_view_()` / `wire_settings_controller_common_()`. Shells keep only the genuinely platform-specific parts (dismissing the surface, native dialogs, device enumeration, relayout); Qt6/GTK4 `SettingsWidget` shed the now-redundant pass-through layers (−1153 lines net). Windows build-verified; user-verified on Linux and macOS too
- fix(join): joins now route through via servers, so a Space child room (or permalink target) on another homeserver joins instead of failing with "join failed or cancelled". New `resolve_route_via()` gathers routing servers in priority order — permalink `?via=`, the Space's `m.space.child` list, then the room-id/alias domain (local state only) — threaded through `join_room` / `knock_room` / `get_room_summary` and the cxx bridge; `join_room_async` now surfaces the homeserver's real error. User-verified end-to-end
- fix(freeze): the known-users roster sweep no longer freezes the client on a stalled member fetch — `get_room_members` reads the store (`room.members_no_sync()`) instead of firing an untimed `room.members()` request that could sit in `block_on` holding the shared FFI lock. `poll_presence_now` drops to a shared lock too. Built + tested on the presence/room-list/roster subset; original freeze was an intermittent stalled-fetch timing issue, not reliably reproducible on demand — user is monitoring post-release rather than blocking on a forced repro
- feat(power): low power mode — Settings → General → Power: `Auto` (default) / `On` / `Off`. When active the client runs only the sliding-sync long-poll: background backfill, unread/favorite prefetch, bridge-status checks, the decoded-image GC timer, the search-index crawl and warm-check auto-pagination all halt (one new `Client::set_low_power_mode` FFI flag). Message sync, encryption sync and user-driven pagination are untouched. `Auto` = `on-battery || OS-energy-saver`, ~20 s debounce. Batteryless machines hide the feature entirely (`IPowerMonitor::has_battery()`). New `tesseract::IPowerMonitor` DI interface with per-platform probes (UPower + power-profiles-daemon / `GetSystemPowerStatus` / IOKit `IOPowerSources`), a `PowerPolicy` pref+signals resolver, and a status-bar glyph. Linux (Qt6 + GTK4) build + full ctest; user-verified on Windows and macOS too. +9 C++ / +1 Rust tests
...
- feat(ui): IRC layout shows a reply's context as a dim `<sender> original message` scrollback line above the reply (wraps in full, flush to the column edge, click-to-jump preserved) instead of the framed quote card. Classic/Bubbles keep the card. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows too. +2 C++ tests
- fix(ui): a short grouped message is no longer padded up to the 28px hover-action-pill height — the pill rests flush on the row's bottom edge and overflows upward into the row above instead. While the pointer is over that overflow the hovered row no longer changes, so the pill stays put and its overflowing part stays clickable. Linux-verified. +4 C++ tests
- perf(settings): opening the Settings view no longer blocks the UI thread — the search-index stats read (a `dbstat` walk under the indexer's lock) and the "launch at login" OS query now run on the worker pool and push their results in when they return. macOS-verified; other shells share the code, unbuilt
- feat(ui): third message layout — `IRC` in Appearance → Layout. A monospaced mIRC look-alike: one flat left-aligned column, every line `[HH:MM] <nick> message` (wrapped lines flush left), a colour per person from the classic 16-colour mIRC palette, `* nick` for `/me`, `-nick-` for notices, and join/part/quit + day/read-marker rows restyled as IRC system lines. Adds `bool tk::TextStyle::monospace`, honoured by all four canvas backends. Applied live, no restart. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows too. +6 C++ tests
- fix(ui): compose bar's Send button now shows a Lucide send-horizontal icon instead of the "Send" text label, and is a square button like the other compose-bar icons instead of a wide pill. Same accent-pill fill, enable/disable behavior, and accessible name as before. `Button::paint()` can now self-paint an icon on any variant, not just `Icon`. Linux (Qt6 + GTK4) build; user-verified visually
- fix(tk): `Widget::set_enabled()` repaints its own bounds on a real change, so a widget toggled from a scoped-repaint-only event (e.g. the compose bar's Send button) no longer stays visually stale until an unrelated full repaint. Linux (Qt6 + GTK4) build + full ctest. +1 C++ test
- feat(ui): message layout is now a choice — Appearance → Layout combobox, `Classic` (default) or `Bubbles`. Bubbles right-aligns your own messages in a subtle rounded bubble (avatar dropped) and gives other messages a faint bubble too. Applied live, no restart. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows too. +14 C++ tests
- fix(ui): bubble layout — the sender name is no longer clipped to the (often narrow) hugged bubble width, and a message containing a mention (or any rich formatting) no longer forces the bubble to full width. The Qt rich-text layout's `measure().w` returned the wrap constraint rather than the content width (`QTextDocument::size()` after `setTextWidth()`); it now returns `idealWidth()`, matching the Pango/DirectWrite/CoreText backends. +2 C++ tests
- fix(ui): the main window, room pop-outs, and call windows now enforce a minimum size (previously only the main window did, width-only), so they can no longer be resized down to unusable dimensions. Windows-verified; user-verified on macOS/Qt6/GTK4
...
- fix(qt6): an update-available notification no longer blanks the status bar — `refreshSyncStatus()` was hiding the hyperlink label and returning early past the code that re-shows it
- feat(ui): the reply-quote card renders the replied-to message's inline formatting (bold, italic, code, links, strikethrough) instead of its raw markdown-ish plain body. Thread-preview snippets are unchanged for now. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows. +2 C++ tests
- feat(ui): Markdown tables render as a column-aligned grid — cell borders, per-column width sizing, a tinted and ruled header row, and honored column alignment (`|:--|`, `|:-:|`, `|--:|`). Wide tables shrink, then wrap, then clip to the message width. Text selection works inside a cell (byte range) and across cells (a rectangular block of whole cells); copy yields plain text for one cell or tab/newline TSV for a block. The HTML sanitizer now passes `text-align` through on `<td>`/`<th>` (canonicalized, no other CSS); block-level content inside a cell (`<ul>`, nested `<p>` — allowed by the Matrix HTML subset) flows inline rather than corrupting the grid. Linux (Qt6 + GTK4) verified; user-verified on macOS/Windows. +11 Rust / +29 C++ tests
- fix(tk): native text controls (`tk::TextField` / `tk::TextArea`) inset 2px inside their border box by default, so the opaque native capture no longer paints over the border or squares off rounded corners on Qt6. Fixes global search, the emoji/sticker pickers, and ~20 other views. Linux-verified. +2 C++ tests
...
- build(flatpak): regenerate `cargo-sources.json` for the current lockfile, add the 0.8.17 / 0.8.18 metainfo releases, reframe the packaging README around self-hosted publishing, and add a PR-triggered `flatpak.yml` CI job that builds the manifest offline. Manifest build tested and works; the update path itself (old → new version) still needs a second published version to test against
- perf(build): Linux builds use jemalloc as the process-wide allocator (interposing `malloc`/`free` for the C++ side too), tuned to return freed pages to the OS. Addresses RSS climbing far past the live heap under long media-heavy sessions. Gated on `TESSERACT_ENABLE_JEMALLOC` (default on, Linux only). Linux-verified; RSS confirmed being released back to the OS as expected in long sessions
- feat(media): the room media gallery is backed by a persistent per-room index (`room_media` in `app_cache.db`) instead of re-scanning the SDK timeline on every open. It seeds once from the SDK's event-cache store (no network) and stays current from the timeline diff stream; the gallery paints from SQLite and only hits the network once the index is drained. The "Media (N)" badge counts all synced history. Image + video only. Also fixes gallery thumbnails that never loaded when not already cached by the chat timeline. Linux-verified; user-verified on macOS/Windows. +9 Rust / +3 C++ tests
- perf(media): the decoded-image caches (avatars, thumbnails, stickers) are garbage-collected to the on-screen working set via a generational mark-and-sweep gated on recent user input, replacing indefinite per-row pinning and a fixed TTL. A new 32 MB in-RAM compressed-bytes tier re-decodes a dropped image in ~2 ms on scroll-back. Heaptrack had the decoded-bitmap heap growing unbounded to ~200 MB over 80 s; it now tracks the visible set. Adds `tk::IntervalTimer`. Qt6-verified; user-verified on GTK4/macOS/Windows. +17 C++ tests
- fix(settings): "Clear all caches" now wipes the account's state, event-cache and media stores, closes tabs and pop-outs, and re-fetches the room list from a fresh sync — keeping the crypto store and session so you stay logged in and verified. Refuses while a call or verification is in progress. User-verified on Linux, macOS, Windows, GTK4
- fix(tk): `ConfirmDialog` no longer clips body text past ~4 lines — the card sizes to the wrapped text height
...
- fix(search): the search index's one-time history backfill restarts after "Clear all caches" — the indexing gate was left enabled, so only live indexing resumed and old history stayed un-searchable until the next launch
- fix(tk): `Label::set_text` / `set_role` / `set_wrap` / `set_trim` / `set_halign` schedule a coalesced relayout, so text changes take effect without waiting for an unrelated repaint
- fix(settings): the indexed-size figure ("· ~N MB") shown while indexing is re-measured every poll tick instead of frozen at its open-time value
