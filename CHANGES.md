...
- fix(tk): native text controls (`tk::TextField` / `tk::TextArea`) now inset 2px inside their border box by default. The native capture composites opaque, so on Qt6 the field's background painted over the border, squared off its rounded corners, and ran text flush to the edge — the one mitigation (`set_overlay_inset`) was only ever called from the Windows shell. `overlay_inset_` now defaults to 2px on both widgets (matching `RoomListView`'s old workaround), is added to `TextArea`, and is clamped against underflow; the redundant per-shell and per-view calls are dropped, and `ComposeBar` opts back out since it already pads and clips. Fixes global search, the emoji/sticker pickers, and ~20 other views. Shared code, Linux build + full test suite verified. +2 C++ tests
- fix(windows): the DirectComposition-backed login surface painted over the custom title-bar strip — `LoginView::layout()` repositioned its HWND back to `(0,0)` on every call, undoing the `(0, TITLEBAR_H)` offset `MainWindow::on_size()` had just applied. Unbuilt (written from a Linux session)
- fix(windows): the native text control's own D3D device (separate from the `canvas_d2d.cpp` backend) only reset on `DEVICE_REMOVED` / `DEVICE_RESET`, missing `DEVICE_HUNG` — the common TDR case on hybrid-graphics laptops switching between integrated and discrete GPUs — so BetterText's device context could silently go dead with no crash and no visible error. Its device-loss check now matches `canvas_d2d.cpp`'s `is_device_lost_hr()`, plus a debug trace for the HRESULT if it recurs. Unbuilt (written from a Linux session)
- build(flatpak): regenerate `cargo-sources.json` against the current lockfile (matrix-sdk is now the `surakin/matrix-rust-sdk` fork at `1c44fb66` plus the jemalloc allocator crates), add the 0.8.17 / 0.8.18 metainfo `<release>` entries, reframe the packaging README around self-hosted-repo publishing (Flathub's policy bars AI-assisted code), and add a new PR-triggered `flatpak.yml` CI job that builds the manifest offline and validates AppStream + the desktop file

### 2026-08-30

- perf(build): Linux builds now use jemalloc as the process-wide allocator. The Rust SDK links it as a `#[global_allocator]`, built `unprefixed` so it also interposes `malloc`/`free` for the C++ side (Qt/GTK, bundled SQLite, GStreamer), and configured `background_thread:true,dirty_decay_ms:5000,muzzy_decay_ms:5000` so freed pages go back to the OS. Addresses RSS climbing far past the live heap (~1 GB observed vs ~266 MB peak heap in heaptrack) under long media-heavy sessions — stock glibc malloc retains freed memory per-arena and this app runs many threads. Gated on `TESSERACT_ENABLE_JEMALLOC` (default `ON`, Linux only; inert on macOS/Windows). Linux build + full test suite verified; live RSS impact pending user measurement
- feat(media): the room media gallery is backed by a persistent per-room index (`room_media` in `app_cache.db`) instead of re-scanning the SDK timeline on every open. The first open of a room's gallery seeds the index from the SDK's own event-cache store (no network); after that the timeline diff stream keeps it current — image/video events upsert as they flow past, redactions delete. Opening the gallery now paints the newest page instantly from SQLite and pages older history from the index, only falling through to network back-pagination once the index is drained. Once a room's index has been seeded (first gallery open), the Room Info panel's "Media (N)" badge shows the index's exact `COUNT(*)` — read fresh each time the panel opens — instead of a linear scan of the loaded timeline window. Image + video only. Along the way, fixed a latent bug the old (timeline-seeded) gallery masked: its thumbnail fetches run under a salted media group that `fetch_media_pipeline_`'s `should_deliver_` gate rejected, so any preview not already cached by the chat timeline silently never loaded — `ShellBase` now tracks open galleries' groups; a plain-source video with no embedded thumbnail falls back to a server-generated thumbnail instead of the timeline-only `"thumb::"` sentinel. Shared code, Linux build + full test suite verified; other shells benefit unchanged but unverified. +9 Rust / +3 C++ tests
- perf(media): the decoded-image caches (avatars, inline thumbnails, stickers) are now garbage-collected to what's actually on screen instead of held indefinitely. A generational mark-and-sweep — every `peek()` marks its entry live; a ~2 s cycle, gated on recent user input so an idle window's images are frozen, forces a full repaint of every surface then evicts anything that has been off-screen for ~4 s **and** untouched for ~30 s — replaces the old fixed 30 s / 30 min TTL sweep and the per-row `ImageRef` pinning that kept every loaded timeline row's image resident for the life of the room. A new 32 MB in-RAM compressed-bytes tier (`tk::CompressedImageCache`) sits between the decoded caches and the on-disk media cache, so an image dropped by the GC re-decodes in ~2 ms on scroll-back with no disk read or network round-trip; every `media_disk_cache_` load/store now routes through it. Heaptrack on Qt6 had the decoded-bitmap heap climbing unbounded to ~200 MB over an 80 s session; it now follows the visible working set. Also adds `tk::IntervalTimer`, a shared backend-agnostic repeating timer. Shared code, Qt6-verified (idle-room retention + room media grid); GTK4/macOS/Windows benefit unchanged but unverified. +17 C++ tests
...
- fix(search): after "Clear all caches" the search index's one-time history backfill never restarted — `clear_caches` deleted `search_index.db` but left the SDK's indexing gate `true`, so the post-restore `set_search_indexing_enabled(true)` was a no-op and only live indexing resumed; history stayed un-searchable until the next app launch. `clear_caches` now resets the gate, `restart_sdk_begin_` re-applies the preference after the in-place restore, and `spawn_index_backfill` waits (bounded) for the room list to repopulate rather than marking an empty pass complete
- fix(tk): `Label::set_text` (and `set_role`/`set_wrap`/`set_trim`/`set_halign`) now schedule a coalesced relayout — a text change alters `measure()`, but nothing was dirtying the surface, so e.g. the Settings search-index progress line sat stale between poll ticks until an unrelated repaint
- fix(settings): the search-index size ("· ~N MB") shown while indexing froze at whatever it was when the panel opened — `cached_index_bytes_` was measured once and never refreshed, and the poll stops the instant the backfill finishes. It's now re-measured on every poll tick, so the figure grows during the crawl and lands on the correct final value
- i18n: wrap the previously-bare strings in the Settings → Privacy → Search section (`PrivacySection`), and switch the index-stats line from sentence concatenation to `tk::trf`; month names in `month_year` now go through `tk::tr` (matching `MessageListView`). New `es.po` / `pseudo.po` entries added, including the twelve month names
- docs: three code comments called the FTS search index `app_cache.db`; it lives in its own `search_index.db` (`bridge.rs`, `client/mod.rs`, `settings.h`)
