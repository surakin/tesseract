...
static bool has_formatting_tags(const std::string& html)
{
    ...
    {
        if (html.find(kTags[i]) != std::string::npos)
        {
            return true;
        }
    }
...
static void parse_inline(std::string_view text, std::string& out, int depth = 0)
{
    ...
    while (i < n)
    {
        ...
        if (c == '_' && i + 1 < n && text[i + 1] == '_')
        {
            ...
            if (left_ok)
            {
                ...
                {
                    bool right_ok =
                        (j + 2 >= n || !std::isalnum(static_cast<unsigned char>(
                                           text[j + 2])));
                    if (right_ok)
                    {
                        out += "<strong>";
        ...
        if (c == '_')
        {
            ...
            if (left_ok)
            {
                ...
                {
                    bool right_ok =
                        (j + 1 >= n || !std::isalnum(static_cast<unsigned char>(
                                           text[j + 1])));
                    if (right_ok)
                    {
                        out += "<em>";
        ...
        if (c == '[')
        {
            ...
            if (close_br != std::string_view::npos && close_br + 1 < n &&
                text[close_br + 1] == '(')
            {
                ...
                {
                    std::string_view url =
                        text.substr(url_start, url_end - url_start);
                                            ...
                                            url.substr(0, 8) == "https://"))
                    {
                        ...
                        out += "\">";
                        parse_inline(text.substr(i + 1, close_br - (i + 1)),
                                     out, depth + 1);
                        out += "</a>";
                        i = url_end + 1;
                        continue;
...
MarkdownResult markdown_to_html(std::string_view text)
{
    ...
    auto close_block = [&]()
    {
        switch (mode)
        {
        case BlockMode::Paragraph:
            html += "</p>";
            break;
        case BlockMode::Blockquote:
            html += "</blockquote>";
            break;
        case BlockMode::UL:
            html += "</li></ul>";
