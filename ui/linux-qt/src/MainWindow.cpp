    ...
    tesseract::EventList snapshot)
{
    if (room_id == current_room_id_)
    {
        ...
        if (mainApp_ && mainApp_->room_view()->message_list())
        {
            ...
            mainApp_->room_view()->message_list()->set_historical_mode(
                pstate.is_focused);
            if (pstate.is_focused)
            {
                mainApp_->room_view()->message_list()->scroll_to_event_id(
                    pstate.focus_event_id);
            }
    ...
    std::unique_ptr<tesseract::Event> ev)
{
    ...
    if (room_id == current_room_id_)
    {
        ...
        if (mainApp_)
        {
            mainApp_->room_view()->insert_message(
    ...
    std::unique_ptr<tesseract::Event> ev)
{
    ...
    if (room_id == current_room_id_)
    {
        ...
        if (mainApp_)
        {
            mainApp_->room_view()->update_message(
                index, tesseract::views::make_row_data(*ev, my_user_id_));
        }
        if (mainAppSurface_)
        {
