    ...
    : QMainWindow(parent)
{
...

    {
        ...
        mainApp_->room_list_view()->on_search_clear = [this] {
            if (roomSearchField_) roomSearchField_->set_text("");
            roomSearchPendingText_.clear();
            if (mainApp_) mainApp_->room_list_view()->set_search_text("");
            refreshRoomList();
        };
                    ...
                    runOnPool_([this, url]() {
                        ...
                        QMetaObject::invokeMethod(this,
                            [this, url, bytes = std::move(bytes)]() mutable {
                                ...
                                if (mainAppSurface_) mainAppSurface_->update();
                            }, Qt::QueuedConnection);
                    });
                }
            };
    ...
    roomSearchField_->set_on_changed([this](const std::string& s) {
        ...
        if (mainApp_) mainApp_->room_list_view()->set_search_text(s);
        refreshRoomList();
    });
    ...
    emojiPicker_->onSelected = [this](const QString& glyph) {
        ...
        if (mainApp_) mainApp_->room_view()->set_current_text(roomTextArea_->text());
        roomTextArea_->set_focused(true);
        client_->recent_emoji_bump(glyph.toStdString());
    };
...
void MainWindow::doLogin() {
    ...
    if (idx.user_ids.empty()) {
        ...
        loginView_->set_on_begin_oauth([this] {
            if (!pending_login_temp_dir_.empty()) return;
...
void MainWindow::onSendClicked() {
    if (mainApp_) mainApp_->room_view()->compose_bar()->trigger_send();
}
...
void MainWindow::onRoomSelected(const std::string& room_id) {
    ...
    for (const auto& r : rooms_)
        if (r.id == current_room_id_) { if (mainApp_) mainApp_->room_view()->set_room(r); break; }
...
void MainWindow::clearMessages() {
    if (mainApp_) { mainApp_->room_view()->clear_room(); mainApp_->room_view()->set_messages({}); }
}
...
void MainWindow::on_url_preview_ready_(const std::string& url,
    ...
    if (mainApp_) mainApp_->room_view()->notify_url_preview_ready(url);
    if (mainAppSurface_) {
        mainAppSurface_->relayout();
        mainAppSurface_->update();
    }
...
void MainWindow::refreshRoomList() {
    ...
    if (!client_) {
        showRooms({});
        if (mainApp_) mainApp_->set_space_nav(false);
        return;
    }
    if (space_stack_.empty()) {
        ...
        if (mainApp_) {
            ...
            for (const auto& r : rooms_)
                if (r.id == space_id) { space_name = r.name; break; }
...
void MainWindow::maybeShowRecoveryBanner() {
    ...
    if (verification_banner_dismissed_ == false && mainApp_->verif_banner()->visible()) return;
...
void MainWindow::onUserStripContextMenu(const QPoint& global_pos) {
    ...
    QObject::connect(menu, &QMenu::triggered, this, [this, addAct, logoutAct](QAction* a) {
        if      (a == addAct)    beginAddAccount();
        else if (a == logoutAct) logoutActiveAccount();
    });
...
{
    if (mainApp_) mainApp_->room_view()->set_typing_text(text);
}
...
void MainWindow::logoutActiveAccount() {
    ...
    refreshRoomList();
    clearMessages();
    if (mainApp_) { mainApp_->show_recovery_banner(false); mainAppSurface_->relayout(); }
