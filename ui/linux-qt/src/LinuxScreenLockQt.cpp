...
LinuxScreenLockQt::LinuxScreenLockQt()
{
    ...
    bus.connect(kService, session_path_, kSessIf, "Lock",
                this, SLOT(onLock()));
    bus.connect(kService, session_path_, kSessIf, "Unlock",
                this, SLOT(onUnlock()));
}
...
void LinuxScreenLockQt::refresh_locked_hint()
{
    ...
    QDBusReply<QVariant> r =
        props.call("Get", QString::fromLatin1(kSessIf),
                   QStringLiteral("LockedHint"));
    if (r.isValid()) locked_ = r.value().toBool();
}
