...
constexpr const char* kLabelNotif =
...
{
    paint_row(ctx, 0, bounds_.y,
              checked_,          kLabelNotif,    label_layout_);
    paint_row(ctx, 1, bounds_.y + kRowH,
              previews_checked_, kLabelPreviews, previews_label_layout_);
}
...
int NotificationsSection::row_at(tk::Point local) const
{
    ...
    if (local.x < kPadX || local.x >= bounds_.w - kPadX) return -1;
    if (local.y < 0 || local.y >= kRowH * 2.0f)          return -1;
...
void NotificationsSection::on_pointer_up(tk::Point local, bool inside_self)
{
    ...
    if (row_at(local) != was_pressed)    { return; }
