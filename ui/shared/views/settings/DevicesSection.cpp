...
// ---------------------------------------------------------------------------
// Scroll support: override SettingsPage's layout so the section can host
// more device rows than fit in the viewport. The base VBox layout knows how
// to stack children; we ask it to lay out at the content's natural height
// (which may be taller than the viewport), then clip painting to the
// section's bounds. Wheel events adjust scroll_y_ within [0, max].
// ---------------------------------------------------------------------------

tk::Size DevicesSection::measure(tk::LayoutCtx&, tk::Size constraints)
{
    // Fill whatever the host gives us; SettingsPage is the inner widget of
    // a SideTabView, which always feeds us a fixed viewport.
    return constraints;
}

void DevicesSection::arrange(tk::LayoutCtx& ctx, tk::Rect bounds)
{
    // Probe the natural content height by asking the base measure with an
    // unbounded vertical constraint.
    tk::Size natural = SettingsPage::measure(ctx, {bounds.w, 1.0e6f});
    ...
    // Arrange the base at the natural height, shifted up by scroll_y_.
    // The base will set each child's bounds_ in world coords accordingly,
    // so off-screen rows just sit outside the section's bounds — paint
    // clips them away and pointer dispatch's contains_world filters them.
    const float laid_h = std::max(bounds.h, content_height_);
    SettingsPage::arrange(ctx,
                          {bounds.x, bounds.y - scroll_y_, bounds.w, laid_h});

    // FlexBox::arrange wrote `this->bounds_` to the laid-out rect (which is
...
void DevicesSection::paint(tk::PaintCtx& ctx)
{
    ...
    // CanvasFactory + Theme so the rows land at the current scroll offset
    ...
    SettingsPage::arrange(
        lc, {viewport.x, viewport.y - scroll_y_, viewport.w, laid_h});
    // SettingsPage::arrange overwrote our viewport bounds — restore.
    bounds_ = viewport;
...
bool DevicesSection::on_wheel(tk::Point /*local*/, float /*dx*/, float dy)
{
    if (content_height_ <= bounds_.h)
        return false;
    const float max_scroll = content_height_ - bounds_.h;
