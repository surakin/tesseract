...
//
// Two data sources feed the cards, resolved by cards_for():
//   * MSC4095 bundled previews carried inline on the event
//     (MessageRowData::bundled_previews) — the sender generated these; render
//     them directly. A bundled entry that carries only a matched_url (no
//     content) is resolved through the PreviewProvider like the legacy path.
...
public:
    ...
    // which the caller adds). 0 when there is no card.
    float stack_height(const MessageRowData& row) const;
...
private:
    void paint_one_(const std::string& target_url, const std::string& fallback_url,
                    const UrlPreviewData& p, tk::PaintCtx& ctx, float x, float y,
                    float col_w);
