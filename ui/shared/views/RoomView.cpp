...
void RoomView::wire_internal_callbacks() {
    ...
    message_list_->on_reply_requested =
    ...
    compose_bar_->on_send_file =
        [this](std::vector<std::uint8_t> bytes,
            ...
            if (on_send_file)
                on_send_file(std::move(bytes), std::move(mime),
                             std::move(filename), std::move(caption),
                             std::move(reply_id));
        };
    ...
    message_list_->on_reaction_toggled =
        [this](const std::string& event_id, const std::string& emoji) {
            if (on_reaction_toggled) on_reaction_toggled(event_id, emoji);
        };
    ...
    message_list_->on_link_clicked =
        [this](const std::string& url) {
            if (on_link_clicked) on_link_clicked(url);
        };
    message_list_->on_link_hovered =
        [this](const std::string& url) {
...
void RoomView::set_avatar_provider(MessageListView::ImageProvider p) {
    ...
    if (header_) header_->set_avatar_provider(p);
    if (message_list_) message_list_->set_avatar_provider(std::move(p));
}

void RoomView::set_image_provider(MessageListView::ImageProvider p) {
    if (message_list_) message_list_->set_image_provider(std::move(p));
}

void RoomView::set_preview_provider(MessageListView::PreviewProvider p) {
    if (message_list_) message_list_->set_preview_provider(std::move(p));
}

void RoomView::set_audio_player(std::unique_ptr<tk::AudioPlayer> player) {
    if (message_list_) message_list_->set_audio_player(std::move(player));
}

void RoomView::set_voice_bytes_provider(MessageListView::VoiceBytesProvider p) {
    if (message_list_) message_list_->set_voice_bytes_provider(std::move(p));
}
...
void RoomView::insert_message(std::size_t index, MessageRowData msg) {
    if (message_list_) message_list_->insert_message(index, std::move(msg));
}

void RoomView::update_message(std::size_t index, MessageRowData msg) {
    if (message_list_) message_list_->update_message(index, std::move(msg));
}

void RoomView::remove_message(std::size_t index) {
    if (message_list_) message_list_->remove_message(index);
}

void RoomView::append_message(MessageRowData msg) {
    if (message_list_) message_list_->append_message(std::move(msg));
}

void RoomView::notify_image_ready(const std::string& url) {
    if (message_list_) message_list_->notify_image_ready(url);
}

void RoomView::notify_url_preview_ready(const std::string& url) {
    if (message_list_) message_list_->notify_url_preview_ready(url);
}
...
void RoomView::set_text_area_natural_height(float h) {
    if (compose_bar_) compose_bar_->set_text_area_natural_height(h);
}

void RoomView::set_current_text(std::string text) {
    if (compose_bar_) compose_bar_->set_current_text(std::move(text));
}
