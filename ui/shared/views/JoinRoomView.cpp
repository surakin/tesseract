...
std::string join_rule_label(const std::string& rule) {
    ...
    if (rule == "knock_restricted")    return "Knock+Restricted";
...
JoinRoomView::JoinRoomView() {
    auto lookup = std::make_unique<tk::Button>("Look up",
        std::function<void()>{}, tk::Button::Variant::Primary);
    lookup->set_on_click([this] {
        if (on_lookup_requested && !alias_text_.empty())
            on_lookup_requested(alias_text_);
    });
    lookup_btn_ = add_child(std::move(lookup));

    auto join = std::make_unique<tk::Button>("Join",
        std::function<void()>{}, tk::Button::Variant::Primary);
    join->set_on_click([this] {
        if (on_join_requested) {
            std::string id = preview_.room_id.empty() ? alias_text_ : preview_.room_id;
            if (!id.empty()) on_join_requested(id);
        }
    });
...
void JoinRoomView::apply_state() {
    ...
    if (join_btn_)  join_btn_->set_visible(show_join);
...
void JoinRoomView::arrange(tk::LayoutCtx& ctx, tk::Rect bounds) {
    ...
    if (state_ == State::Preview) {
        ...
        float card_h = kCardPadY * 2.0f + info_h + (topic_h > 0 ? kSmallGap + topic_h : 0.0f);
...
void JoinRoomView::paint(tk::PaintCtx& ctx) {
    ...
    float y = bounds_.y + kPadY;
    {
        ...
        if (lo) ctx.canvas.draw_text(*lo, { x, y }, pal.text_primary);
    }
    y += kTitleH + kGap;
    ...
    if (alias_field_visible() && !alias_field_rect_.empty()) {
        ...
        ctx.canvas.stroke_rounded_rect(alias_field_rect_, kRadius, pal.border, kBorderW);
    }
    y += kInputH + kGap;
    ...
    if (state_ == State::Preview && !preview_card_rect_.empty()) {
        ...
        ctx.canvas.stroke_rounded_rect(preview_card_rect_, kRadius, pal.border, kBorderW);
        ...
        if (av_img) {
            ...
            std::string_view disp = preview_.name.empty()
                ? std::string_view("#") : std::string_view(preview_.name);
        ...
        if (!preview_.join_rule.empty()) {
            ...
            if (lo) {
                ...
                ctx.canvas.draw_text(*lo,
                    { info_x + kPillPadX, info_y + (kPillH - lo->measure().h) * 0.5f },
                    tk::Color{ 255, 255, 255, 255 });
        ...
        {
            std::string detail = std::to_string(preview_.num_joined_members) + " members";
            if (!preview_.encryption.empty()) detail += "  \xF0\x9F\x94\x92";  // UTF-8 🔒
            tk::TextStyle ts;
        ...
        if (!preview_.topic.empty()) {
            float topic_y  = preview_card_rect_.y + kCardPadY + kAvatarSize + kSmallGap;
            ...
            if (lo) ctx.canvas.draw_text(*lo, { cx, topic_y }, pal.text_secondary);
        }
    }

    // Buttons.
    if (join_btn_ && join_btn_->visible()) join_btn_->paint(ctx);
    if (cancel_btn_) cancel_btn_->paint(ctx);

    // Look up button.
    if (lookup_btn_ && lookup_btn_->visible()) lookup_btn_->paint(ctx);
}
