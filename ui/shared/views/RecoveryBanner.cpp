...
RecoveryBanner::RecoveryBanner() {
    ...
    verify->set_on_click([this] {
        if (on_verify) on_verify(current_key_);
    });
...
void RecoveryBanner::set_state(State s) {
    state_ = s;
    if (s != State::Failed)    failure_msg_.clear();
    if (s != State::Importing) imported_keys_ = 0;
    apply_state();
}
...
std::string RecoveryBanner::label_text() const {
    switch (state_) {
        case State::Form:      return "Verify this device:";
...
void RecoveryBanner::apply_state() {
    ...
    if (verify_) verify_->set_visible(show_field);
...
void RecoveryBanner::arrange(tk::LayoutCtx& ctx, tk::Rect bounds) {
    ...
    dismiss_rect_ = {
        bounds.x + bounds.w - kPadX - kDismissSide,
    ...
    if (show_field) {
        ...
        if (verify_) verify_size = verify_->measure(ctx,
            { kVerifyMinWidth, bounds.h - kPadY * 2 });
        verify_rect_ = {
            right_cursor - verify_size.w,
        ...
        key_field_rect_ = {
            right_cursor - kKeyFieldWidth,
            bounds.y + (bounds.h - 28.0f) * 0.5f,
            kKeyFieldWidth,
            28.0f
        };
    ...
    label_rect_ = {
        ...
        std::max(0.0f, right_cursor - (bounds.x + kPadX)),
        20.0f
    };

    if (label_)   label_->arrange(ctx, label_rect_);
    if (verify_)  verify_->arrange(ctx, verify_rect_);
    if (dismiss_) dismiss_->arrange(ctx, dismiss_rect_);
}
...
void RecoveryBanner::paint(tk::PaintCtx& ctx) {
    ...
    if (verify_ && verify_->visible())  verify_->paint(ctx);
    if (dismiss_)                       dismiss_->paint(ctx);
}
