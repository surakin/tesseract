...
void VideoViewerOverlay::open(std::string source_json,
...
void VideoViewerOverlay::set_video_player(
    ...
    video_player_->on_frame    = [this]() { if (request_repaint_) request_repaint_(); };
    video_player_->on_progress = [this]() { if (request_repaint_) request_repaint_(); };
}
...
void VideoViewerOverlay::recompute_layout() {
    ...
    scrub_bar_ = { scrub_x,
                   controls_bar_.y + (kCtrlBarH - kScrubH) * 0.5f,
                   scrub_w, kScrubH };

    close_btn_ = { b.x + b.w - (kCloseBtnS + 8.0f),
                   b.y + 8.0f,
...
void VideoViewerOverlay::paint(tk::PaintCtx& ctx) {
    ...
    if (display) {
        ...
        cv.stroke_rounded_rect(video_rect_, 4.0f, ctx.theme.palette.border, 1.0f);
    }
    ...
    {
        char rate_buf[8];
        if (rate_ >= 1.99f)      std::snprintf(rate_buf, sizeof(rate_buf), "2\xC3\x97");
        else if (rate_ >= 1.49f) std::snprintf(rate_buf, sizeof(rate_buf), "1.5\xC3\x97");
        else                     std::snprintf(rate_buf, sizeof(rate_buf), "1\xC3\x97");
    ...
    if (scrub_bar_.w > 0) {
        const std::uint64_t pos  = video_player_ ? video_player_->position_ms() : 0u;
        const std::uint64_t dur  = (video_player_ && video_player_->duration_ms() > 0)
                                   ? video_player_->duration_ms()
                                   : duration_ms_;
        const float frac = (dur > 0)
            ? std::clamp(static_cast<float>(pos) / static_cast<float>(dur), 0.0f, 1.0f)
            : 0.0f;
        ...
        cv.fill_rounded_rect({ kx, ky, kKnobD, kKnobD },
...
void VideoViewerOverlay::on_pointer_up(tk::Point local, bool inside_self) {
    ...
    if (press_close_) {
        press_close_ = false;
        if (inside_self && rect_contains(close_btn_, w)) close();
        return;
    }
    if (press_play_) {
        press_play_ = false;
        if (inside_self && rect_contains(play_btn_, w)) do_play_or_pause();
        return;
    }
    if (press_speed_) {
        press_speed_ = false;
        if (inside_self && rect_contains(speed_pill_, w)) cycle_speed();
        return;
    }
    if (press_scrub_) {
        press_scrub_ = false;
...
void VideoViewerOverlay::do_play_or_pause() {
    if (!video_player_) return;
    if (video_player_->is_playing()) video_player_->pause();
    else                             video_player_->resume();
}

void VideoViewerOverlay::cycle_speed() {
    if (!video_player_) return;
    if      (rate_ < 1.4f) rate_ = 1.5f;
    else if (rate_ < 1.9f) rate_ = 2.0f;
    else                   rate_ = 1.0f;
