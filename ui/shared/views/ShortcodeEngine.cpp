...
ShortcodeEngine::find_complete(std::string_view text, int cursor) const
{
    ...
    if (word_start == 0 || text[word_start - 1] != ':')
    {
        return std::nullopt;
    }
    if (word_start == word_end)
    {
        return std::nullopt; // empty word "::"
    }
                        ...
                        int max_results) const
{
    ...
    for (const auto& img : packs)
    {
        ...
        if (!any(img.usage & tesseract::PackUsage::Emoticon))
        {
            continue;
        }
        if (!icontains(img.shortcode, prefix))
        {
            continue;
        }
        ShortcodeSuggestion s;
