...
public:
    ...
    void paint_cell(std::size_t index, tk::PaintCtx& ctx, tk::Rect bounds,
        ...
        if (index >= owner_.current_items_.size()) return;

        if (selected) {
            ctx.canvas.fill_rounded_rect(
                bounds, 8.0f, ctx.theme.palette.subtle_pressed);
        } else if (hovered) {
            ctx.canvas.fill_rounded_rect(
                bounds, 8.0f, ctx.theme.palette.subtle_hover);
        }
        ...
        if (img) {
            ...
            tk::Rect ph{
                bounds.x + 8.0f, bounds.y + 8.0f,
                std::max(0.0f, bounds.w - 16.0f),
                std::max(0.0f, bounds.h - 16.0f)
            };
            ctx.canvas.fill_rounded_rect(
                ph, 6.0f, ctx.theme.palette.chrome_bg);
        }
    }
...
StickerPicker::StickerPicker()
    ...
    grid->on_cell_clicked = [this](int idx) {
        if (idx < 0 ||
            static_cast<std::size_t>(idx) >= current_items_.size()) return;
        if (on_selected) on_selected(current_items_[idx]);
    };
...
void StickerPicker::rebuild_current_items() {
    ...
    switch (page_) {
        ...
        case Page::Pack: {
            ...
            for (const auto& img :
                    (client_ ? client_->list_pack_images(
...
void StickerPicker::arrange(tk::LayoutCtx& ctx, tk::Rect bounds) {
    ...
    search_rect_ = {
        bounds.x + kPadding,
        bounds.y + kPadding,
        std::max(0.0f, bounds.w - kPadding * 2),
        kSearchHeight
    };
...
tk::Rect StickerPicker::tab_strip_rect() const { return tab_rect_; }
