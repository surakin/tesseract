...
// `current_room_id`: the personal pack, current_room_id's own pack, or any
// pack the user has explicitly subscribed to via
// im.ponies.emote_rooms/m.image_pack.rooms. Hides packs from any other
// room that only appears in list_image_packs() because that room was
// recently visited (see active_rooms in the Rust aggregator) but isn't
// the room in question and isn't subscribed.
...
// (if any), then every subscribed-room pack in their existing relative
// order. `current_room_id` empty means no room context (e.g. viewing the
// room list) — bucket 2 is simply empty in that case.
