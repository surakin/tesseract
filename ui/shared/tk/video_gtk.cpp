...
public:
    ...
    std::uint64_t position_ms() const override {
        if (!pipeline_) return 0u;
        gint64 pos = 0;
        if (!gst_element_query_position(pipeline_, GST_FORMAT_TIME, &pos)) return 0u;
        return static_cast<std::uint64_t>(pos / GST_MSECOND);
    }
    std::uint64_t duration_ms() const override {
        if (!pipeline_) return 0u;
        gint64 dur = 0;
        if (!gst_element_query_duration(pipeline_, GST_FORMAT_TIME, &dur)) return 0u;
...
private:
    void build_pipeline() {
        ...
        gst_bin_add_many(GST_BIN(pipe),
                         src, decode, aconv, asink, vconv, vsink, nullptr);
    ...
    void teardown_pipeline() {
        stop_timer();
        if (bus_watch_id_) { g_source_remove(bus_watch_id_); bus_watch_id_ = 0; }
        if (pipeline_) {
    ...
    static GstFlowReturn on_new_sample_(GstAppSink* sink, gpointer user_data) {
        ...
        if (w <= 0 || h <= 0) { gst_sample_unref(sample); return GST_FLOW_OK; }
    ...
    static void on_pad_added_(GstElement* /*dec*/, GstPad* pad, gpointer user_data) {
        ...
        const gchar* target_elem = g_str_has_prefix(name, "audio")
                                   ? "audioconvert" : "videoconvert";
        GstElement* sink_elem =
            gst_bin_get_by_name(GST_BIN(pipe), target_elem);
        if (!sink_elem) {
            ...
            while (gst_iterator_next(it, &val) == GST_ITERATOR_OK) {
                GstElement* e = static_cast<GstElement*>(
                    g_value_get_object(&val));
                GstElementFactory* f = gst_element_get_factory(e);
                if (f && g_str_has_prefix(
                        gst_element_factory_get_longname(f), target_elem)) {
    ...
    static gboolean bus_cb_(GstBus* /*bus*/, GstMessage* msg, gpointer user_data) {
        auto* self = static_cast<GtkVideoPlayer*>(user_data);
