...
void MediaDiskCache::store(const std::string& key, const std::vector<uint8_t>& bytes) const {
...
void MediaDiskCache::prune(std::uintmax_t max_bytes) const {
    struct Entry { fs::file_time_type mtime; std::uintmax_t size; fs::path path; };
    ...
    for (const auto& de : fs::directory_iterator(dir_, ec)) {
        ...
        if (ec) { ec.clear(); entries.back().mtime = {}; }
        total += sz;
    }
