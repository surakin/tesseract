...
public:
    ...
    void play(const std::uint8_t* data,
        ...
        GInputStream* stream =
            g_memory_input_stream_new_from_data(bytes_.data(), bytes_.size(),
                                                nullptr);
        ...
        bus_watch_id_ = gst_bus_add_watch(bus, &GtkAudioPlayer::on_bus_message, this);
    ...
    std::uint64_t position_ms() const override {
        ...
        gint64 pos = 0;
        if (gst_element_query_position(current_pipeline_, GST_FORMAT_TIME, &pos)) {
    ...
    std::uint64_t duration_ms() const override {
        if (!current_pipeline_) return 0;
        gint64 dur = 0;
        if (gst_element_query_duration(current_pipeline_, GST_FORMAT_TIME, &dur)) {
...
private:
    void send_seek_(gint64 position_ns) {
        if (!current_pipeline_) return;
        gst_element_seek(current_pipeline_,
                         static_cast<gdouble>(rate_),
                         GST_FORMAT_TIME,
