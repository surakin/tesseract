...
public:
    ...
    void play(const std::uint8_t* data,
        ...
        if (engine_) {
            ...
            stream.Attach(SHCreateMemStream(
                reinterpret_cast<const BYTE*>(bytes_.data()),
                static_cast<UINT>(bytes_.size())));
            Microsoft::WRL::ComPtr<IMFByteStream> mf_stream;
            if (stream &&
                SUCCEEDED(MFCreateMFByteStreamOnStream(stream.Get(),
    ...
    std::uint64_t duration_ms() const override {
        ...
        return (d > 0.0 && d < 1.0e10)
            ? static_cast<std::uint64_t>(d * 1000.0) : 0u;
    }
...
private:
    ...
    void decode_loop() {
        ...
        stream.Attach(SHCreateMemStream(
            reinterpret_cast<const BYTE*>(bytes_.data()),
            static_cast<UINT>(bytes_.size())));
        if (!stream) { CoUninitialize(); return; }
        ...
        if (FAILED(MFCreateSourceReaderFromByteStream(mf_stream.Get(),
                                                       attrs.Get(),
                                                       reader.GetAddressOf()))) {
            CoUninitialize(); return;
        }
        ...
        MFGetAttributeSize(actual_type.Get(), MF_MT_FRAME_SIZE,
                           &frame_w, &frame_h);
        if (frame_w == 0 || frame_h == 0) { CoUninitialize(); return; }
        ...
        while (decode_running_) {
            ...
            BYTE*  ptr  = nullptr;
            DWORD  len  = 0;
            if (FAILED(buf->Lock(&ptr, nullptr, &len))) continue;
            ...
            if (backend_) {
                ...
                if (img) {
                    ...
                    post_([alive = std::move(alive),
                           on_frm = std::move(on_frm)]() {
                        if (*alive && on_frm) on_frm();
                    });
                }
            }
    ...
    void start_timer() {
        if (timer_) return;
        CreateTimerQueueTimer(&timer_, nullptr, timer_cb_, this,
                              60, 60, WT_EXECUTEDEFAULT);
    }
    void stop_timer() {
        if (!timer_) return;
