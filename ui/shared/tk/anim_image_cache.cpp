                           ...
                           std::int64_t                            now_ms)
{
    ...
    entry.current   = 0;
    entry.next_advance_ms = now_ms + (entry.delays_ms.empty() ? 100 : entry.delays_ms[0]);
...
const tk::Image* AnimImageCache::current_frame(const std::string& key) const {
    ...
    if (it == entries_.end() || it->second.frames.empty()) return nullptr;
...
bool AnimImageCache::advance(std::int64_t now_ms) {
    ...
    for (auto& [_, entry] : entries_) {
        if (entry.frames.empty() || entry.delays_ms.size() != entry.frames.size())
            continue;
