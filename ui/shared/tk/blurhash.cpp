...
static const char kBase83Chars[] =
    "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz#$%*+,-.:;=?@[]^_{|}~";

int base83_decode_char(char c) {
    for (int i = 0; i < 83; ++i)
        if (kBase83Chars[i] == c) return i;
    return -1;
}
...
bool decode_blurhash(const std::string& hash, int w, int h,
    ...
    if (static_cast<int>(hash.size()) < expected) return false;
    ...
    // DC component (chars 2..5, 5-char base83 value → packed sRGB).
    {
        ...
        colors[0] = { srgb_to_linear(dc_r), srgb_to_linear(dc_g), srgb_to_linear(dc_b) };
    }
    ...
    for (int py = 0; py < h; ++py) {
        for (int px = 0; px < w; ++px) {
            ...
            for (int j = 0; j < comp_y; ++j) {
                float basis_y = std::cos((M_PI * static_cast<float>(j) * static_cast<float>(py))
                                         / static_cast<float>(h));
                for (int i = 0; i < comp_x; ++i) {
                    float basis = basis_y
                        * std::cos((M_PI * static_cast<float>(i) * static_cast<float>(px))
                                   / static_cast<float>(w));
                    const Color3& c = colors[static_cast<size_t>(j * comp_x + i)];
                    r += c.r * basis;
                    g += c.g * basis;
                    b += c.b * basis;
                }
