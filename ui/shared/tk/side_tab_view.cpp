...
void SideTabView::add_tab(std::string label, std::unique_ptr<Widget> content) {
    ...
    // Hide all content initially; select(0) below will show the first tab.
...
void SideTabView::select(int idx) {
    ...
    if (on_tab_selected) on_tab_selected(selected_idx_);
}
...
void SideTabView::arrange(LayoutCtx& ctx, Rect bounds) {
    ...
    for (int i = 0; i < static_cast<int>(tabs_.size()); ++i)
        ensure_layout(ctx, i, label_max_w);
...
void SideTabView::paint(PaintCtx& ctx) {
    ...
    for (int i = 0; i < static_cast<int>(tabs_.size()); ++i) {
        ...
        // Draw label text.
        ensure_layout_paint(ctx, i, label_w);
        if (t.layout) {
...
bool SideTabView::on_pointer_down(Point local) {
    ...
    if (local.x < 0 || local.x >= kSidebarWidth) return false;
...
void SideTabView::on_pointer_up(Point local, bool inside_self) {
    ...
    if (local.x < 0 || local.x >= kSidebarWidth) return;
    int idx = tab_at_y(local.y);
    if (idx >= 0 && idx == pressed) select(idx);
}

void SideTabView::on_pointer_move(Point local) {
...
void SideTabView::on_pointer_leave() {
    for (auto& t : tabs_) t.hovered = false;
}
...
void SideTabView::ensure_layout(LayoutCtx& ctx, int i, float max_w) {
    auto& t = tabs_[i];
    if (t.layout && t.layout_max_w == max_w) return;
    TextStyle st{};
...
void SideTabView::ensure_layout_paint(PaintCtx& ctx, int i, float max_w) {
    auto& t = tabs_[i];
    if (t.layout && t.layout_max_w == max_w) return;
    TextStyle st{};
    st.role      = FontRole::Body;
