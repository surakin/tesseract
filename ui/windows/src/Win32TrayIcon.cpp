...
LRESULT CALLBACK Win32TrayIcon::wnd_proc(HWND hwnd, UINT msg,
    ...
    case WM_TESSERACT_TRAY:
        if (self) self->on_tray_message(wParam, lParam);
        return 0;
    ...
    case WM_COMMAND:
        ...
        case kMenuShowId:
            if (self->on_show_) self->on_show_();
            return 0;
        case kMenuQuitId:
            if (self->on_quit_) self->on_quit_();
            return 0;
        }
        break;
    }
      ...
      on_quit_(std::move(on_quit))
{
    ...
    hwnd_ = CreateWindowExW(0, CLASS_NAME, L"", 0, 0, 0, 0, 0,
                            HWND_MESSAGE, nullptr, hInst_, nullptr);
    if (!hwnd_) return;
