...
struct FormatKey
{
    ...
    {
        return size_x100 == o.size_x100 && weight == o.weight &&
               slant == o.slant && halign == o.halign && valign == o.valign &&
               trim == o.trim && wrap == o.wrap && family == o.family;
    }
};
...
{
    FormatKey key{
        s.family, static_cast<int>(std::lround(dip_from_style(s) * 100.0f)),
        s.weight, s.slant,
        s.halign, s.valign,
        s.trim,   s.wrap,
    };
    ...
    HRESULT hr = g_dw->CreateTextFormat(
        s.family, nullptr, to_dw_weight(s.weight), to_dw_style(s.slant),
        DWRITE_FONT_STRETCH_NORMAL, dip_from_style(s), L"", &fmt);
    if (FAILED(hr) || !fmt)
    {
        return nullptr;
    }
    ...
    fmt->SetWordWrapping(s.wrap == Wrap::Word ? DWRITE_WORD_WRAPPING_WRAP
                                      ...
                                      float max_h_dip)
{
    ...
    HRESULT hr =
        g_dw->CreateTextLayout(text, len, fmt, max_w_dip, max_h_dip, &layout);
    if (FAILED(hr) || !layout)
    {
        return nullptr;
    }
...
bool init()
{
...

    {
        ...
        hr = DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED,
                                 __uuidof(IDWriteFactory2), unk.GetAddressOf());
        if (FAILED(hr) || !unk)
        {
            return false;
        }
        if (FAILED(unk.As(&g_dw)))
        {
            return false;
        }
    }

    if (!create_render_target(g_dpi))
    {
        return false;
    }
          ...
          const Style& s)
{
    ...
    if (brush)
    {
        g_rt->DrawTextLayout(D2D1::Point2F(0.0f, 0.0f), layout.Get(), brush,
...
Metrics measure(const wchar_t* text, int len, const Style& s, int max_width)
{
    Metrics out{0, 0, 0};
    if (!g_dw || !text)
    {
    ...
    if (!wrap)
    {
        eff.wrap = Wrap::NoWrap;
    }

    UINT32 ulen = (len < 0) ? 0xFFFFFFFFu : static_cast<UINT32>(len);
    ComPtr<IDWriteTextLayout> layout =
        make_layout(text, ulen, eff, w_dip, h_dip);
    if (!layout)
    {
        return out;
    }
    ...
    out.width = static_cast<int>(
        std::ceil(m.widthIncludingTrailingWhitespace * px_per_dip));
