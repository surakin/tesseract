...
- (instancetype)init
{
    ...
    _hsField->set_on_changed(
        [weakSelf](const std::string& text)
        {
            LoginView* s = weakSelf;
            if (!s || !s->_shared)
            {
            ...
            std::thread(
                [weakSelf, gen, snap, text]
                {
                    std::this_thread::sleep_for(std::chrono::milliseconds(300));
                    LoginView* strongSelf = weakSelf;
                    if (!strongSelf || gen != strongSelf->_discoveryGen.load())
                    {
                        return;
                    }
        ...
        [weakSelf, hs]
        {
            ...
            strongSelf->_surface->host().post_to_ui(
                [weakSelf, ok, payload = std::move(payload)]
                {
                    ...
                    if (s)
                    {
                        [s _onBeginCompleted:ok payload:payload];
                    }
                });
        });
}
        ...
        [weakSelf]
        {
            ...
            strongSelf->_surface->host().post_to_ui(
                [weakSelf, ok, msg = std::move(msg)]
                {
                    ...
                    if (s)
                    {
                        [s _onAwaitCompleted:ok message:msg];
                    }
                });
        });
}
...
- (void)_onAwaitCompleted:(bool)ok message:(std::string)err
{
    ...
    _shared->set_status("Sign-in failed: " + err, tk::Color::rgb(0xB00020));
    _shared->set_state(tesseract::views::LoginView::State::Form);
    _hsField->set_enabled(true);
    _surface->relayout();
}
