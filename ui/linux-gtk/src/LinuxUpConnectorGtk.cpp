...
struct UpSharedBusGtk {
    ...
    void dispatch_new_endpoint(const char* token, const char* endpoint) {
        ...
        if (it != routes_.end()) it->second->on_new_endpoint(endpoint);
    }
    void dispatch_unregistered(const char* token) {
        auto it = routes_.find(token);
        if (it != routes_.end()) it->second->on_unregistered();
    }
...
static const GDBusInterfaceVTable kConnector1Vtable = {
    handle_method_call, nullptr, nullptr
};
...
bool UpSharedBusGtk::acquire(GDBusConnection* bus) {
    ...
    if (!info) { g_error_free(err); return false; }
    ...
    if (!reg_id_) { g_error_free(err); active_ = false; }
    return active_;
}
...
std::string UpSharedBusGtk::find_distributor(GDBusConnection* bus) {
    ...
    for (gsize i = 0; i < n && found.empty(); ++i) {
        ...
        if (svc[0] == ':') { g_variant_unref(sv); continue; } // skip unique names
        ...
        GVariant* xml_v = g_dbus_connection_call_sync(
            ...
            G_DBUS_CALL_FLAGS_NONE, 200 /* ms */, nullptr, nullptr);
        if (xml_v) {
