...
- **Step 6 done — Device verification + key backup (recovery key)** — `needs_recovery`, `recover(key_or_passphrase)`, `backup_state` FFI; `on_backup_progress` callback. `RecoveryBanner` shared widget (in-toolkit banner; not a separate modal dialog) shown after login when `needs_recovery()` is true; "Verify" calls `recover(key)`, "Dismiss" hides the banner; recovery key input via `NativeTextField` with `set_password(true)`. All 4 shells wired.
- **Step 7 done — Spaces (room-list drill-in navigation)** — `is_space: bool` in `RoomInfo` (FFI + C++); spaces appear in the Spaces section of the room list. `space_children(space_id) -> Vec<String>` FFI via `m.space.child` state events. Stack-based drill-in navigation: selecting a space replaces the room list with its children; Back button (`←`) + space name label at the sidebar top; recursive sub-spaces supported; auto-pop when stack is empty. Space children hidden from the root room list; appear only when the user navigates into the parent space. All 4 shells wired.
...
- **Shutdown stability hardening** — Two-phase crash fix: `stop_sync` now takes a handler pointer and drains all background worker threads before tearing down the tokio runtime, preventing use-after-free when a worker posts back to the UI thread after the `EventHandler` is destroyed; separate fix prevents a double-callback segfault when `stop_sync` is called re-entrantly during the `SyncService` drop.
...
- **Initial-sync + key-backfill progress in the status bar — Qt6 / GTK4 / Win32** — Right after login the SDK can spend seconds-to-minutes hydrating the joined-room set (sliding-sync `Init → SettingUp → Running`) and downloading room keys from the encryption backup; the status bar previously said "Connected" the whole time. New FFI: a `room_list_state_code()` mapper in [sdk/src/client.rs](sdk/src/client.rs) that compresses `matrix_sdk_ui::room_list_service::State` to a 6-value `u8` (`Init/SettingUp/Recovering/Running/Error/Terminated`), plus an `on_room_list_state(state: u8)` extern-C++ callback on `EventHandlerBridge` and a 4th watcher task in `start_sync()` that subscribes to `SyncService::room_list_service().state()`, emits an initial snapshot, and forwards every transition. C++ side adds a `tesseract::RoomListState` enum mirroring the codes (`client/include/tesseract/types.h`) and an `on_room_list_state(RoomListState)` virtual on `IEventHandler` (default no-op so macOS — which has no status bar yet — compiles unchanged). Each shell caches `last_room_list_state_` + `last_backup_state_` + `last_imported_keys_` and recomputes a single combined string via `refreshSyncStatus()` / `refresh_sync_status()`: priority is `Init/SettingUp → "Syncing rooms…"`, `Recovering → "Reconnecting…"`, backup `Downloading → "Downloading encryption keys (N)…"`, otherwise clear back to "Connected". A 300 ms debounce on entry into the room-busy state suppresses the flash on quiet restored sessions where `Init→Running` happens in <500 ms (Qt6 `QTimer`, GTK4 `g_timeout_add`, Win32 `SetTimer`-based `WM_TIMER`). Win32 wiring: `WM_TESSERACT_ROOM_LIST_STATE = WM_APP + 21` (skipping +20 which the tray-icon HWND uses), routed through the existing wnd_proc. Pre-existing test-build break in `stop_sync()` (un-gated reference to a `#[cfg(not(test))]` field) fixed in passing so `cargo test -p tesseract-sdk-ffi` runs again. 2 new C++ Catch2 tests (`test_types.cpp`) for the `RoomListState` enum encoding + bridge clamp behaviour, 1 new Rust unit test exhaustively mapping `room_list_state_code` for all six variants; 191/191 ctest pass under Qt6 + GTK4, 68 Rust unit tests pass.
...
**Step 6 — Device verification + key backup (recovery key)** ✓ done — see status block above.

**Step 7 — Spaces (room-list drill-in navigation)** ✓ done — see status block above.
...
- **macOS** ✓ done — `UNUserNotificationCenter`; `UNUserNotificationCenterDelegate` on `MainWindowController`; in-foreground suppression via `willPresentNotification:withCompletionHandler:` (suppressed when window is key and source room is active); notification click → `_navigateToRoom:` via `didReceiveNotificationResponse:withCompletionHandler:`; authorization requested (alert + sound + badge) in `_afterAuthSucceeded`. UserNotifications + CoreMedia + AVFoundation + CoreVideo frameworks added to CMakeLists.
...
- ~~**Win32 MinGW build is unverified**~~ ✓ fixed — MF forward declarations added, link libraries lowercased for MinGW compatibility (commit 5c41d08).
...
- **Initial-sync progress not shown on macOS** — the new `on_room_list_state` callback is wired through the `IEventHandler` virtual everywhere, but `MainWindowController.mm` doesn't override it because macOS has no status-bar surface. Qt6 / GTK4 / Win32 show "Syncing rooms…" / "Downloading encryption keys (N)…" until both sliding-sync and key-backfill settle; macOS stays silent during that window. Follow-up: either add a thin status label below the sidebar's user strip, or append the progress text to the window title.
