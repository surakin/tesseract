...
pub mod ffi {
    ...
    extern "Rust" {
        ...
        /// Set the key that encrypts the matrix-sdk SQLite store for a
        /// restored session (looked up by the caller from the platform secret
        /// store). Must be called before `restore_session`. Pass an empty
        /// slice, or simply don't call this, for a session that predates
        /// store encryption — it opens unencrypted, permanently.
        fn set_store_key(self: &mut ClientFfi, key: &[u8]);

        /// Retrieve the store-encryption key — either one set via
        /// `set_store_key`, or one freshly generated by `oauth_begin` /
        /// `login_password` for a brand-new login. Call right after a new
        /// login succeeds and persist the result via the platform secret
        /// store. Returns an empty vector when no key is set.
        fn store_key(self: &ClientFfi) -> Vec<u8>;
