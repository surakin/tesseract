...
pub struct ClientFfi {
    ...
    /// Key encrypting the matrix-sdk SQLite store, if any. For a restored
    /// session, set via `set_store_key` (looked up by the C++ caller) before
    /// `restore_session` — `None` means this session predates store
    /// encryption and stays unencrypted permanently. For a fresh login,
    /// `oauth_begin` / `login_password` generate one lazily via
    /// `get_or_insert_with` and the caller retrieves it afterwards via
    /// `store_key()` to persist alongside the new session.
    pub(super) store_key: Option<[u8; 32]>,
...
impl ClientFfi {
    ...
    /// session. Callers should invoke this (when the C++ secret store has a
    /// key on file for the account) before `restore_session`. An empty slice
    /// or any length other than 32 bytes is ignored — the store then opens
    /// unencrypted, same as if this were never called.
    pub fn set_store_key(&mut self, key: &[u8]) {
    ...
    /// Retrieve the store-encryption key, if one is set — either passed in
    /// via `set_store_key`, or freshly generated by `oauth_begin` /
    /// `login_password` for a brand-new login. Callers persist this
    /// alongside the session immediately after a successful new login.
    /// Returns an empty `Vec` when no key is set.
    pub fn store_key(&self) -> Vec<u8> {
        self.store_key.map(|k| k.to_vec()).unwrap_or_default()
    }
...
mod tests {
    ...
    #[test]
    fn store_key_empty_by_default() {
        let c = ClientFfi::new();
        assert!(c.store_key().is_empty());
    }

    #[test]
    fn set_store_key_round_trips_32_bytes() {
        let mut c = ClientFfi::new();
        let key: Vec<u8> = (0u8..32).collect();
        c.set_store_key(&key);
        assert_eq!(c.store_key(), key);
    }

    #[test]
    fn set_store_key_ignores_wrong_length() {
        let mut c = ClientFfi::new();
        c.set_store_key(&[1, 2, 3]);
        assert!(c.store_key().is_empty(), "short key must be ignored");

        c.set_store_key(&[0u8; 32]);
        assert_eq!(c.store_key(), vec![0u8; 32]);

        // A subsequent wrong-length call leaves the previously-set key alone.
        c.set_store_key(&[9; 5]);
        assert_eq!(c.store_key(), vec![0u8; 32]);
    }

    #[test]
    fn set_store_key_ignores_empty_slice() {
        let mut c = ClientFfi::new();
        c.set_store_key(&[7u8; 32]);
        c.set_store_key(&[]);
        assert_eq!(c.store_key(), vec![7u8; 32], "empty slice must be a no-op");
    }
